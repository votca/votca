...
void KMCLight::Initialize(const char *filename, Property *options, const char *outputfile )
{
...
vector<Node*> KMCLight::LoadGraph()
{
    vector<Node*> node;
...
void KMCLight::InitBoxSize(vector<Node*> node)
{
...
void KMCLight::InitialRates(vector<Node*> node)
{
...
void KMCLight::RateUpdateCoulomb(vector<Node*> &node,  vector< Chargecarrier* > &carrier, CoulombMap &coulomb)
{
...
vector<double> KMCLight::RunVSSM(vector<Node*> node, double runtime, unsigned int numberofcharges, votca::tools::Random2 *RandomVariable, CoulombMap coulomb)
{
...
//        }
    ...
    while((stopcondition == "runtime" && simtime < runtime) || (stopcondition == "steps" && step < maxsteps))
    {
        ...
        {
            KMCLight::RateUpdateCoulomb(node,carrier,coulomb);
        }
        for(unsigned int i=0; i<numberofcharges; i++)
        {
...
void KMCLight::WriteOcc(vector<double> occP, vector<Node*> node)
{
    votca::tools::Database db;
...
bool KMCLight::EvaluateFrame()
{
    ...
    occP = KMCLight::RunVSSM(node, _runtime, _numberofcharges, RandomVariable, coulomb);

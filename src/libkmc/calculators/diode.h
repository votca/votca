...
CoulombMap Diode::LoadCoulomb(int numberofnodes)
{
...
void Diode::InitBoxSize(vector<Node*> node)
{
...
void Diode::InitialRates(vector<Node*> node)
{
...
void Diode::RateUpdateCoulomb(vector<Node*> &node,  vector< Chargecarrier* > &carrier, CoulombMap &coulomb)
{
...
vector<double> Diode::RunVSSM(vector<Node*> node, double runtime, unsigned int numberofcharges, votca::tools::Random2 *RandomVariable, CoulombMap coulomb)
{
...
//        }
    ...
    while((stopcondition == "runtime" && simtime < runtime) || (stopcondition == "steps" && step < maxsteps))
    {
        ...
        {
            Diode::RateUpdateCoulomb(node,carrier,coulomb);
        }
        for(unsigned int i=0; i<numberofcharges; i++)
        {
...
void Diode::WriteOcc(vector<double> occP, vector<Node*> node)
{
    votca::tools::Database db;
...
bool Diode::EvaluateFrame()
{
    ...
    occP = Diode::RunVSSM(node, _runtime, _numberofcharges, RandomVariable, coulomb);
    
    // write occupation probabilites
    Diode::WriteOcc(occP, node);

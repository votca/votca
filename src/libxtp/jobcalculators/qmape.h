...
class QMAPE : public ctp::ParallelXJobCalc<vector<ctp::Job *>, ctp::Job *,
                                           ctp::Job::JobResult> {
...
void QMAPE::Initialize(Property *options) {
  ...
  _mps_table =
      options->ifExistsReturnElseThrowRuntimeError<string>(key + ".mps_table");
  _polar_bg_arch =
      options->ifExistsReturnElseReturnDefault<string>(key + ".polar_bg", "");
  _pdb_check =
      options->ifExistsReturnElseReturnDefault<bool>(key + ".pdb_check", false);
  _ptop_check = options->ifExistsReturnElseReturnDefault<bool>(
      key + ".ptop_check", false);

  key = "options." + Identify();

  string package_xml = options->ifExistsReturnElseThrowRuntimeError<string>(
      key + ".dft.dftengine");
...
ctp::XJob QMAPE::ProcessInputString(ctp::Job *job, ctp::Topology *top,
                                    ctp::QMThread *thread) {
...
ctp::Job::JobResult QMAPE::EvalJob(ctp::Topology *top, ctp::Job *job,
                                   ctp::QMThread *thread) {
  ...
  if (_polar_bg_arch == "") {
    ...
    CTP_LOG(ctp::logINFO, *log)
        << "Mps-Mapper: Generate FGC, load FGN BGN from '" << _polar_bg_arch
        << "'" << flush;
  ...
  // SETUP MM METHOD
  ctp::PEwald3D3D cape =
      ctp::PEwald3D3D(top, xjob.getPolarTop(), _options, thread->getLogger());
  if (_pdb_check) cape.WriteDensitiesPDB(xjob.getTag() + ".densities.pdb");

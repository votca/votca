...
void IQM::ParseOptionsXML(votca::tools::Property& opt) {
  ...
  // read linker groups
  string linker =
      opt.ifExistsReturnElseReturnDefault<string>(key + ".linker_names", "");
...
void IQM::addLinkers(std::vector<ctp::Segment*>& segments, ctp::Topology* top) {
  ...
  if (moleculeIdSeg1 == moleculeIdSeg2) {  // Check that both segments belong to
    ...
    std::vector<ctp::Segment*> segmentsInMolecule =
...
void IQM::SetJobToFailed(ctp::Job::JobResult& jres, ctp::Logger* pLog,
...
ctp::Job::JobResult IQM::EvalJob(ctp::Topology* top, ctp::Job* job,
                                 ctp::QMThread* opThread) {
  ...
  string orbFileA = (arg_pathA / eqm_work_dir / "molecules" / frame_dir /
                     (format("%1%_%2%%3%") % "molecule" % ID_A % ".orb").str())
                        .c_str();
  string orbFileB = (arg_pathB / eqm_work_dir / "molecules" / frame_dir /
  ...
  CTP_LOG(ctp::logINFO, *pLog)
      << ctp::TimeStamp() << " Evaluating pair " << job_ID << " [" << ID_A
      << ":" << ID_B << "] out of " << (top->NBList()).size() << flush;
    ...
    if (_do_dft_input) {
      ...
      if (qmpackage->GuessRequested()) {
        ...
        if (qmpackage->getPackageName() == "orca") {
          CTP_LOG(ctp::logINFO, *pLog)
              << "Copying monomer .gbw files to pair folder" << flush;
          string gbwFileA =
          ...
          CTP_LOG(ctp::logDEBUG, *pLog)
              << "Constructing the guess for dimer orbitals" << flush;
          orbitalsAB.PrepareDimerGuess(orbitalsA, orbitalsB);
        }
      } else {
        CTP_LOG(ctp::logINFO, *pLog)
            << "No Guess requested, starting from DFT starting Guess" << flush;
      }
  ...
  if (_do_gwbse) {
    try {
      ...
      gwbse_logger.setPreface(ctp::logWARNING,
                              (format("\nGWBSE WAR ...")).str());
  ...
  if (_do_bsecoupling) {
    ...
    try {
      ...
      bsecoupling_logger.setPreface(ctp::logINFO,
                                    (format("\nGWBSE INF ...")).str());
      bsecoupling_logger.setPreface(ctp::logERROR,
                                    (format("\nGWBSE ERR ...")).str());
      bsecoupling_logger.setPreface(ctp::logWARNING,
                                    (format("\nGWBSE WAR ...")).str());
      bsecoupling_logger.setPreface(ctp::logDEBUG,
                                    (format("\nGWBSE DBG ...")).str());
  ...
  if (_store_dft || _store_gw || _store_singlets || _store_triplets) {
    ...
    CTP_LOG(ctp::logDEBUG, *pLog)
        << "Orb file is not saved according to options " << flush;
  }
...
void IQM::ReadJobFile(ctp::Topology* top) {
  ...
  for (ctp::QMPair* pair : top->NBList()) {
    ...
    if (ptype != ctp::QMPair::PairType::Hopping &&
      ...
      cout << "WARNING Pair " << pair->getId()
           << " is not of any of the "
              "Hopping or SuperExchangeAndHopping type. Skipping pair"
           << flush;
      continue;
    }

...
class QMMM : public ctp::ParallelXJobCalc<vector<ctp::Job *>, ctp::Job *,
                                          ctp::Job::JobResult> {
 ...
 public:
  ...
  ctp::Job::JobResult EvalJob(ctp::Topology *top, ctp::Job *job,
                              ctp::QMThread *thread);
  ctp::XJob ProcessInputString(ctp::Job *job, ctp::Topology *top,
                               ctp::QMThread *thread);
...
void QMMM::Initialize(Property *options) {
  ...
  _xml_file =
      options->ifExistsReturnElseThrowRuntimeError<string>(key + ".mapping");
  _jobfile =
      options->ifExistsReturnElseThrowRuntimeError<string>(key + ".job_file");
  _emp_file =
      options->ifExistsReturnElseThrowRuntimeError<string>(key + ".emp_file");

  key = "options." + Identify() + ".coulombmethod";

  std::vector<string> choices = {"cutoff", "cut-off"};
  _method = options->ifExistsAndinListReturnElseThrowRuntimeError<string>(
      key + ".method", choices);
  _cutoff1 =
      options->ifExistsReturnElseThrowRuntimeError<double>(key + ".cutoff1");
  _cutoff2 = options->ifExistsReturnElseReturnDefault<double>(key + ".cutoff2",
                                                              _cutoff1);
  ...
  _subthreads =
      options->ifExistsReturnElseReturnDefault<int>(key + ".subthreads", 1);

  key = "options." + Identify();

  string package_xml =
      options->ifExistsReturnElseThrowRuntimeError<string>(key + ".dftpackage");
  ...
  if (options->exists(key)) {
    string gwbse_xml = options->ifExistsReturnElseThrowRuntimeError<string>(
        key + ".gwbse_options");
...
void QMMM::CustomizeLogger(ctp::QMThread *thread) {
  ...
  log->setPreface(ctp::logERROR,
                  (format("\nT%1$02d ERR ...") % thread->getId()).str());
  log->setPreface(ctp::logWARNING,
...
ctp::XJob QMMM::ProcessInputString(ctp::Job *job, ctp::Topology *top,
                                   ctp::QMThread *thread) {
...
ctp::Job::JobResult QMMM::EvalJob(ctp::Topology *top, ctp::Job *job,
                                  ctp::QMThread *thread) {
  ...
  qlog.setPreface(ctp::logINFO,
                  (format("\nQ%1$02d ... ...") % thread->getId()).str());
  qlog.setPreface(ctp::logERROR,
                  (format("\nQ%1$02d ERR ...") % thread->getId()).str());
  qlog.setPreface(ctp::logWARNING,
                  (format("\nQ%1$02d WAR ...") % thread->getId()).str());
  qlog.setPreface(ctp::logDEBUG,
  ...
  if (_cutoff2 > 0.5 * min) {
    throw runtime_error(
        (format("Cutoff is larger than half the box size. Maximum allowed "
                "cutoff is %1$1.1f - molecule extension.") %
         (0.5 * min))
            .str());
  }
  ...
  QMMachine machine =
      QMMachine(&xjob, &xind, qmpack, &_options, "options." + Identify());

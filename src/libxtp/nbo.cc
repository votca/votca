...
void NBO::EvaluateNBO(std::vector<QMAtom*>& _atomlist,
  ...
  Eigen::MatrixXd PNAOs_trans =
      IntercenterOrthogonalisation(P, S, _atomlist, bs);
...
Eigen::MatrixXd NBO::IntercenterOrthogonalisation(
    Eigen::MatrixXd& P, Eigen::MatrixXd& overlap,
    std::vector<QMAtom*>& _atomlist, BasisSet& bs) {

  // make an array which stores for each atom the the starting point for all s
  // functions, p functions, d functions etc...
  ...
  for (unsigned i = 0; i < sorting.size(); i++) {
    ...
    for (it_type iterator = sorting[i].begin(); iterator != sorting[i].end();
      ...
      for (unsigned i = 0; i < size; i++) {
        ...
        for (unsigned j = 0; j < size; j++) {
          for (int m = 0; m < ll; m++) {
            transformation(index[i] + m, index[j] + m) =
                es.eigenvectors()(i, j);
          }
        }
      }

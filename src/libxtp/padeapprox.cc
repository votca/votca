...
namespace xtp {
    ...
    void PadeApprox::addPoint(std::complex<double> w, Eigen::MatrixXcd val){
        ...
        if(_coeff.at(_coeff.size()-1).norm()!=_coeff.at(_coeff.size()-1).norm()){
            std::cout<<"reject point, unvalid coeficient at w="<<w<<std::endl;//<<_coeff.at(_coeff.size()-1)<<std::endl<<std::endl;
            _coeff.pop_back();
            _imgrid.pop_back();
            _imval.pop_back();
            _rejected_points++;
    ...
    Eigen::MatrixXcd PadeApprox::RecursiveA(std::complex<double> w, int n){
        ...
        if(n==0){
            ...
            Eigen::MatrixXcd A = RecursiveA(w, n-1)+(w-_imgrid.at(n-2))*_coeff.at(n-1)*RecursiveA(w, n-2);
            return A;
        }
    ...
    Eigen::MatrixXcd PadeApprox::RecursiveB(std::complex<double> w, int n){
        ...
        if(n==0){
            ...
            Eigen::MatrixXcd B = RecursiveB(w, n-1)+(w-_imgrid.at(n-2))*_coeff.at(n-1)*RecursiveB(w, n-2);
            return B;
        }
    }
    
    void PadeApprox::initialize(int basis_size){
    ...
    Eigen::MatrixXcd PadeApprox::evaluate(std::complex<double> w){
        ...
        if(abs(RecursiveB(w, _imgrid.size()).determinant())==0){
            std::cout<<std::endl<<"Warning, Matrix close to singular, det="<<abs(RecursiveB(w, _imgrid.size()).determinant())<<std::endl;
            return Eigen::MatrixXcd(_basis_size, _basis_size);
        }
        return RecursiveB(w, _imgrid.size()).inverse()*RecursiveA(w,_imgrid.size());     
    }

...
namespace xtp {
    ...
    Eigen::MatrixXcd SternheimerW::SternheimerLHS(Eigen::MatrixXcd hamiltonian, Eigen::MatrixXcd overlap, double eps, std::complex<double> omega, bool pm){
    ...
    Eigen::VectorXcd SternheimerW::SternheimerRHS(Eigen::MatrixXcd overlap, Eigen::MatrixXcd density, Eigen::MatrixXcd pertubation, Eigen::VectorXcd coeff){
    ...
    Eigen::VectorXcd SternheimerW::SternheimerSolve(Eigen::MatrixXcd& LHS, Eigen::VectorXcd& RHS){
    ...
    std::vector<Eigen::MatrixXcd> SternheimerW::DeltaNOSOP(std::vector<std::complex<double>> w, Eigen::Vector3d r){
            ...
            for(int v=0; v<_basis_size; v++){
                ...
                for(int i=0;i<_basis_size; i++){
                    ...
                    for(int j=0;j<_basis_size; j++){
                        ...
                        delta_n.at(m)(i,j)=delta_n.at(m)(i,j)+2*_mo_coefficients(i,v)*delta_c_p.at(m)(j,v)+2*_mo_coefficients(i,v)*delta_c_m.at(m)(j,v);                        
                    }
    ...
    std::vector<Eigen::MatrixXcd> SternheimerW::Polarisability(std::vector<std::complex<double>> grid_w,std::vector<std::complex<double>> w, std::string gridtype){
                    ...
                    for(int j=0;j<3;j++){
                        for(int n=0; n<basis_size; n++){
                            for(int m=0; m<basis_size; m++){
                                ...
                                r_x=gridcont(i);
                                Polar.at(o)(i,j)=Polar.at(o)(i,j)+delta_n.at(o)(n,m)*r_x*grid.at(r).first*dipole.Matrix()[j](n,m);    
                            }
                        }
                    }
                } 
        ...
        for(int i=0;i<Polar.size();i++){
            //std::cout<<"Complex Polar at w="<<grid_w.at(i)<<std::endl<<Polar.at(i)<<std::endl;
           _pade.addPoint(grid_w.at(i),Polar.at(i));
           //std::cout<<"Complex Polar* at w*="<<conj(grid_w.at(i))<<std::endl<<Polar.at(i).adjoint()<<std::endl;

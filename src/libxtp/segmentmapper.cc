...
void SegmentMapper<AtomContainer, mapAtom>::ParseFragment(Seginfo& seginfo, const tools::Property& frag){
    tools::Tokenizer tok_map_atoms(frag.get(_mapatom_xml["atoms"]).as<std::string>(), " \t\n");
    ...
    if(md_atoms.size()!=map_atoms.size()){
        throw std::runtime_error("Mapping for segment "+seginfo.segname+
                " fragment "+frag.get("name").as<std::string>()+" does not have same numbers of md and "+_mapatom_xml["atoms"]+"."
    ...
    if(md_atoms.size()!=weights.size()){
        throw std::runtime_error("Mapping for segment "+seginfo.segname+
                ...
                "If you want to leave a "+_mapatom_xml["name"]+" out, place a '0' instead");
    }
...
void SegmentMapper<AtomContainer, mapAtom>::LoadMappingFile(const std::string& mapfile){
  ...
  for (tools::Property* mol : molecules) {
    ...
    for (tools::Property* seg : segments) {
        ...
        int map_atom_min_id = std::min_element(seginfo.mapatoms.begin(), seginfo.mapatoms.end(),[](const mapsite_id& a, const mapsite_id & b) {
            return a.first < b.first;
        })->first;            
        if(map_atom_min_id!=0){
            throw std::runtime_error(_mapatom_xml["atoms"]+" for segment "+seginfo.segname+" do not start at zero index. Each segment should have its own coordinate file");
        }
...
template <class AtomContainer, class mapAtom>
  std::pair<int,std::string> SegmentMapper<AtomContainer, mapAtom>::StringToMapIndex(const std::string& map_string)const{
      tools::Tokenizer tok(map_string,":");
      std::vector<std::string> result=tok.ToVector();
      if(result.size()!=2){
        throw std::runtime_error("Entry "+map_string+" is not properly formatted.");
      }
      return std::pair<int,std::string>(std::stoi(result[0]),result[1]);
  }
template <class AtomContainer, class mapAtom>
  MD_atom_id SegmentMapper<AtomContainer, mapAtom>::StringToMDIndex(const std::string& md_string)const{
...
template <class AtomContainer, class mapAtom>
std::pair<int,int> SegmentMapper<AtomContainer, mapAtom>::CalcResidueRange(const std::vector<MD_atom_id>& seg)const{
...
template <class AtomContainer, class mapAtom>
std::pair<int,int> SegmentMapper<AtomContainer, mapAtom>::CalcResidueRange(const Segment& seg)const{
...
template <class AtomContainer, class mapAtom>
void SegmentMapper<AtomContainer, mapAtom>::PlaceMapAtomonMD(const std::vector<mapAtom*>& fragment_mapatoms,
...
template <class AtomContainer, class mapAtom>
int SegmentMapper<AtomContainer, mapAtom>::FindVectorIndexFromAtomId(int atomid,
...
template <class AtomContainer, class mapAtom>
void SegmentMapper<AtomContainer, mapAtom>::MapMapAtomonMD(const FragInfo& frag,
    ...
    if(symmetry>1){
        ...
        if(x_map.squaredNorm()<1e-18 || y_map.squaredNorm()<1e-18 || z_map.squaredNorm()<1e-18 ){
            throw std::runtime_error(_mapatom_xml.at("tag")+" basis vectors are very small, choose different local basis");
        }
    ...
    for(mapAtom* atom:fragment_mapatoms){
        if(getRank(*atom)>0 && symmetry<3){
            throw std::runtime_error("Local frame has less than 3 atoms, thus higher rank multipoles cannot be mapped.");
        }
...
template <class AtomContainer, class mapAtom>
AtomContainer SegmentMapper<AtomContainer, mapAtom>::map(const Segment& seg,QMState state)const{
   ...
   std::string coordsfilename=_mapatom_xml.at("coords")+"_"+state.ToString();
   if (seginfo.coordfiles.count(coordsfilename) == 0){
    throw std::runtime_error("Could not find a coordinate file for segment/state: "+coordsfilename);
   }
   ...
   if(int(seginfo.mapatoms.size())!=Result.size()){
        throw std::runtime_error(_mapatom_xml.at("tag")+"Segment '"+seg.getName()+
                "' does not contain the same number of atoms as mapping file: "+
                std::to_string(seginfo.mapatoms.size())+" vs. "+std::to_string(Result.size()));
    }

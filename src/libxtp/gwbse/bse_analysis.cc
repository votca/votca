    ...
    namespace xtp {
        ...
        void GWBSE::BSE_FragmentPopulations(ub::matrix<real_gwbse>& _bse_coefficients, std::vector<double>& popHA, std::vector<double>& popEA, 
            ...
            if (_fragA > 0) {
                ...
                CTP_LOG(ctp::logDEBUG, *_pLog) << ctp::TimeStamp() << " Running Excitation fragment population analysis " << flush;
                ...
                CTP_LOG(ctp::logDEBUG, *_pLog) << ctp::TimeStamp() << " --- complete! " << flush;
        ...
        void GWBSE::BSE_analyze_triplets() {
            ...
            for (int _i = 0; _i < _bse_nprint; _i++) {
                ...
                for (unsigned _i_bse = 0; _i_bse < _bse_size; _i_bse++) {
                    ...
                    if (_weight > 0.2) {
                        CTP_LOG(ctp::logINFO, *_pLog) << (format("           HOMO-%1$-3d -> LUMO+%2$-3d  : %3$3.1f%%") % (_homo - _index2v[_i_bse]) % (_index2c[_i_bse] - _homo - 1) % (100.0 * _weight)).str() << flush;
                    }
                }
                // results of fragment population analysis 
                if (_fragA > 0) {
                    CTP_LOG(ctp::logINFO, *_pLog) << (format("           Fragment A -- hole: %1$5.1f%%  electron: %2$5.1f%%  dQ: %3$+5.2f  Qeff: %4$+5.2f") % (100.0 * _popHA[_i]) % (100.0 * _popEA[_i]) % (_CrgsA[_i]) % (_CrgsA[_i] + _popA)).str() << flush;
                    CTP_LOG(ctp::logINFO, *_pLog) << (format("           Fragment B -- hole: %1$5.1f%%  electron: %2$5.1f%%  dQ: %3$+5.2f  Qeff: %4$+5.2f") % (100.0 * _popHB[_i]) % (100.0 * _popEB[_i]) % (_CrgsB[_i]) % (_CrgsB[_i] + _popB)).str() << flush;
                }
                CTP_LOG(ctp::logINFO, *_pLog) << (format("   ")).str() << flush;
            }

...
namespace votca {
  ...
  void Sigma::PrepareScreening(const RPA& rpa){
      _ppm.PPM_construct_parameters(rpa);
      _Mmn.MultiplyRightWithAuxMatrix(_ppm.getPpm_phi());
  }

   Eigen::VectorXd Sigma::CalcCorrelationDiag(const Eigen::VectorXd& frequencies, const Eigen::VectorXd& RPAEnergies)const{
...
#pragma omp parallel for
     ...
     void Sigma::Stabilize(Eigen::ArrayXd& denom)const{
   ...
   Eigen::MatrixXd Sigma::CalcCorrelationOffDiag(const Eigen::VectorXd& frequencies, const Eigen::VectorXd& RPAEnergies)const{

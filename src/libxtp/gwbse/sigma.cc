...
namespace votca {
  namespace xtp {
    ...
    void Sigma::CalcdiagElements(const TCMatrix_gwbse& _Mmn, const PPM & ppm) {
      ...
      unsigned levelsum = _Mmn.get_ntot(); // total number of bands
      unsigned gwsize = _Mmn.getAuxDimension(); // size of the GW basis
      const double pi = boost::math::constants::pi<double>();
      const double fourpi=4*pi;
#pragma omp parallel for
      for (unsigned gw_level = 0; gw_level < _qptotal; gw_level++) {
        const MatrixXfd & Mmn = _Mmn[ gw_level + _qpmin ];
        double sigma_x = 0;
        for (unsigned i_gw = 0; i_gw < gwsize; i_gw++) {
          ...
          for (unsigned _i_occ = 0; _i_occ <= _homo; _i_occ++) {
            sigma_x -= Mmn( _i_occ,i_gw) * Mmn( _i_occ,i_gw);
          } // occupied bands
        } // gwbasis functions             
          ...
          for (unsigned i_gw = 0; i_gw < gwsize; i_gw++) {
            ...
            for (unsigned i = 0; i < _homo+1; i++) {
              ...
              if (std::abs(denom) < 0.25) {
                _stab = 0.5 * (1.0 - std::cos(fourpi * denom));
              }
              const double _factor = _stab / denom; //Hartree
            ...
            for (unsigned i = _homo+1; i < levelsum; i++) {
              ...
              // sigma_c diagonal elements
              sigma_c_loc += factor * Mmn(i,i_gw) * Mmn(i,i_gw);
            }// bands

...
namespace votca {
    namespace xtp {
        ...
        ub::matrix<double> GWBSE::RPA_imaginary(const TCMatrix& _Mmn_RPA, double _shift, ub::vector<double>& _dft_energies,const ub::matrix<double>& screening_freq){
             int _size = _Mmn_RPA[0].size1(); // size of gwbasis
             ub::matrix<double> epsilon_temp=ub::zero_matrix(_rpamax-_rpamin,_rpamax-_rpamin);
                ...
                for ( int _m_level = 0; _m_level < _Mmn_RPA.get_mtot() ; _m_level++ ){
                    ...
                    for ( int _n_level = 0 ; _n_level < _Mmn_RPA.get_ntot() ; _n_level++ ){
                        ...
                        // this only works, if we have either purely real or purely imaginary frequencies
                            ...
                            // purely imaginary
                        double     _energy_factor = 8.0 * _deltaE / (_deltaE*_deltaE + screening_freq( _i_freq, 1) *  screening_freq( _i_freq, 1 ));
        ...
        //real
        ub::matrix<double> GWBSE::RPA_real(const TCMatrix& _Mmn_RPA, double _shift, ub::vector<double>& _dft_energies,const ub::matrix<double>& screening_freq){
             int _size = _Mmn_RPA[0].size1(); // size of gwbasis
             ub::matrix<double> epsilon_temp=ub::zero_matrix(_rpamax-_rpamin,_rpamax-_rpamin);
             #pragma omp parallel for 
                for ( int _m_level = 0; _m_level < _Mmn_RPA.get_mtot() ; _m_level++ ){
                    ...
                    for ( int _n_level = 0 ; _n_level < _Mmn_RPA.get_ntot() ; _n_level++ ){
                        ...
                        // this only works, if we have either purely real or purely imaginary frequencies
                            ...
                            // purely real
                        double _energy_factor = 4.0 * (1.0 / (_deltaE - screening_freq( _i_freq, 0 ) ) +  1.0 / (_deltaE + screening_freq( _i_freq, 0 ) ) );
        ...
        void GWBSE::RPA_calculate_epsilon(TCMatrix& _Mmn_RPA, const ub::matrix<double>& screening_freq, double _shift, const ub::vector<double>& _dft_energies){
            ...
            for ( unsigned _i_freq = 0 ; _i_freq < screening_freq.size1() ; _i_freq++ ){
                 ...
                 if ( screening_freq( _i_freq, 0) == 0.0 ) {
                     _epsilon[ _i_freq ]+=RPA_imaginary(_Mmn_RPA, _shift, _dft_energies,screening_freq);
                 }
                 
                 else if ( screening_freq( _i_freq, 1) == 0.0  ) {
                            // purely real
                            _epsilon[ _i_freq ]+=RPA_real(_Mmn_RPA, _shift, _dft_energies,screening_freq);
                        } 
                 else {
                            // mixed -> FAIL
    ...
    void GWBSE::RPA_prepare_threecenters(TCMatrix& _Mmn_RPA,const TCMatrix& _Mmn_full,const AOBasis& gwbasis,const AOMatrix& gwoverlap,const AOMatrix& gwoverlap_inverse     ){

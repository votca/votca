...
RowVectorXfd BSE_MF::row(int index) const
{
  
    std::cout << "BSE_MF.row() not defined in class" << std::endl;
    exit(0);
}

// virtual here : get a col of the operator
VectorXfd BSE_MF::col(int index) const
{

    std::cout << "BSE_MF.col() not defined in class" << std::endl;
    exit(0);
}
...
void BSE_MF::SetupDirectInteractionOperator() {
    ...
    void BSE_MF::Solve_singlets_TDA() {
      MatrixXfd H = MatrixXfd::Zero(_bse_size,_bse_size);
      Add_Hd<real_gwbse>(H);
      Add_Hqp<real_gwbse>(H);
  ...
  void BSE_MF::SetupHt(){
    ...
    void BSE_MF::Solve_singlets_BTDA() {
     ...
     Eigen::MatrixXd LmT = AmB.inverse().transpose();
      ...
      return;
        ...
        template <typename T>
        void BSE_MF::Add_Hqp(Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic>& H) {
...
template <typename T>
    void BSE_MF::Add_Hd(Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic>& H) {
...
template <typename T, int factor>
        void BSE_MF::Add_Hd2(Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic>& H) {
            int auxsize = _Mmn.auxsize();
            vc2index vc = vc2index(0, 0, _bse_ctotal);
#pragma omp parallel for       
            for (int c1 = 0; c1 < _bse_ctotal; c1++) {
...
template <typename T,int factor>
    void BSE_MF::Add_Hx(Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic>& H) { 
      int auxsize = _Mmn.auxsize();
       vc2index vc=vc2index(0,0,_bse_ctotal);
#pragma omp parallel for
            for (int v1 = 0; v1 < _bse_vtotal; v1++) {
    ...
    void BSE_MF::printFragInfo(const Population& pop, int i){
    ...
    void BSE_MF::printWeights(int i_bse, double weight){
    ...
    void BSE_MF::Analyze_singlets(const AOBasis& dftbasis) {
    ...
    void BSE_MF::Analyze_triplets(const AOBasis& dftbasis) {
    ...
    Eigen::VectorXd BSE_MF::Analyze_IndividualContribution(const QMStateType& type,const MatrixXfd& H){
        ...
        if (type == QMStateType::Singlet) {
            ...
            throw std::runtime_error("BSE_MF::Analyze_eh_interaction:Spin not known!");
        }
        return contrib;
    }

    BSE_MF::Interaction BSE_MF::Analyze_eh_interaction(const QMStateType& type) {
    ...
    BSE_MF::Population BSE_MF::FragmentPopulations(const QMStateType& type, const AOBasis& dftbasis) {
      Population pop;
    ...
    std::vector<Eigen::MatrixXd > BSE_MF::CalcFreeTransition_Dipoles(const AOBasis& dftbasis) {
    ...
    std::vector<tools::vec > BSE_MF::CalcCoupledTransition_Dipoles(const AOBasis& dftbasis) {

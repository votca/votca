...
Eigen::VectorXd QPGrid::Evaluate(const Eigen::VectorXd& gwa_energies) {
  ...
  for (Index i_node = 0; i_node < _opt.steps; ++i_node) {
    sigc_mat.col(i_node) =
        _sigma.CalcCorrelationDiag(gwa_energies.array() + offset[i_node]);
  }
  ...
  for (Index level = 0; level < qptotal; ++level) {
    ...
    const Eigen::VectorXd targ =
        sigc.array() + _energy_intercept[level] - freq.array();
    double qp_energy;
...
bool QPGrid::FindQPEnergy(const Eigen::VectorXd& freq,
                          const Eigen::VectorXd& sigc,
                          const Eigen::VectorXd& targ,
                          double& qp_energy) const {
  double pole_weight_max = -1.0;
  for (Index i_node = 0; i_node < _opt.steps - 1; ++i_node) {
    if (targ[i_node] * targ[i_node + 1] < 0.0) {  // Sign change
      double dsigc_dfreq =
          (sigc[i_node + 1] - sigc[i_node]) / (freq[i_node + 1] - freq[i_node]);
      double dtarg_dfreq =
          (targ[i_node + 1] - targ[i_node]) / (freq[i_node + 1] - freq[i_node]);
      double pole =

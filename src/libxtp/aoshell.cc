...
int AOShell::detlmax( string shell_type ) {
    ...
    if ( shell_type.length() == 1 ){
       ...
       else if ( shell_type == "P" ){ _lmax = 1;}
       else if ( shell_type == "D" ){ _lmax = 2;}
       else if ( shell_type == "F" ){ _lmax = 3;}
       else if ( shell_type == "G" ){ _lmax = 4;}
       else if ( shell_type == "H" ){ _lmax = 5;} 
       else if ( shell_type == "I" ){ _lmax = 6;} 
       else{
           throw runtime_error("detlmax: shell_type not known");
       }
    } else {
...
///                  gradAOvalues(i_act, 2) += factor_1 * (6. * cz_cz) * _expofactor - 2. * center_z * alpha * AOvalue;
                  ...
                  _i_func = i_act;
                    ...
                    cerr << " H functions not implemented in AOGradeval at the moment!" << endl;
                    exit(1);
                }
            }
            }// contractions

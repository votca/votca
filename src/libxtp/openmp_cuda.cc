...
void OpenMP_CUDA::setOperators(const std::vector<Eigen::MatrixXd>& tensor,
  ...
  if (gpu_available) {
    ...
    B = std::make_unique<CudaMatrix>(operator, stream);
    C = std::make_unique<CudaMatrix>(head.rows(), matrix.cols(), stream);
  }
#else
void OpenMP_CUDA::setOperators(const std::vector<Eigen::MatrixXd>&,
  ...
  void OpenMP_CUDA::setOperators(const Eigen::MatrixXd& leftoperator,
    ...
    if (gpu_available) {
      ...
      B = std::make_unique<CudaMatrix>(leftoperator.cols(),
                                       rightoperator.rows(), stream);
      C = std::make_unique<CudaMatrix>(leftoperator.rows(),
                                       rightoperator.rows(), stream);
      D = std::make_unique<CudaMatrix>(rightoperator, stream);
      E = std::make_unique<CudaMatrix>(leftoperator.rows(),

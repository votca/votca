...
void StateSaverSQLite::Open(ctp::Topology &qmtop, const string &file,
                            bool lock) {
  _sqlfile = file;
...
void StateSaverSQLite::WriteFrame() {
  ...
  cout << "MD+QM topology ID " << _qmtop->getDatabaseId()
       << " (step = " << _qmtop->getStep() << ", time = " << _qmtop->getTime()
       << ") to " << _sqlfile << endl;
  cout << "... ";
...
void StateSaverSQLite::WriteSuperExchange(bool update) {
  if (!_qmtop->NBList().getSuperExchangeTypes().size()) {
    return;
  }
  ...
  for (seit = _qmtop->NBList().getSuperExchangeTypes().begin();
       seit != _qmtop->NBList().getSuperExchangeTypes().end(); seit++) {
...
void StateSaverSQLite::ReadFrame() {
  ...
  cout << "Import MD+QM Topology ID " << topId << " (i.e. frame "
       << _current_frame << ")"
       << " from " << _sqlfile << endl;
  cout << "...";

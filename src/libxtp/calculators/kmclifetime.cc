...
void KMCLifetime::ReadLifetimeFile(std::string filename) {
  ...
  for (list<tools::Property*>::iterator it = jobProps.begin();
       it != jobProps.end(); ++it) {
...
void KMCLifetime::RunVSSM(ctp::Topology* top) {
  ...
  while (insertioncount < _insertions) {
    ...
    if (_do_carrierenergy) {
      ...
      if (_carriers[0]->id > carrieridold) {
        ...
        avgenergy = _alpha * _carriers[0]->getCurrentEnergy() +
                    (1 - _alpha) * avgenergy;
        print = true;
      }
      if (print) {
        energyfile << simtime << "\t" << step << "\t" << _carriers[0]->id
                   << "\t" << avgenergy << endl;
      }
    }
    ...
    while (secondlevel) {
      ...
      while (true) {
        ...
        if (event->decayevent) {
          ...
          difflength += tools::elementwiseproduct(
              affectedcarrier->dr_travelled, affectedcarrier->dr_travelled);
          traj << simtime << "\t" << insertioncount << "\t"
               ...
               << affectedcarrier->dr_travelled.getX() << "\t"
               << affectedcarrier->dr_travelled.getY() << "\t"
               << affectedcarrier->dr_travelled.getZ() << endl;
  ...
  cout << "Average diffusionlength\t d=sqrt(<(r_x-r_o)^2>)\t"
       << sqrt(abs(difflength) / insertioncount) << " nm" << endl;
  cout << endl;

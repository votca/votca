...
void JobWriter::mps_monomer(Topology& top) {
  ...
  std::string str_states =
      _options->ifExistsReturnElseReturnDefault<std::string>(
  ...
  for (Segment& seg1 : top.Segments()) {
    ...
    for (auto& pattern : seg_patterns) {
      if (tools::wildcmp(pattern.c_str(), name1.c_str())) {
        do_continue = false;
        break;
      }
    }
  ...
  for (QMPair* pair : nblist) {
    ...
    for (auto& state1 : state1_state2) {
      for (auto& state2 : state1.second) {
        ...
        std::string tag =
            (format("%1$d%2$s:%3$d%4$s") % id1 % s1 % id2 % s2).str();

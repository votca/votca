...
void JobWriter::mps_monomer(ctp::Topology* top) {
  ...
  std::string str_states = _options->ifExistsReturnElseReturnDefault<string>(
      "options." + Identify() + ".states", "n e h");
  std::string seg_pattern = _options->ifExistsReturnElseReturnDefault<string>(
      "options." + Identify() + ".pattern", "*");
  ...
  cout << endl;
  for (ctp::Segment* seg1 : top->Segments()) {
    int id1 = seg1->getId();
    ...
    for (QMState& state : states) {
      int id = ++jobCount;
      std::string tag =
          (format("%1$d:%3$s:%2$s") % id1 % state.ToString() % name1).str();
      std::string input = (format("%1$d:%2$s:MP_FILES/%2$s_%3$s.mps") % id1 %
                           name1 % state.ToString())
                              .str();
...
void JobWriter::mps_dimer(ctp::Topology* top) {
...
void JobWriter::mps_background(ctp::Topology* top) {
  ...
  for (ctp::Segment* seg : top->Segments()) {
    ofs << (format("%1$4d %2$15s %3$-30s %4$-30s %5$-30s\n") % seg->getId() %
            seg->getName() % ("MP_FILES/" + seg->getName() + "_n.mps") %

...
{   // std::complex result of Gamma(z) with complex z
    ...
    gsl_sf_lngamma_complex_e(real(argument),imag(argument), &result_logradius,
&result_phi); double radius = result_logradius.val; radius = exp(radius); double
phi = result_phi.val; result  = polar(radius,phi);
    // std::cout << "Complex Gamma functions not supported by MKL " <<
std::endl;
    // exit(1);
    return result;
} */
...
void Rates::Initialize(tools::Property *options) {
  ...
  if (_rateType != "marcus" && _rateType != "jortner" &&
      _rateType != "weissdorsey" && _rateType != "sven") {
    std::cout << std::endl
  ...
  if (_rateType == "jortner") {
    ...
    if (options->exists(key + ".nmaxvib")) {
      ...
      _nMaxVib = 20;
      std::cout << std::endl
                << "... ... WARNING: No cut-off number for QM vibrations "
    ...
    if (options->exists(key + ".omegavib")) {
      ...
      _omegaVib = 0.2;
      std::cout << std::endl
                << "... ... WARNING: No QM vibration frequency provided, "
                   "using default 0.2eV.";
    }
  }
  ...
  if (_rateType == "weissdorsey") {
    ...
    if (options->exists(key + ".kondo")) {
      ...
      std::cout
          << std::endl
          << "... ... WARNING: No Kondo parameter provided. Using default 4.0.";
    }
  }
...
void Rates::ParseEnergiesXML(ctp::Topology *top, tools::Property *opt) {
  ...
  for (molit = mols.begin(); molit != mols.end(); ++molit) {
    ...
    for (segit = segs.begin(); segit != segs.end(); ++segit) {
      ...
      if ((*segit)->exists("U_cC_nN_e") && (*segit)->exists("U_nC_nN_e") &&
      ...
      if ((*segit)->exists("U_cC_nN_h") && (*segit)->exists("U_nC_nN_h") &&
...
void Rates::EvaluatePair(ctp::Topology *top, ctp::QMPair *qmpair) {
  ...
  std::cout << "\r... ... Evaluating pair " << qmpair->getId() + 1 << ". "
            << std::flush;
  ...
  //    }
  //    catch (out_of_range) {
  //        std::cout << std::endl << "... ... WARNING: No energy information
  //        for pair ["
  //                     << segName1 << ", " << segName2 << "]. "
  //                     << "Skipping... " << std::endl;
  //
...
void Rates::CalculateRate(ctp::Topology *top, ctp::QMPair *qmpair, int state) {
  ...
  if (state < 2) {
    ...
    dG_Site = seg2->getU_cC_nN(state)  // 1->2 == - 2->1
              + seg2->getEMpoles(state) - seg1->getU_cC_nN(state) -
    ...
    dG_Site = seg2->getU_xX_nN(state)  // 1->2 == - 2->1
              + seg2->getEMpoles(state) - seg1->getU_xX_nN(state) -
              seg1->getEMpoles(state);
  }
  ...
  */
  ...
  if (_rateType == "jortner") {
    ...
    for (nvib = 0; nvib <= _nMaxVib; nvib++) {
      ...
      rate12 += 1 / hbar_eV * sqrt(M_PI / (lOut * _kT)) * J2 *
                exp(-huang_rhys12) * pow(huang_rhys12, nvib) / Factorial(nvib) *
                exp(-pow((dG + nvib * _omegaVib + lOut), 2) / (4 * _kT * lOut));

      // Hopping from Seg2 -> Seg1
      rate21 +=
          1 / hbar_eV * sqrt(M_PI / (lOut * _kT)) * J2 * exp(-huang_rhys21) *
          pow(huang_rhys21, nvib) / Factorial(nvib) *
          exp(-pow((-dG + nvib * _omegaVib + lOut), 2) / (4 * _kT * lOut));
    }
  }
  ...
  // ++++++++++++ //
    ...
    /* std::cout << std::endl;
    std::cout << "  CGAMMA via GSL: " << gsl_sf_gamma(2*_kondo) << " native: "
    << ccgamma(2*_kondo,0).real() << std::endl; std::cout << " LCGAMMA via GSL:
    " << cgamma(_kondo+M_I*(+dG/2/M_PI/_kT)) << " native: " <<
    ccgamma(_kondo+M_I*(+dG/2/M_PI/_kT),0) << std::endl;
   */
    ...
    double e1 =
        seg2->getU_cC_nN(state) + seg2->getEMpoles(state);  // 1->2 == - 2->1
    double e2 = seg1->getU_cC_nN(state) + seg1->getEMpoles(state);

    //
    rate_symm12 = J2 / hbar_eV * sqrt(M_PI / (reorg12 * _kT)) *
                  exp(-(dG * dG + reorg12 * reorg12 - 2. * dG * reorg12) /
                      (4 * _kT * reorg12));

    rate_symm21 = J2 / hbar_eV * sqrt(M_PI / (reorg21 * _kT)) *
                  exp(-(dG * dG + reorg21 * reorg21 + 2. * dG * reorg21) /
                      (4 * _kT * reorg21));

    double _rate_symm12 = rate12 * exp(-e1 / _kT);
    double _rate_symm21 = rate21 * exp(-e2 / _kT);

    std::cout << " " << qmpair->Seg1()->getId() << " "
              << qmpair->Seg2()->getId() << rate_symm12 << " "
              << " " << rate_symm21 << " " << _rate_symm12 << " "
              << _rate_symm21 << std::endl;
  }

...
bool Profile::EvaluateFrame(ctp::Topology *top) {
  ...
  for (sit = top->Segments().begin(); sit < top->Segments().end(); ++sit) {
    ...
    if (seg->getId() < _firstSegId) {
      continue;
    }
    if (seg->getId() == _lastSegId + 1) {
      break;
    }

    if (!set_seg.count(seg->getName())) {
      set_seg[seg->getName()] = true;
    }
  ...
  for (setit = set_seg.begin(); setit != set_seg.end(); ++setit) {
    ...
    for (zit = com_zs.begin(), sit = com_seg.begin(); zit < com_zs.end();
         ++zit, ++sit) {
    ...
    for (int bin = 0; bin < BIN; ++bin) {
        ...
        for (unsigned int entry = 0; entry < binned_ea[bin].size(); ++entry) {
            //double avg_ea = binned_avg_ea[bin];
            //double avg_ec = binned_avg_ec[bin];
            std_ea += (binned_ea[bin][entry] - avg_ea)*(binned_ea[bin][entry] -
avg_ea); std_ec += (binned_ec[bin][entry] - avg_ec)*(binned_ec[bin][entry] -
avg_ec);
        }
...
*/
  ...
  for (int bin = 0; bin < BIN; ++bin) {
    ...
    if (_density_type == "atoms") {
      ...
      cout << endl
           << "ERROR: Invalid density type " << _density_type

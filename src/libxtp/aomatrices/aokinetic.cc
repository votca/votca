 ...
 int i_less_z[] = {  0,
       ...
       // cout << "row shell is " << shell_row->getSize() << " -fold contracted!" << endl;
	    ...
	    if ( exparg > 30.0 ) { continue; }
            ...
            const double PmA0 = rzeta*( decay_row * pos_row.getX() + decay_col * _pos_col.getX() ) - pos_row.getX();
            const double PmA1 = rzeta*( decay_row * pos_row.getY() + decay_col * _pos_col.getY() ) - pos_row.getY();
            const double PmA2 = rzeta*( decay_row * pos_row.getZ() + decay_col * _pos_col.getZ() ) - pos_row.getZ();

            const double PmB0 = rzeta*( decay_row * pos_row.getX() + decay_col * _pos_col.getX() ) - _pos_col.getX();
            const double PmB1 = rzeta*( decay_row * pos_row.getY() + decay_col * _pos_col.getY() ) - _pos_col.getY();
            const double PmB2 = rzeta*( decay_row * pos_row.getZ() + decay_col * _pos_col.getZ() ) - _pos_col.getZ();
...
//Integrals     f - s
if (lmax_row > 2) {
  kin(Cart::xxx,0) = xi2*ol(Cart::xxx,0) + PmA0*kin(Cart::xx,0) + 2*(fak*kin(Cart::x,0)-fak_a*ol(Cart::x,0));
  ...
  kin(Cart::yyy,0) = xi2*ol(Cart::yyy,0) + PmA1*kin(Cart::yy,0) + 2*(fak*kin(Cart::y,0)-fak_a*ol(Cart::y,0));
  kin(Cart::yyz,0) = xi2*ol(Cart::yyz,0) + PmA2*kin(Cart::yy,0);
  kin(Cart::yzz,0) = xi2*ol(Cart::yzz,0) + PmA1*kin(Cart::zz,0);
  kin(Cart::zzz,0) = xi2*ol(Cart::zzz,0) + PmA2*kin(Cart::zz,0) + 2*(fak*kin(Cart::z,0)-fak_a*ol(Cart::z,0));
}
//------------------------------------------------------
...
if (lmax_col > 0) {
  ...
  //Integrals     p - p     d - p     f - p     g - p
  for (int i = 1; i < n_orbitals[lmax_row]; i++) {
    kin(i,Cart::x) = xi2*ol(i,Cart::x) + PmB0*kin(i,0) + nx[i]*fak*kin(i_less_x[i],0);
    kin(i,Cart::y) = xi2*ol(i,Cart::y) + PmB1*kin(i,0) + ny[i]*fak*kin(i_less_y[i],0);
    kin(i,Cart::z) = xi2*ol(i,Cart::z) + PmB2*kin(i,0) + nz[i]*fak*kin(i_less_z[i],0);
  }
  //------------------------------------------------------
...
if (lmax_col > 1) {
  ...
  for (int i = 1; i < n_orbitals[lmax_row]; i++) {
    ...
    kin(i,Cart::xx) = xi2*ol(i,Cart::xx) + PmB0*kin(i,Cart::x) + nx[i]*fak*kin(i_less_x[i],Cart::x) + term;
    kin(i,Cart::xy) = xi2*ol(i,Cart::xy) + PmB0*kin(i,Cart::y) + nx[i]*fak*kin(i_less_x[i],Cart::y);
    kin(i,Cart::xz) = xi2*ol(i,Cart::xz) + PmB0*kin(i,Cart::z) + nx[i]*fak*kin(i_less_x[i],Cart::z);
    kin(i,Cart::yy) = xi2*ol(i,Cart::yy) + PmB1*kin(i,Cart::y) + ny[i]*fak*kin(i_less_y[i],Cart::y) + term;
    kin(i,Cart::yz) = xi2*ol(i,Cart::yz) + PmB1*kin(i,Cart::z) + ny[i]*fak*kin(i_less_y[i],Cart::z);
    kin(i,Cart::zz) = xi2*ol(i,Cart::zz) + PmB2*kin(i,Cart::z) + nz[i]*fak*kin(i_less_z[i],Cart::z) + term;
//    }
  }
  //------------------------------------------------------
...
if (lmax_col > 2) {
  ...
  //Integrals     s - f
  kin(0,Cart::xxx) = xi2*ol(0,Cart::xxx) + PmB0*kin(0,Cart::xx) + 2*(fak*kin(0,Cart::x)-fak_b*ol(0,Cart::x));
  ...
  kin(0,Cart::yyy) = xi2*ol(0,Cart::yyy) + PmB1*kin(0,Cart::yy) + 2*(fak*kin(0,Cart::y)-fak_b*ol(0,Cart::y));
  kin(0,Cart::yyz) = xi2*ol(0,Cart::yyz) + PmB2*kin(0,Cart::yy);
  kin(0,Cart::yzz) = xi2*ol(0,Cart::yzz) + PmB1*kin(0,Cart::zz);
  kin(0,Cart::zzz) = xi2*ol(0,Cart::zzz) + PmB2*kin(0,Cart::zz) + 2*(fak*kin(0,Cart::z)-fak_b*ol(0,Cart::z));
  //------------------------------------------------------
  ...
  for (int i = 1; i < n_orbitals[lmax_row]; i++) {
    ...
    kin(i,Cart::xxx) = xi2*ol(i,Cart::xxx) + PmB0*kin(i,Cart::xx) + nx[i]*fak*kin(i_less_x[i],Cart::xx) + term_x;
    kin(i,Cart::xxy) = xi2*ol(i,Cart::xxy) + PmB1*kin(i,Cart::xx) + ny[i]*fak*kin(i_less_y[i],Cart::xx);
    kin(i,Cart::xxz) = xi2*ol(i,Cart::xxz) + PmB2*kin(i,Cart::xx) + nz[i]*fak*kin(i_less_z[i],Cart::xx);
    kin(i,Cart::xyy) = xi2*ol(i,Cart::xyy) + PmB0*kin(i,Cart::yy) + nx[i]*fak*kin(i_less_x[i],Cart::yy);
    kin(i,Cart::xyz) = xi2*ol(i,Cart::xyz) + PmB0*kin(i,Cart::yz) + nx[i]*fak*kin(i_less_x[i],Cart::yz);
    kin(i,Cart::xzz) = xi2*ol(i,Cart::xzz) + PmB0*kin(i,Cart::zz) + nx[i]*fak*kin(i_less_x[i],Cart::zz);
    kin(i,Cart::yyy) = xi2*ol(i,Cart::yyy) + PmB1*kin(i,Cart::yy) + ny[i]*fak*kin(i_less_y[i],Cart::yy) + term_y;
    kin(i,Cart::yyz) = xi2*ol(i,Cart::yyz) + PmB2*kin(i,Cart::yy) + nz[i]*fak*kin(i_less_z[i],Cart::yy);
    kin(i,Cart::yzz) = xi2*ol(i,Cart::yzz) + PmB1*kin(i,Cart::zz) + ny[i]*fak*kin(i_less_y[i],Cart::zz);
    kin(i,Cart::zzz) = xi2*ol(i,Cart::zzz) + PmB2*kin(i,Cart::zz) + nz[i]*fak*kin(i_less_z[i],Cart::zz) + term_z;
  }
  //------------------------------------------------------
...
if (lmax_col > 3) {
  ...
  for (int i = 1; i < n_orbitals[lmax_row]; i++) {
    ...
    kin(i,Cart::xxxx) = xi2*ol(i,Cart::xxxx) + PmB0*kin(i,Cart::xxx) + nx[i]*fak*kin(i_less_x[i],Cart::xxx) + 3*term_xx;
    kin(i,Cart::xxxy) = xi2*ol(i,Cart::xxxy) + PmB1*kin(i,Cart::xxx) + ny[i]*fak*kin(i_less_y[i],Cart::xxx);
    kin(i,Cart::xxxz) = xi2*ol(i,Cart::xxxz) + PmB2*kin(i,Cart::xxx) + nz[i]*fak*kin(i_less_z[i],Cart::xxx);
    kin(i,Cart::xxyy) = xi2*ol(i,Cart::xxyy) + PmB0*kin(i,Cart::xyy) + nx[i]*fak*kin(i_less_x[i],Cart::xyy) + term_yy;
    kin(i,Cart::xxyz) = xi2*ol(i,Cart::xxyz) + PmB1*kin(i,Cart::xxz) + ny[i]*fak*kin(i_less_y[i],Cart::xxz);
    kin(i,Cart::xxzz) = xi2*ol(i,Cart::xxzz) + PmB0*kin(i,Cart::xzz) + nx[i]*fak*kin(i_less_x[i],Cart::xzz) + term_zz;
    kin(i,Cart::xyyy) = xi2*ol(i,Cart::xyyy) + PmB0*kin(i,Cart::yyy) + nx[i]*fak*kin(i_less_x[i],Cart::yyy);
    kin(i,Cart::xyyz) = xi2*ol(i,Cart::xyyz) + PmB0*kin(i,Cart::yyz) + nx[i]*fak*kin(i_less_x[i],Cart::yyz);
    kin(i,Cart::xyzz) = xi2*ol(i,Cart::xyzz) + PmB0*kin(i,Cart::yzz) + nx[i]*fak*kin(i_less_x[i],Cart::yzz);
    kin(i,Cart::xzzz) = xi2*ol(i,Cart::xzzz) + PmB0*kin(i,Cart::zzz) + nx[i]*fak*kin(i_less_x[i],Cart::zzz);
    kin(i,Cart::yyyy) = xi2*ol(i,Cart::yyyy) + PmB1*kin(i,Cart::yyy) + ny[i]*fak*kin(i_less_y[i],Cart::yyy) + 3*term_yy;
    kin(i,Cart::yyyz) = xi2*ol(i,Cart::yyyz) + PmB2*kin(i,Cart::yyy) + nz[i]*fak*kin(i_less_z[i],Cart::yyy);
    kin(i,Cart::yyzz) = xi2*ol(i,Cart::yyzz) + PmB1*kin(i,Cart::yzz) + ny[i]*fak*kin(i_less_y[i],Cart::yzz) + term_zz;
    kin(i,Cart::yzzz) = xi2*ol(i,Cart::yzzz) + PmB1*kin(i,Cart::zzz) + ny[i]*fak*kin(i_less_y[i],Cart::zzz);
    kin(i,Cart::zzzz) = xi2*ol(i,Cart::zzzz) + PmB2*kin(i,Cart::zzz) + nz[i]*fak*kin(i_less_z[i],Cart::zzz) + 3*term_zz;
  }
  //------------------------------------------------------

...
void AODipole_Potential::FillBlock(Eigen::Block<Eigen::MatrixXd>& matrix,
  ...
  tools::vec dipole =
      -(apolarsite->getU1() + apolarsite->getQ1()) * tools::conv::nm2bohr;
  ...
  for (AOShell::GaussianIterator itr = shell_row->begin();
    ...
    for (AOShell::GaussianIterator itc = shell_col->begin();
      ...
      // some helpers
      double PmA0 =
          fak2 * (decay_row * pos_row.getX() + decay_col * _pos_col.getX()) -
          pos_row.getX();
      double PmA1 =
          fak2 * (decay_row * pos_row.getY() + decay_col * _pos_col.getY()) -
          pos_row.getY();
      double PmA2 =
          fak2 * (decay_row * pos_row.getZ() + decay_col * _pos_col.getZ()) -
          pos_row.getZ();
      ...
      for (int i = 0; i < nrows; i++) {
        for (int j = 0; j < ncols; j++) {
          dip(i, j) = dipole.getX() * dip4[i][j][0][0] +
                      dipole.getY() * dip4[i][j][1][0] +
                      dipole.getZ() * dip4[i][j][2][0];
        }
      }

      Eigen::MatrixXd dip_sph =
          getTrafo(*itr).transpose() * dip * getTrafo(*itc);
      // save to matrix
...
void AODipole_Potential::Fillextpotential(
  ...
  for (unsigned int i = 0; i < sites.size(); i++) {
    for (ctp::APolarSite* site : *(sites[i])) {
      ...
      if (site->getRank() > 0 || site->IsPolarizable()) {
        if (tools::abs(site->getU1() + site->getQ1()) < 1e-12) {
          continue;
        }
        _aomatrix =
            Eigen::MatrixXd::Zero(aobasis.AOBasisSize(), aobasis.AOBasisSize());

...
void AOMomentum::FillBlock(std::vector<Eigen::Block<Eigen::MatrixXd> >& matrix,
  ...
  Eigen::MatrixXd ol =
      Eigen::MatrixXd::Zero(nrows_ol, ncols_ol);  //////////////
    ...
    for (AOShell::GaussianIterator itc = shell_col->begin();
      ...
      if (lmax_col > 1) {
        ...
        for (int i = 1; i < n_orbitals[lmax_row]; i++) {
          ...
          ol(i, Cart::xxy) =
              PmB1 * ol(i, Cart::xx) + ny_i * fak * ol(ily_i, Cart::xx);
          ol(i, Cart::xxz) =
              PmB2 * ol(i, Cart::xx) + nz_i * fak * ol(ilz_i, Cart::xx);
          ol(i, Cart::xyy) =
              PmB0 * ol(i, Cart::yy) + nx_i * fak * ol(ilx_i, Cart::yy);
          ol(i, Cart::xyz) =
              PmB0 * ol(i, Cart::yz) + nx_i * fak * ol(ilx_i, Cart::yz);
          ol(i, Cart::xzz) =
              PmB0 * ol(i, Cart::zz) + nx_i * fak * ol(ilx_i, Cart::zz);
          ol(i, Cart::yyy) = PmB1 * ol(i, Cart::yy) +
                             ny_i * fak * ol(ily_i, Cart::yy) + term_y;
          ol(i, Cart::yyz) =
              PmB2 * ol(i, Cart::yy) + nz_i * fak * ol(ilz_i, Cart::yy);
          ol(i, Cart::yzz) =
              PmB1 * ol(i, Cart::zz) + ny_i * fak * ol(ily_i, Cart::zz);
      ...
      if (lmax_col > 2) {
        ...
        for (int i = 1; i < n_orbitals[lmax_row]; i++) {
          ...
          ol(i, Cart::xxxy) =
              PmB1 * ol(i, Cart::xxx) + ny_i * fak * ol(ily_i, Cart::xxx);
          ol(i, Cart::xxxz) =
              PmB2 * ol(i, Cart::xxx) + nz_i * fak * ol(ilz_i, Cart::xxx);
          ol(i, Cart::xxyy) = PmB0 * ol(i, Cart::xyy) +
                              nx_i * fak * ol(ilx_i, Cart::xyy) + term_yy;
          ol(i, Cart::xxyz) =
              PmB1 * ol(i, Cart::xxz) + ny_i * fak * ol(ily_i, Cart::xxz);
          ol(i, Cart::xxzz) = PmB0 * ol(i, Cart::xzz) +
                              nx_i * fak * ol(ilx_i, Cart::xzz) + term_zz;
          ol(i, Cart::xyyy) =
              PmB0 * ol(i, Cart::yyy) + nx_i * fak * ol(ilx_i, Cart::yyy);
          ol(i, Cart::xyyz) =
              PmB0 * ol(i, Cart::yyz) + nx_i * fak * ol(ilx_i, Cart::yyz);
          ol(i, Cart::xyzz) =
              PmB0 * ol(i, Cart::yzz) + nx_i * fak * ol(ilx_i, Cart::yzz);
          ol(i, Cart::xzzz) =
              PmB0 * ol(i, Cart::zzz) + nx_i * fak * ol(ilx_i, Cart::zzz);
          ol(i, Cart::yyyy) = PmB1 * ol(i, Cart::yyy) +
                              ny_i * fak * ol(ily_i, Cart::yyy) + 3 * term_yy;
          ol(i, Cart::yyyz) =
              PmB2 * ol(i, Cart::yyy) + nz_i * fak * ol(ilz_i, Cart::yyy);
          ol(i, Cart::yyzz) = PmB1 * ol(i, Cart::yzz) +
                              ny_i * fak * ol(ily_i, Cart::yzz) + term_zz;
          ol(i, Cart::yzzz) =
              PmB1 * ol(i, Cart::zzz) + ny_i * fak * ol(ily_i, Cart::zzz);
      ...
      if (lmax_col > 3) {
        ...
        for (int i = 1; i < n_orbitals[lmax_row]; i++) {
          ...
          ol(i, Cart::xxxxx) = PmB0 * ol(i, Cart::xxxx) +
                               nx_i * fak * ol(ilx_i, Cart::xxxx) +
                               4 * term_xxx;
          ol(i, Cart::xxxxy) =
              PmB1 * ol(i, Cart::xxxx) + ny_i * fak * ol(ily_i, Cart::xxxx);
          ol(i, Cart::xxxxz) =
              PmB2 * ol(i, Cart::xxxx) + nz_i * fak * ol(ilz_i, Cart::xxxx);
          ol(i, Cart::xxxyy) = PmB1 * ol(i, Cart::xxxy) +
                               ny_i * fak * ol(ily_i, Cart::xxxy) + term_xxx;
          ol(i, Cart::xxxyz) =
              PmB1 * ol(i, Cart::xxxz) + ny_i * fak * ol(ily_i, Cart::xxxz);
          ol(i, Cart::xxxzz) = PmB2 * ol(i, Cart::xxxz) +
                               nz_i * fak * ol(ilz_i, Cart::xxxz) + term_xxx;
          ol(i, Cart::xxyyy) = PmB0 * ol(i, Cart::xyyy) +
                               nx_i * fak * ol(ilx_i, Cart::xyyy) + term_yyy;
          ol(i, Cart::xxyyz) =
              PmB2 * ol(i, Cart::xxyy) + nz_i * fak * ol(ilz_i, Cart::xxyy);
          ol(i, Cart::xxyzz) =
              PmB1 * ol(i, Cart::xxzz) + ny_i * fak * ol(ily_i, Cart::xxzz);
          ol(i, Cart::xxzzz) = PmB0 * ol(i, Cart::xzzz) +
                               nx_i * fak * ol(ilx_i, Cart::xzzz) + term_zzz;
          ol(i, Cart::xyyyy) =
              PmB0 * ol(i, Cart::yyyy) + nx_i * fak * ol(ilx_i, Cart::yyyy);
          ol(i, Cart::xyyyz) =
              PmB0 * ol(i, Cart::yyyz) + nx_i * fak * ol(ilx_i, Cart::yyyz);
          ol(i, Cart::xyyzz) =
              PmB0 * ol(i, Cart::yyzz) + nx_i * fak * ol(ilx_i, Cart::yyzz);
          ol(i, Cart::xyzzz) =
              PmB0 * ol(i, Cart::yzzz) + nx_i * fak * ol(ilx_i, Cart::yzzz);
          ol(i, Cart::xzzzz) =
              PmB0 * ol(i, Cart::zzzz) + nx_i * fak * ol(ilx_i, Cart::zzzz);
          ol(i, Cart::yyyyy) = PmB1 * ol(i, Cart::yyyy) +
                               ny_i * fak * ol(ily_i, Cart::yyyy) +
                               4 * term_yyy;
          ol(i, Cart::yyyyz) =
              PmB2 * ol(i, Cart::yyyy) + nz_i * fak * ol(ilz_i, Cart::yyyy);
          ol(i, Cart::yyyzz) = PmB2 * ol(i, Cart::yyyz) +
                               nz_i * fak * ol(ilz_i, Cart::yyyz) + term_yyy;
          ol(i, Cart::yyzzz) = PmB1 * ol(i, Cart::yzzz) +
                               ny_i * fak * ol(ily_i, Cart::yzzz) + term_zzz;
          ol(i, Cart::yzzzz) =
              PmB1 * ol(i, Cart::zzzz) + ny_i * fak * ol(ily_i, Cart::zzzz);
          ol(i, Cart::zzzzz) = PmB2 * ol(i, Cart::zzzz) +

        ...
        void AOPlanewave::FillBlock(Eigen::Block<Eigen::MatrixXcd>& matrix, const AOShell* shell_row, const AOShell* shell_col) {
            ...
            for (AOShell::GaussianIterator itr = shell_row->begin(); itr != shell_row->end(); ++itr) {
                ...
                for (AOShell::GaussianIterator itc = shell_col->begin(); itc != shell_col->end(); ++itc) {
                    ...
                    const COMPLEX PmA0(fak2 * (decay_row * pos_row.getX() + decay_col * pos_col.getX()) - pos_row.getX(), fak * _k.getX());
                    const COMPLEX PmA1(fak2 * (decay_row * pos_row.getY() + decay_col * pos_col.getY()) - pos_row.getY(), fak * _k.getY());
                    const COMPLEX PmA2(fak2 * (decay_row * pos_row.getZ() + decay_col * pos_col.getZ()) - pos_row.getZ(), fak * _k.getZ());

                    const COMPLEX PmB0(fak2 * (decay_row * pos_row.getX() + decay_col * pos_col.getX()) - pos_col.getX(), fak * _k.getX());
                    const COMPLEX PmB1(fak2 * (decay_row * pos_row.getY() + decay_col * pos_col.getY()) - pos_col.getY(), fak * _k.getY());
                    const COMPLEX PmB2(fak2 * (decay_row * pos_row.getZ() + decay_col * pos_col.getZ()) - pos_col.getZ(), fak * _k.getZ());
                    ...
                    COMPLEX ssol(pow(4.0 * decay_row*decay_col, 0.75) * pow(fak2, 1.5) * exp(-exparg), 0.0); // s-s element

                    // calculate s-W-s matrix element
                    double kdotr_row = (_k * pos_row);
                    double kdotr_col = (_k * pos_col);
                    COMPLEX kexparg(fak2 * (-0.25)*(kmodulus), fak2 * (decay_row)*(kdotr_row) + fak2 * (decay_col)*(kdotr_col));
                    ...
                    if (lmax_col > 0) {
                        ...
                        for (int i = 1; i < n_orbitals[lmax_row]; i++) {
                            ...
                            olk(i, Cart::x) = PmB0 * olk(i, 0) + double(nx[i]) * cfak * olk(i_less_x[i], 0);
                            //COMPLEX cny = (ny[i] * fak, 0.0);
                            olk(i, Cart::y) = PmB1 * olk(i, 0) + double(ny[i]) * cfak * olk(i_less_y[i], 0);
                            //COMPLEX cnz = (nz[i] * fak, 0.0);
                            olk(i, Cart::z) = PmB2 * olk(i, 0) + double(nz[i]) * cfak * olk(i_less_z[i], 0);
                        }
                        //------------------------------------------------------
                    ...
                    if (lmax_col > 1) {
                        ...
                        for (int i = 1; i < n_orbitals[lmax_row]; i++) {
                            ...
                            olk(i, Cart::xx) = PmB0 * olk(i, Cart::x) + double(nx[i]) * cfak * olk(i_less_x[i], Cart::x) + term;
                            olk(i, Cart::xy) = PmB0 * olk(i, Cart::y) + double(nx[i]) * cfak * olk(i_less_x[i], Cart::y);
                            olk(i, Cart::xz) = PmB0 * olk(i, Cart::z) + double(nx[i]) * cfak * olk(i_less_x[i], Cart::z);
                            olk(i, Cart::yy) = PmB1 * olk(i, Cart::y) + double(ny[i]) * cfak * olk(i_less_y[i], Cart::y) + term;
                            olk(i, Cart::yz) = PmB1 * olk(i, Cart::z) + double(ny[i]) * cfak * olk(i_less_y[i], Cart::z);
                            olk(i, Cart::zz) = PmB2 * olk(i, Cart::z) + double(nz[i]) * cfak * olk(i_less_z[i], Cart::z) + term;
                        }
                        //------------------------------------------------------
                    ...
                    if (lmax_col > 2) {
                        ...
                        //Integrals     s - W - f
                        olk(0, Cart::xxx) = PmB0 * olk(0, Cart::xx) + 2.0 * cfak * olk(0, Cart::x);
                        ...
                        olk(0, Cart::yyy) = PmB1 * olk(0, Cart::yy) + 2.0 * cfak * olk(0, Cart::y);
                        olk(0, Cart::yyz) = PmB2 * olk(0, Cart::yy);
                        olk(0, Cart::yzz) = PmB1 * olk(0, Cart::zz);
                        olk(0, Cart::zzz) = PmB2 * olk(0, Cart::zz) + 2.0 * cfak * olk(0, Cart::z);
                        //------------------------------------------------------
                        ...
                        for (int i = 1; i < n_orbitals[lmax_row]; i++) {
                            ...
                            olk(i, Cart::xxx) = PmB0 * olk(i, Cart::xx) + double(nx[i]) * cfak * olk(i_less_x[i], Cart::xx) + term_x;
                            olk(i, Cart::xxy) = PmB1 * olk(i, Cart::xx) + double(ny[i]) * cfak * olk(i_less_y[i], Cart::xx);
                            olk(i, Cart::xxz) = PmB2 * olk(i, Cart::xx) + double(nz[i]) * cfak * olk(i_less_z[i], Cart::xx);
                            olk(i, Cart::xyy) = PmB0 * olk(i, Cart::yy) + double(nx[i]) * cfak * olk(i_less_x[i], Cart::yy);
                            olk(i, Cart::xyz) = PmB0 * olk(i, Cart::yz) + double(nx[i]) * cfak * olk(i_less_x[i], Cart::yz);
                            olk(i, Cart::xzz) = PmB0 * olk(i, Cart::zz) + double(nx[i]) * cfak * olk(i_less_x[i], Cart::zz);
                            olk(i, Cart::yyy) = PmB1 * olk(i, Cart::yy) + double(ny[i]) * cfak * olk(i_less_y[i], Cart::yy) + term_y;
                            olk(i, Cart::yyz) = PmB2 * olk(i, Cart::yy) + double(nz[i]) * cfak * olk(i_less_z[i], Cart::yy);
                            olk(i, Cart::yzz) = PmB1 * olk(i, Cart::zz) + double(ny[i]) * cfak * olk(i_less_y[i], Cart::zz);
                            olk(i, Cart::zzz) = PmB2 * olk(i, Cart::zz) + double(nz[i]) * cfak * olk(i_less_z[i], Cart::zz) + term_z;
                        }
                        //------------------------------------------------------
                    ...
                    if (lmax_col > 3) {
                        ...
                        for (int i = 1; i < n_orbitals[lmax_row]; i++) {
                            ...
                            olk(i, Cart::xxxx) = PmB0 * olk(i, Cart::xxx) + double(nx[i]) * cfak * olk(i_less_x[i], Cart::xxx) + 3.0 * term_xx;
                            olk(i, Cart::xxxy) = PmB1 * olk(i, Cart::xxx) + double(ny[i]) * cfak * olk(i_less_y[i], Cart::xxx);
                            olk(i, Cart::xxxz) = PmB2 * olk(i, Cart::xxx) + double(nz[i]) * cfak * olk(i_less_z[i], Cart::xxx);
                            olk(i, Cart::xxyy) = PmB0 * olk(i, Cart::xyy) + double(nx[i]) * cfak * olk(i_less_x[i], Cart::xyy) + term_yy;
                            olk(i, Cart::xxyz) = PmB1 * olk(i, Cart::xxz) + double(ny[i]) * cfak * olk(i_less_y[i], Cart::xxz);
                            olk(i, Cart::xxzz) = PmB0 * olk(i, Cart::xzz) + double(nx[i]) * cfak * olk(i_less_x[i], Cart::xzz) + term_zz;
                            olk(i, Cart::xyyy) = PmB0 * olk(i, Cart::yyy) + double(nx[i]) * cfak * olk(i_less_x[i], Cart::yyy);
                            olk(i, Cart::xyyz) = PmB0 * olk(i, Cart::yyz) + double(nx[i]) * cfak * olk(i_less_x[i], Cart::yyz);
                            olk(i, Cart::xyzz) = PmB0 * olk(i, Cart::yzz) + double(nx[i]) * cfak * olk(i_less_x[i], Cart::yzz);
                            olk(i, Cart::xzzz) = PmB0 * olk(i, Cart::zzz) + double(nx[i]) * cfak * olk(i_less_x[i], Cart::zzz);
                            olk(i, Cart::yyyy) = PmB1 * olk(i, Cart::yyy) + double(ny[i]) * cfak * olk(i_less_y[i], Cart::yyy) + 3.0 * term_yy;
                            olk(i, Cart::yyyz) = PmB2 * olk(i, Cart::yyy) + double(nz[i]) * cfak * olk(i_less_z[i], Cart::yyy);
                            olk(i, Cart::yyzz) = PmB1 * olk(i, Cart::yzz) + double(ny[i]) * cfak * olk(i_less_y[i], Cart::yzz) + term_zz;
                            olk(i, Cart::yzzz) = PmB1 * olk(i, Cart::zzz) + double(ny[i]) * cfak * olk(i_less_y[i], Cart::zzz);
                            olk(i, Cart::zzzz) = PmB2 * olk(i, Cart::zzz) + double(nz[i]) * cfak * olk(i_less_z[i], Cart::zzz) + 3.0 * term_zz;
                        }
                        //------------------------------------------------------
                    ...
                    if (lmax_col > 4) {
                        ...
                        for (int i = 1; i < n_orbitals[lmax_row]; i++) {
                            ...
                            olk(i, Cart::xxxxx) = PmB0 * olk(i, Cart::xxxx) + double(nx[i]) * cfak * olk(i_less_x[i], Cart::xxxx) + 4.0 * term_xxx;
                            olk(i, Cart::xxxxy) = PmB1 * olk(i, Cart::xxxx) + double(ny[i]) * cfak * olk(i_less_y[i], Cart::xxxx);
                            olk(i, Cart::xxxxz) = PmB2 * olk(i, Cart::xxxx) + double(nz[i]) * cfak * olk(i_less_z[i], Cart::xxxx);
                            olk(i, Cart::xxxyy) = PmB1 * olk(i, Cart::xxxy) + double(ny[i]) * cfak * olk(i_less_y[i], Cart::xxxy) + term_xxx;
                            olk(i, Cart::xxxyz) = PmB1 * olk(i, Cart::xxxz) + double(ny[i]) * cfak * olk(i_less_y[i], Cart::xxxz);
                            olk(i, Cart::xxxzz) = PmB2 * olk(i, Cart::xxxz) + double(nz[i]) * cfak * olk(i_less_z[i], Cart::xxxz) + term_xxx;
                            olk(i, Cart::xxyyy) = PmB0 * olk(i, Cart::xyyy) + double(nx[i]) * cfak * olk(i_less_x[i], Cart::xyyy) + term_yyy;
                            olk(i, Cart::xxyyz) = PmB2 * olk(i, Cart::xxyy) + double(nz[i]) * cfak * olk(i_less_z[i], Cart::xxyy);
                            olk(i, Cart::xxyzz) = PmB1 * olk(i, Cart::xxzz) + double(ny[i]) * cfak * olk(i_less_y[i], Cart::xxzz);
                            olk(i, Cart::xxzzz) = PmB0 * olk(i, Cart::xzzz) + double(nx[i]) * cfak * olk(i_less_x[i], Cart::xzzz) + term_zzz;
                            olk(i, Cart::xyyyy) = PmB0 * olk(i, Cart::yyyy) + double(nx[i]) * cfak * olk(i_less_x[i], Cart::yyyy);
                            olk(i, Cart::xyyyz) = PmB0 * olk(i, Cart::yyyz) + double(nx[i]) * cfak * olk(i_less_x[i], Cart::yyyz);
                            olk(i, Cart::xyyzz) = PmB0 * olk(i, Cart::yyzz) + double(nx[i]) * cfak * olk(i_less_x[i], Cart::yyzz);
                            olk(i, Cart::xyzzz) = PmB0 * olk(i, Cart::yzzz) + double(nx[i]) * cfak * olk(i_less_x[i], Cart::yzzz);
                            olk(i, Cart::xzzzz) = PmB0 * olk(i, Cart::zzzz) + double(nx[i]) * cfak * olk(i_less_x[i], Cart::zzzz);
                            olk(i, Cart::yyyyy) = PmB1 * olk(i, Cart::yyyy) + double(ny[i]) * cfak * olk(i_less_y[i], Cart::yyyy) + 4.0 * term_yyy;
                            olk(i, Cart::yyyyz) = PmB2 * olk(i, Cart::yyyy) + double(nz[i]) * cfak * olk(i_less_z[i], Cart::yyyy);
                            olk(i, Cart::yyyzz) = PmB2 * olk(i, Cart::yyyz) + double(nz[i]) * cfak * olk(i_less_z[i], Cart::yyyz) + term_yyy;
                            olk(i, Cart::yyzzz) = PmB1 * olk(i, Cart::yzzz) + double(ny[i]) * cfak * olk(i_less_y[i], Cart::yzzz) + term_zzz;
                            olk(i, Cart::yzzzz) = PmB1 * olk(i, Cart::zzzz) + double(ny[i]) * cfak * olk(i_less_y[i], Cart::zzzz);
                            olk(i, Cart::zzzzz) = PmB2 * olk(i, Cart::zzzz) + double(nz[i]) * cfak * olk(i_less_z[i], Cart::zzzz) + 4.0 * term_zzz;
                        }
                        //------------------------------------------------------
                    ...
                    if (lmax_col > 5) {
                        ...
                        for (int i = 1; i < n_orbitals[lmax_row]; i++) {
                            ...
                            olk(i, Cart::xxxxxx) = PmB0 * olk(i, Cart::xxxxx) + double(nx[i]) * cfak * olk(i_less_x[i], Cart::xxxxx) + 5.0 * term_xxxx;
                            olk(i, Cart::xxxxxy) = PmB1 * olk(i, Cart::xxxxx) + double(ny[i]) * cfak * olk(i_less_y[i], Cart::xxxxx);
                            olk(i, Cart::xxxxxz) = PmB2 * olk(i, Cart::xxxxx) + double(nz[i]) * cfak * olk(i_less_z[i], Cart::xxxxx);
                            olk(i, Cart::xxxxyy) = PmB1 * olk(i, Cart::xxxxy) + double(ny[i]) * cfak * olk(i_less_y[i], Cart::xxxxy) + term_xxxx;
                            olk(i, Cart::xxxxyz) = PmB1 * olk(i, Cart::xxxxz) + double(ny[i]) * cfak * olk(i_less_y[i], Cart::xxxxz);
                            olk(i, Cart::xxxxzz) = PmB2 * olk(i, Cart::xxxxz) + double(nz[i]) * cfak * olk(i_less_z[i], Cart::xxxxz) + term_xxxx;
                            olk(i, Cart::xxxyyy) = PmB0 * olk(i, Cart::xxyyy) + double(nx[i]) * cfak * olk(i_less_x[i], Cart::xxyyy) + 2.0 * term_xyyy;
                            olk(i, Cart::xxxyyz) = PmB2 * olk(i, Cart::xxxyy) + double(nz[i]) * cfak * olk(i_less_z[i], Cart::xxxyy);
                            olk(i, Cart::xxxyzz) = PmB1 * olk(i, Cart::xxxzz) + double(ny[i]) * cfak * olk(i_less_y[i], Cart::xxxzz);
                            olk(i, Cart::xxxzzz) = PmB0 * olk(i, Cart::xxzzz) + double(nx[i]) * cfak * olk(i_less_x[i], Cart::xxzzz) + 2.0 * term_xzzz;
                            olk(i, Cart::xxyyyy) = PmB0 * olk(i, Cart::xyyyy) + double(nx[i]) * cfak * olk(i_less_x[i], Cart::xyyyy) + term_yyyy;
                            olk(i, Cart::xxyyyz) = PmB2 * olk(i, Cart::xxyyy) + double(nz[i]) * cfak * olk(i_less_z[i], Cart::xxyyy);
                            olk(i, Cart::xxyyzz) = PmB0 * olk(i, Cart::xyyzz) + double(nx[i]) * cfak * olk(i_less_x[i], Cart::xyyzz) + term_yyzz;
                            olk(i, Cart::xxyzzz) = PmB1 * olk(i, Cart::xxzzz) + double(ny[i]) * cfak * olk(i_less_y[i], Cart::xxzzz);
                            olk(i, Cart::xxzzzz) = PmB0 * olk(i, Cart::xzzzz) + double(nx[i]) * cfak * olk(i_less_x[i], Cart::xzzzz) + term_zzzz;
                            olk(i, Cart::xyyyyy) = PmB0 * olk(i, Cart::yyyyy) + double(nx[i]) * cfak * olk(i_less_x[i], Cart::yyyyy);
                            olk(i, Cart::xyyyyz) = PmB0 * olk(i, Cart::yyyyz) + double(nx[i]) * cfak * olk(i_less_x[i], Cart::yyyyz);
                            olk(i, Cart::xyyyzz) = PmB0 * olk(i, Cart::yyyzz) + double(nx[i]) * cfak * olk(i_less_x[i], Cart::yyyzz);
                            olk(i, Cart::xyyzzz) = PmB0 * olk(i, Cart::yyzzz) + double(nx[i]) * cfak * olk(i_less_x[i], Cart::yyzzz);
                            olk(i, Cart::xyzzzz) = PmB0 * olk(i, Cart::yzzzz) + double(nx[i]) * cfak * olk(i_less_x[i], Cart::yzzzz);
                            olk(i, Cart::xzzzzz) = PmB0 * olk(i, Cart::zzzzz) + double(nx[i]) * cfak * olk(i_less_x[i], Cart::zzzzz);
                            olk(i, Cart::yyyyyy) = PmB1 * olk(i, Cart::yyyyy) + double(ny[i]) * cfak * olk(i_less_y[i], Cart::yyyyy) + 5.0 * term_yyyy;
                            olk(i, Cart::yyyyyz) = PmB2 * olk(i, Cart::yyyyy) + double(nz[i]) * cfak * olk(i_less_z[i], Cart::yyyyy);
                            olk(i, Cart::yyyyzz) = PmB2 * olk(i, Cart::yyyyz) + double(nz[i]) * cfak * olk(i_less_z[i], Cart::yyyyz) + term_yyyy;
                            olk(i, Cart::yyyzzz) = PmB1 * olk(i, Cart::yyzzz) + double(ny[i]) * cfak * olk(i_less_y[i], Cart::yyzzz) + 2.0 * term_yzzz;
                            olk(i, Cart::yyzzzz) = PmB1 * olk(i, Cart::yzzzz) + double(ny[i]) * cfak * olk(i_less_y[i], Cart::yzzzz) + term_zzzz;
                            olk(i, Cart::yzzzzz) = PmB1 * olk(i, Cart::zzzzz) + double(ny[i]) * cfak * olk(i_less_y[i], Cart::zzzzz);
                            olk(i, Cart::zzzzzz) = PmB2 * olk(i, Cart::zzzzz) + double(nz[i]) * cfak * olk(i_less_z[i], Cart::zzzzz) + 5.0 * term_zzzz;
                        }
                        //------------------------------------------------------

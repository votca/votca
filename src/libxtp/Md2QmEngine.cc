    ...
    for (TOOLS::Property *segprop : segments) {
      ...
      for (TOOLS::Property *fragmentprop : fragments) {
        ...
        try {
          ...
          if (taken) throw runtime_error("(see above, naming collision)");
        ...
        vector<int> trihedron =
            fragmentprop->get("localframe").as<vector<int> >();
        ...
        for (it_md_atom_name = md_atoms_info.begin(),
            ...
            try {
              ...
              if (intCoords.at(qm_atom_id).first.substr(0, 1) !=
                ...
                cout << "WARNING: Atom " << md_atom_name << " in mol. "
                     ...
                     << md_atom_name.substr(0, 1) << ", but QM partner (ID "
                     << qm_atom_id << ") has element type "
          ...
          if (!hasQMPart && weight != 0) {
            cout << "ERROR: "
                 << "Atom " << md_atom_name << " in residue " << residue_name
                 << " in molecule " << molMdName
                 << " has no QM counterpart despite non-zero weight. " << endl;
            throw runtime_error("Error in mapping file");
          }

          // Create atom
          CTP::Atom *atom = AddAtomType(molecule, residue_name, residue_number,
          ...
          try {
            ...
            cout << "ERROR: "
                 ...
                 << ": "
                 << "Atom " << md_atom_name << " in residue " << residue_number
                 << " exists more than once. " << endl;
...
CTP::Molecule *Md2QmEngine::MoleculeFactory(CSG::Molecule *molMDTemplate) {
  ...
  for (int i = 0; i < molMDTemplate->BeadCount(); i++) {
    ...
    try {
      CTP::Atom *counterpart = this->getAtomType(
      ...
      cout << "WARNING: No mapping instruction found for atom "
           << atomMD->getName() << " in residue number " << atomMD->getResnr()
           << " in molecule " << molMDTemplate->getName() << ". Skipping..."
           << endl;
    }
  }
  return molQMTemplate;
...
CTP::Atom *Md2QmEngine::AddAtomType(CTP::Molecule *owner, string residue_name,
                                    int residue_number, string md_atom_name,
                                    int md_atom_id, bool hasQMPart,
                                    int qm_atom_id, vec qmPos, string element,
                                    double weight) {
  CTP::Atom *atom =
      new CTP::Atom(owner, residue_name, residue_number, md_atom_name,
...
CTP::SegmentType *Md2QmEngine::AddQMUnit(int qmunit_id, Property *property) {
  ...
  CTP::SegmentType *qmUnit =
      new CTP::SegmentType(qmunit_id, qmunit_name, basisSetName, orbitalsFile,
...
CTP::Atom *Md2QmEngine::getAtomType(const string &molMdName, int resNr,
                                    const string &mdAtomName) {
...
void Md2QmEngine::ReadXYZFile(string &file,
  ...
  if (!intt)
    throw runtime_error(string("Error reading coordinates from: ") + file);
...
void Md2QmEngine::PrintInfo() {
  ...
  cout << "Summary ~~~~~"
          "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"
       << endl;
  cout << "Created " << _molecule_types.size() << " molecule type(s): ";
  for (CTP::Molecule *mol : _molecule_types) {
    cout << "[ " << mol->getName() << " ]";
  }
  cout << endl
  ...
  for (const std::pair<string, string> &mssit :

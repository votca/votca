 ...
 */
void Orca::WriteBasisset(std::vector<QMAtom*>& qmatoms, std::string& bs_name,
...
void Orca::WriteCoordinates(std::ofstream& inp_file,
  ...
  for (QMAtom* atom : qmatoms) {
    ...
    inp_file << setw(3) << atom->getType().c_str() << setw(12)
...
void Orca::WriteBackgroundCharges() {
  ...
  for (std::shared_ptr<ctp::PolarSeg> seg : _PolarSegments) {
    for (ctp::APolarSite* site : *seg) {
      string sitestring =
          boost::str(fmt % site->getQ00() %
                     ((site->getPos().getX()) * votca::tools::conv::nm2ang) %
      ...
      if (site->getRank() > 0 || _with_polarization) {
        ...
        for (const auto& mpoles : _split_multipoles) {
          string multipole =
              boost::str(fmt % mpoles[3] % mpoles[0] % mpoles[1] % mpoles[2]);
  ...
  while (input_file) {
    ...
    if (OE_pos != std::string::npos) {
      ...
      for (unsigned i = 0; i < levels; i++) {
        ...
        if (levelnumber != i) {
          CTP_LOG(ctp::logDEBUG, *_pLog)
              << "Have a look at the orbital energies something weird is going "
                 "on"
              << flush;
        }
...
bool Orca::CheckLogFile() {
  ...
  while (input_file) {
    ...
    if (error != std::string::npos) {
      CTP_LOG(ctp::logERROR, *_pLog) << "ORCA encountered a fatal error, maybe "
                                        "a look in the log file may help."
                                     << flush;
      return false;
    }
    error = line.find(
...
bool Orca::ParseOrbitalsFile(Orbitals& orbitals) {
  ...
  CTP_LOG(ctp::logDEBUG, *_pLog) << "Number of operators: " << op_read

...
namespace xtp {
    ...
    Eigen::MatrixXd Sternheimer::CalculateHamiltonian(){
        ...
        Eigen::MatrixXd H =  overlap * mo_coefficients * mo_energies * mo_coefficients.transpose()*overlap;
...
//        std::cout<<ges.eigenvectors().col(0)<<std::endl;
        ...
        //Eigen::MatrixXd H = mo_coefficients.block(0,0,mo_coefficients.rows(),mo_coefficients.rows()) * mo_energies.block(0,0,mo_coefficients.rows(),mo_coefficients.rows()) * mo_coefficients.block(0,0,mo_coefficients.rows(),mo_coefficients.rows()).transpose();
        return H;
    ...
    Eigen::MatrixXcd Sternheimer::SternheimerLHS(Eigen::MatrixXcd hamiltonian, Eigen::MatrixXcd overlap, double eps, std::complex<double> omega, bool pm){
         
        Eigen::MatrixXcd S; //= Eigen::MatrixXd::Zero(overlap.cols(), overlap.rows());
    ...
    Eigen::VectorXcd Sternheimer::SternheimerRHS(Eigen::MatrixXcd overlap, Eigen::MatrixXcd density, Eigen::MatrixXcd pertubation, Eigen::VectorXcd coeff){
        //Setup Identity Matrix
        Eigen::MatrixXcd I = Eigen::MatrixXcd::Identity(overlap.rows(), overlap.cols());
    ...
    Eigen::VectorXcd Sternheimer::SternheimerSolve(Eigen::MatrixXcd& LHS, Eigen::VectorXcd& RHS){
...
//         return solver.solve(RHS);
    ...
    Eigen::MatrixXcd Sternheimer::DeltaVMatrix(Eigen::MatrixXcd deltaV){
        ...
        Eigen::MatrixXcd deltaVEx = Eigen::MatrixXcd::Zero(deltaV.cols(),deltaV.rows());
...
//                                    
    ...
    Eigen::MatrixXcd Sternheimer::CalculateDielectricMatrix(std::complex<double> w){    
    ...
    Eigen::MatrixXcd Sternheimer::CalculateScreenedCoulombOS(std::complex<double> w){
    ...
    Eigen::MatrixXcd Sternheimer::CalculateScreenedCoulombSC(std::complex<double> w){
        ...
        while(diff>tol&&count<max_iter){
            ...
            for(int v=0; v<num_occ_lvls; v++){
                ...
                RHS=SternheimerRHS(overlap,density,DeltaVMatrix(delta_V)+coulomb,mo_coefficients.col(v));
...
//            std::cout<<delta_V<<std::endl;
            ...
            if(diff<tol){
                std::cout<<"Self-consistency reached after "<<count<<"iterations."<<std::endl;
                std::cout<<"The last difference was "<<diff<<"."<<std::endl;
            }
            if(count>max_iter-1){                
                std::cout<<"Max interations reached with "<<count<<"iterations."<<std::endl;
                std::cout<<"The last difference was "<<diff<<"."<<std::endl;
            }
    ...
    Eigen::MatrixXcd Sternheimer::NonanalyticGreensfunction(Orbitals& orbitals, double w){
    ...
    Eigen::MatrixXcd Sternheimer::AnalyticGreensLHS(Orbitals& orbitals, double w, double eta){
    ...
    Eigen::MatrixXcd Sternheimer::AnalyticGreensolver(Eigen::MatrixXcd A, Eigen::MatrixXcd b){
    ...
    Eigen::MatrixXcd Sternheimer::AnalyticGreensfunction(Orbitals& orbitals, double w, double eta){
    ...
    Eigen::MatrixXcd Sternheimer::CalculateGreensfunction(Orbitals& orbitals, double w, double eta){
    ...
    Eigen::MatrixXcd Sternheimer::SelfEnergyEx(Orbitals orbitals){
        ...
        Eigen::MatrixXd SE = Eigen::MatrixXd::Zero(mo_coefficients.rows(), mo_coefficients.cols());
    ...
    Eigen::MatrixXcd Sternheimer::SelfEnergyC(Orbitals orbitals, Eigen::VectorXd grid, double w, double eta){
        ...
        Eigen::MatrixXcd SE = Eigen::MatrixXd::Zero(mo_coefficients.rows(),mo_coefficients.cols());
        ...
        for(int i=0; i<grid.size(); i++){
           ...
           SE+=AnalyticGreensfunction(orbitals, w+grid(i), eta)*(CalculateScreenedCoulombOS(grid(i))-V)*step;  
    ...
    Eigen::MatrixXcd Sternheimer::CalculateSelfEnergy(Orbitals& orbitals, double w, double eta, Eigen::VectorXd grid){
...
//    double Sternheimer::CalculateSpectralfunction(Eigen::MatrixXcd Selfenergy, Eigen::MatrixXcd deltaSE, double w){
//        
//        const double pi = 3.14159265358979323846;
//        
//        const Eigen::VectorXd& mo_energies = orbitals.MOEnergies();
...
//    }
//    
//    Eigen::VectorXcd Sternheimer::CalculateqpCorrection(Eigen::MatrixXcd Pertubation, Orbitals& orb){
//        
...
//    
//    
//    Eigen::VectorXcd Sternheimer::CalculateqpCorrection(Eigen::MatrixXcd SelfEnergy, Eigen::MatrixXcd VXC, Orbitals& orb){
//
//        const Eigen::MatrixXd& mo_coefficients = orb.MOCoefficients();
//        
//        Eigen::VectorXcd qpc = Eigen::VectorXcd::Zero(mo_coefficients.cols());
...
//    }
    ...
    Eigen::MatrixXcd Sternheimer::CalculateKSselfenergie(Eigen::MatrixXcd Selfenergie, Eigen::MatrixXd XC){
    ...
    bool Sternheimer::evaluate(){
        return true;
    }
    
//    Eigen::MatrixXd Sternheimer::Polarisability(Orbitals orb,Eigen::MatrixXd delta_n, int x, int y, double w){
//        

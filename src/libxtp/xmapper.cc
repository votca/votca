...
void XMpsMap::GenerateMap(string xml_file, string alloc_table, Topology *top) {
    ...
    for (segit = segs.begin(); segit != segs.end(); segit++) {
      ...
      for (fragit = frags.begin(); fragit != frags.end(); fragit++) {
        ...
        if ((*fragit)->exists("localframe_mps")) {
          cout << endl
               << "... ... ... ... " << segName << ": "
               << "Defining distinct local frame for polar sites." << flush;
          trihedron_mps = (*fragit)->get("localframe_mps").as<vector<int> >();
        } else {
          trihedron_mps = (*fragit)->get("localframe").as<vector<int> >();
        }
        ...
        if ((*fragit)->exists("weights_mps")) {
          cout << endl
               << "... ... ... ... " << segName << ": "
               << "Using distinct weights for polar sites." << flush;
          weights_mps = (*fragit)->get("weights_mps").as<vector<double> >();
        } else {
          weights_mps = (*fragit)->get("weights").as<vector<double> >();
        }
        ...
        if ((*fragit)->exists("virtual_mps")) {
          cout << endl
               << "... ... ... ... " << segName << ": "
               << "Checking for virtual expansion sites." << flush;
          vector<int> isVirtual_ints =
              (*fragit)->get("virtual_mps").as<vector<int> >();
        ...
        if ((mpoleIdcs.size() != mpoleNames.size()) ||
          ...
          cout << endl
               << "ERROR: Bad multipole definition in fragment '" << mapKeyName
               << "'. Misscounted?" << endl;
...
void XMpsMap::CollectSegMpsAlloc(string alloc_table, Topology *top) {
  ...
  if (intt.is_open()) {
    while (intt.good()) {
      ...
      assert(top->getSegment(segId)->getName() ==
             segName);  // <- Input matches topology?
    ...
    cout << endl << "ERROR: No such file " << alloc_table << endl;
    throw std::runtime_error("Input file missing.");
  }

  assert(_segId_mpsFile_n.size() ==
         top->Segments().size());  // <- Input for all segments?
}
...
void XMpsMap::CollectSitesFromMps() {
  ...
  // Neutral species
  for (misit = _segId_mpsFile_n.begin(); misit != _segId_mpsFile_n.end();
       ++misit) {
    if (_mpsFile_pSites.count(misit->second) > 0) {
      continue;
    } else {
      _mpsFile_pSites[misit->second] = APS_FROM_MPS(misit->second, 0);
    }
  }

  // Anion
  for (misit = _segId_mpsFile_e.begin(); misit != _segId_mpsFile_e.end();
       ++misit) {
    if (_mpsFile_pSites.count(misit->second) > 0) {
      continue;
    } else {
      _mpsFile_pSites[misit->second] = APS_FROM_MPS(misit->second, -1);
    }
  }

  // Cation
  for (misit = _segId_mpsFile_h.begin(); misit != _segId_mpsFile_h.end();
       ++misit) {
    if (_mpsFile_pSites.count(misit->second) > 0) {
      continue;
    } else {
  ...
  cout << endl
       << "... ... ..."
       << " Parsed " << _mpsFile_pSites.size() << " mps-files from "
       << _alloc_table << flush;
}
...
void XMpsMap::EquipWithPolSites(Topology *top) {
  ...
  for (sit = top->Segments().begin(); sit < top->Segments().end(); ++sit) {
    ...
    for (fit = seg->Fragments().begin(); fit < seg->Fragments().end(); ++fit) {
      ...
      string idkey =
          frag->getName() + seg->getName() + seg->getMolecule()->getName();
      ...
      if (map2md && polesInFrag.size() != frag->Atoms().size()) {
        cout << endl
             << "ERROR: Segment " << seg->getName() << " Fragment "
             << frag->getName()
             << ": MAP2MD = TRUE requires same number of polar "
             << "sites as there are atoms to perform mapping. " << endl;
      ...
      if (!map2md) {
        ...
        for (iit = trihedron_ints.begin(); iit < trihedron_ints.end(); ++iit) {
        ...
        if (symmetry == 3) {
          ...
          if (!warned_symm_idkey[idkey]) {
            cout << endl
                 << "... ... ... "
                 << "WARNING: Symmetry = 1 for fragment " << frag->getName()
                 << ": This will generate artifacts "
          ...
          cout << endl
               << "WARNING: Symmetry = 1 for fragment " << frag->getName()
               << ": This will generate artifacts "
               << "when mapping higher-rank multipoles (dipoles, ..)." << endl;

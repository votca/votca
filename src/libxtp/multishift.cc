...
namespace xtp {
    ...
    Eigen::VectorXcd Multishift::CBiCG(Eigen::MatrixXcd A, Eigen::VectorXcd b){
        ...
        while(res>tol){
            ...
            i++;
            if(i==max_iter){
                //std::cout<<"Max iter reached, res="<<res<<std::endl<<"Using HousholderQR instead."<<std::endl;
    ...
    Eigen::VectorXcd Multishift::DoMultishift(Eigen::MatrixXcd A, Eigen::VectorXcd b, std::complex<double> w){
        
        if(_r.empty()){
            //std::cout<<"Using HouseholderQR"<<std::endl;
            Eigen::MatrixXcd LHS=A+w*Eigen::MatrixXcd::Identity(_basis_size, _basis_size);
        ...
        while(i<_a.size()){
            ...
            pi_temp=pi_p;
            pi_p=(1+w*_a.at(i))*pi_p+_a.at(i)*_b.at(i-1)/_a.at(i-1)*(pi_p-pi);
            pi=pi_temp;
        }
        return x;
    }

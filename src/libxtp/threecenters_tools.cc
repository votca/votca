...
namespace votca {
    namespace xtp {
        ...
        void TCMatrix::Symmetrize(const ub::matrix<double>& _coulomb) {
	      ...
	      // ub::matrix<real> _temp = ub::prod(_coulomb, _matrix[ _i_occ ]);
	      ub::matrix<real> _temp = ub::prod(_coulomb, _matrix_double);
	  ...
	  //cout << "\n" << endl;
        ...
        void TCMatrix::Fill(AOBasis& _gwbasis, AOBasis& _dftbasis, ub::matrix<double>& _dft_orbitals) {
            ...
            //std::vector< ub::matrix<real> > _block(this->get_mtot());
	  ...
	  //void TCMatrix::FillBlock(std::vector< ub::matrix<real> >& _block, AOShell* _shell, AOBasis& dftbasis, ub::matrix<double>& _dft_orbitals) {
            ...
            //ub::matrix<real> _imstore = ub::zero_matrix<real>(this->mtotal * _shell->getNumFunc(), dftbasis._AOBasisSize);
            ...
            for (std::vector< AOShell* >::iterator _row = dftbasis.firstShell(); _row != dftbasis.lastShell(); _row++) {
                ...
                for (std::vector< AOShell* >::iterator _col = dftbasis.firstShell(); _col != dftbasis.lastShell(); _col++) {
                    ...
                    //ub::matrix<real> _subvector = ub::zero_matrix<real>(_shell_row->getNumFunc(), _shell->getNumFunc() * _shell_col->getNumFunc());
                    ...
                    if (nonzero) {
                        ...
                        //ub::matrix<real> _temp = ub::prod(_m_orbitals, _subvector);
            ...
            //ub::matrix<real> _temp = ub::prod(_imstore, _n_orbitals);

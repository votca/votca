...
public:
   ...
   ~ZMultipole() {};
    ...
    public:
        ...
        Interactor(Topology *top, ZMultipole *em) : _top(top), _em(em) {};
    ...
    class SiteOpZMultipole : public Thread
    {
    public:

        SiteOpZMultipole(int id, Topology *top,
                     ZMultipole *master)
 ...
 */
void ZMultipole::Initialize(Topology *top, Property *opt) {
    ...
    key = "options.zmultipole.multipoles";

        if ( opt->exists(key) ) {
            xmlfile = opt->get(key).as< string >();
        }

    key = "options.zmultipole.control";
        ...
        if ( opt->exists(key+".logbook") ) {
            _logFile = opt->get(key+".logbook").as< string >();
        }
        else { _logFile = "logbook"; }
    ...
    key = "options.zmultipole.alphamol";
    ...
    key = "options.zmultipole.tholeparam";
    ...
    key = "options.zmultipole.convparam";
 ...
 */
void ZMultipole::EStatify(Topology *top, Property *options) {

    cout << endl << "... ... Estatify system: ";

    string key = "options.zmultipole";
 ...
 */
vector<APolarSite*> ZMultipole::ParseGdmaFile(string filename, int state) {
 ...
 */
vector<vec> ZMultipole::ParseCubeFileHeader(string filename) {
 ...
 */
void ZMultipole::CalculateESPInput(Topology *top) {
 ...
 * ... NOTE Grid is read in from file in Gaussian cube format
 */
void ZMultipole::CalculateESPRigid(Topology *top) {
 ...
 */
void ZMultipole::CalculateESP(vector< APolarSite* > &poles, FILE *out) {
 ...
 */
void ZMultipole::CalculateESF(Topology *top) {
 ...
 */
void ZMultipole::CalculateAlphaInput(Topology *top) {
 ...
 */
void ZMultipole::CalculateAlphaRigid(vector< APolarSite* > &poles,
 ...
 */
void ZMultipole::SiteAlphaInduce(Topology *top, vector<APolarSite*> &poles) {
 ...
 */
void ZMultipole::DistributeMpoles(Topology *top) {
 ...
 */
bool ZMultipole::EvaluateFrame(Topology *top) {
    ...
    vector<SiteOpZMultipole*> siteOps;
 ...
 */
Segment *ZMultipole::RequestNextSite(int opId, Topology *top) {
...
void ZMultipole::SiteOpZMultipole::Run(void) {
 ...
 */
void ZMultipole::SiteOpZMultipole::InitSlotData(Topology *top) {
 ...
 */
ZMultipole::SiteOpZMultipole::~SiteOpZMultipole() {
 ...
 */
void ZMultipole::SiteOpZMultipole::EvalSite(Topology *top, Segment *seg) {
...
//        fclose(mpDat);
    ...
    // On-the-fly output
    FILE *out;
    string logbook = _master->_logFile;
    out = fopen(logbook.c_str(), "a");
    fprintf(out, "%5d %4.7f %4.7f %4.7f\n",
                  _seg->getId(),
                  _seg->getEMpoles(0),
                  _seg->getEMpoles(-1),
                  _seg->getEMpoles(+1));
 ...
 * @param state
 */
void ZMultipole::SiteOpZMultipole::Charge(int state) {
 ...
 */
int ZMultipole::SiteOpZMultipole::Induce(int state) {
...
double ZMultipole::SiteOpZMultipole::EnergyStatic(int state) {
 ...
 */
double ZMultipole::SiteOpZMultipole::Energy(int state) {
 ...
 */
void ZMultipole::SiteOpZMultipole::Depolarize() {
 ...
 */
inline double ZMultipole::Interactor::PotentialPerm(vec r,
                                                     APolarSite &pol) {
 ...
 */
inline vec ZMultipole::Interactor::FieldPermESF(vec r,
                                                 APolarSite &pol) {
    // NOTE >>> e12 points from polar site 1 to polar site 2 <<< NOTE //
    e12  = pol.getPos() - r;
    R    = 1/abs(e12);
 ...
 */
inline void ZMultipole::Interactor::FieldInduAlpha(APolarSite &pol1,
                                                    APolarSite &pol2) {
    // NOTE >>> e12 points from polar site 1 to polar site 2 <<< NOTE //
 ...
 */
inline void ZMultipole::Interactor::FieldIndu(APolarSite &pol1,
                                               APolarSite &pol2) {
 ...
 * Used in self-consistent field calculation (evaluation stage)
 */
inline void ZMultipole::Interactor::FieldPerm(APolarSite &pol1,
                                               APolarSite &pol2) {
 ...
 */
inline double ZMultipole::Interactor::EnergyIntra(APolarSite &pol1,
                                                   APolarSite &pol2) {
 ...
 * Used in energy evaluation of converged fields (evaluation stage)
 */
inline double ZMultipole::Interactor::EnergyInter(APolarSite &pol1,
                                                   APolarSite &pol2) {
 ...
 */
inline double ZMultipole::Interactor::EnergyInterESP(APolarSite &pol1,
                                                      APolarSite &pol2) {

...
exit(0);
        ...
        void ERIs::CalculateERIs (ub::matrix<double> &DMAT, AOOverlap &_auxAOverlap){
...
/*
            ...
            */
            ...
            cout << ub::prod(_auxAOverlap._aomatrix , _inverse);
            ...
            for (int _i = 0; _i < _threecenter.getSize(); _i++) {
                ...
                //cout << " S-1(" << _i << "," << _j << ") " << _inverse(_i,_j) << endl; 
                
                    ub::matrix<double>& _3c = _threecenter.getDatamatrix(_j);

                    for ( int alpha = 0 ; alpha < _auxAOverlap.Dimension(); alpha++ ){
                        for ( int beta = 0 ; beta < _auxAOverlap.Dimension(); beta++ ){
                        
                            _FITMAT(alpha,beta) += _inverse(_i,_j)*_3c(alpha,beta);

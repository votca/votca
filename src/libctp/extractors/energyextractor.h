#ifndef VOTCA_CTP_ENERGYEXTRACTOR_H
#define VOTCA_CTP_ENERGYEXTRACTOR_H

#include <votca/ctp/qmcalculator.h>
#include <boost/format.hpp>

namespace votca { namespace ctp {


class EnergyExtractor : public QMCalculator
{
public:

    EnergyExtractor() { };
   ~EnergyExtractor() { };

    string Identify() { return "extract.energy"; }
    void Initialize(Property *options);
    bool EvaluateFrame(Topology *top);

private:

};


void EnergyExtractor::Initialize(Property *options) {
    return;
}


bool EnergyExtractor::EvaluateFrame(Topology *top) {
    
    string xmlfile = Identify() + ".xml";    
    
    Property state("state", "", "");
    Property &segs = state.add("segments","");
    Property &pairs = state.add("pairs","");
    Property *next = NULL;
    
    
    using boost::format;
    
    // SEGMENTS
    vector<Segment*> ::iterator sit;
    next = &segs;
    for (sit = top->Segments().begin(); sit < top->Segments().end(); ++sit) {
        Segment *seg = *sit;
        Property &segprop = next->add("segment", "");
        segprop.add("id", (format("%1$d") % seg->getId()).str());
        segprop.add("name", seg->getName());
        if (seg->hasChrgState(+1)) {
            segprop.add("energy_hn", (format("%1$+1.7f") % seg->getSiteEnergy(+1)).str());
            segprop.add("lambda_hn", (format("%1$+1.7f") % seg->getU_nC_nN(+1)).str());
            segprop.add("lambda_nh", (format("%1$+1.7f") % seg->getU_cN_cC(+1)).str());
        }
        if (seg->hasChrgState(-1)) {
            segprop.add("energy_en", (format("%1$+1.7f") % seg->getSiteEnergy(-1)).str());
            segprop.add("lambda_en", (format("%1$+1.7f") % seg->getU_nC_nN(-1)).str());
            segprop.add("lambda_ne", (format("%1$+1.7f") % seg->getU_cN_cC(-1)).str());
        }
    }
    
    // PAIRS
    QMNBList::iterator pit;
    QMNBList &nb = top->NBList();
    next = &pairs;
    for (pit = nb.begin(); pit != nb.end(); ++pit) {
        QMPair *qmp = *pit;
        Property &pairprop = next->add("pair", "");
        pairprop.add("id1",   (format("%1$d")   % qmp->Seg1()->getId()).str());
        pairprop.add("name1", (format("%1$s")   % qmp->Seg1()->getName()).str());
        pairprop.add("id2",   (format("%1$d")   % qmp->Seg2()->getId()).str());
        pairprop.add("name2", (format("%1$s")   % qmp->Seg2()->getName()).str());
        
        if (qmp->isPathCarrier(+1)) {
            pairprop.add("deltaE_h12", (format("%1$1.7f") % qmp->getdE12(+1)).str());
            pairprop.add("lambdaI_h12", (format("%1$1.7f") % qmp->getReorg12(+1)).str());
            pairprop.add("lambdaI_h21", (format("%1$1.7f") % qmp->getReorg21(+1)).str());
            pairprop.add("lambdaO_h", (format("%1$1.7f") % qmp->getLambdaO(+1)).str());
        }
        if (qmp->isPathCarrier(-1)) {
            pairprop.add("deltaE_e12", (format("%1$1.7f") % qmp->getdE12(-1)).str());
            pairprop.add("lambdaI_e12", (format("%1$1.7f") % qmp->getReorg12(-1)).str());
            pairprop.add("lambdaI_e21", (format("%1$1.7f") % qmp->getReorg21(-1)).str());
            pairprop.add("lambdaO_e", (format("%1$1.7f") % qmp->getLambdaO(-1)).str());
        }
    }
    
    
    
    ofstream ofs;    
    ofs.open(xmlfile.c_str(), ofstream::out);
    if (!ofs.is_open()) {
        throw runtime_error("Bad file handle: " + xmlfile);
    }
    PrintNodeXML(ofs, state, 0, 0, "", "");
    ofs.close();
    
    return true;
}


}}

#endif // VOTCA_CTP_ENERGYEXTRACTOR_H

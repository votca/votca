...
void GraphVisitor::exploreNode_(pair<int, GraphNode>& vertex_and_node,
...
void GraphVisitor::exec(Graph& graph, Edge edge) {
  ...
  exploreNode_(vertex_and_node, graph, edge);
}
...
Edge GraphVisitor::nextEdge(Graph graph) {
  ...
  auto unexplored_vertices = getUnexploredVertex(edge);

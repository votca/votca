...
long EdgeContainer::getMaxDegree(void) const {
  long max = 0;
  for (const auto& vertex_and_neigh_and_count : adj_list_) {
    long degree = getDegree(vertex_and_neigh_and_count.first);
    if (degree > max) {
      max = degree;
    }
  }
...
long EdgeContainer::getDegree(const long vertex) const {
  ...
  for (const pair<long int, long>& neighbor_and_count : adj_list_.at(vertex)) {
...
vector<long> EdgeContainer::getVerticesDegree(long degree) const {
  vector<long> vertices;
  for (const auto& vertex_and_neigh_and_count : adj_list_) {
    long degree_count = getDegree(vertex_and_neigh_and_count.first);
...
bool EdgeContainer::vertexExistWithDegree(long degree) const {
  for (const auto& vertex_and_neigh_and_count : adj_list_) {
    long degree_count = getDegree(vertex_and_neigh_and_count.first);
    if (degree_count == degree) {
      return true;
...
bool EdgeContainer::vertexExist(const long vertex) const {
  return adj_list_.count(vertex);
}

void EdgeContainer::addEdge(Edge edge) {

  long point1 = edge.getEndPoint1();
  long point2 = edge.getEndPoint2();
...
void EdgeContainer::addVertex(long vertex) {
  assert(adj_list_.count(vertex) == 0 && "Cannot add vertex already exists");
  unordered_map<long int, long> empty_temp;
  adj_list_[vertex] = empty_temp;
}

vector<long> EdgeContainer::getVertices() const {
  vector<long> vertices;
...
vector<long> EdgeContainer::getNeighVertices(long vertex) const {
  vector<long> neigh_verts;
  if (adj_list_.count(vertex)) {
    for (const pair<long int, long>& neigh_and_count : adj_list_.at(vertex)) {
...
vector<Edge> EdgeContainer::getNeighEdges(long vertex) const {
...
vector<Edge> EdgeContainer::getEdges() const {
  unordered_map<Edge, long> extra_edge_count;
  for (const auto& vertex_and_neigh_and_count : adj_list_) {
    for (const pair<long int, long>& neigh_and_count :
  ...
  vector<Edge> edges;
  for (pair<const Edge, long>& edge_count : extra_edge_count) {
    for (long count = 0; count < edge_count.second; ++count) {

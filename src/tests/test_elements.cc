...
BOOST_AUTO_TEST_CASE(accessors_test) {
  ...
  BOOST_CHECK_EQUAL(ceil_(ele.getCovRad("Cl","ang"),3),1.02);
  BOOST_CHECK_EQUAL(ceil_(ele.getCovRad("Cl","nm"),3),0.102);
  BOOST_CHECK_THROW(ceil_(ele.getCovRad("Cl","Blah"),3),invalid_argument);
}

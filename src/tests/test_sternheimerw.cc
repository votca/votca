...
BOOST_AUTO_TEST_CASE(sternheimer_polar) {
  ...
  //Mmn.Fill(aobasis, aobasis, MOs);
    ...
    std::vector<Eigen::Matrix3cd> polar=sternheimer.Polarisability(0,5,10,3,0.3,10);

    std::vector<double> iA = sternheimer.getIsotropicAverage(polar);

    for(int i=0;i<iA.size();i++){
        std::cout<<iA.at(i)<<std::endl;
    }
    double test1;
    double test2;

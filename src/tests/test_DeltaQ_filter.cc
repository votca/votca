...
BOOST_AUTO_TEST_CASE(coeffs_test) {
  ...
  prop.LoadFromXML("chargeTransfer.xml");
  deltaQ->Initialize(prop.get("root"));
  ...
  BOOST_CHECK_EQUAL(deltaQ->NeedsInitialState(), false);

  std::vector<votca::Index> ref = {0, 1};
  std::vector<votca::Index> results =
      deltaQ->CalcIndeces(A, QMStateType::Singlet);

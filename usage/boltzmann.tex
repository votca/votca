...
Afterwards, non-bonded potentials can be obtained using iterative  methods or force matching. There are several useful tools for this method, which help to analyze distributions of specified degrees of freedom, their correlations, prepare tabulated potentials for coarse-grained simulations. In addition, exclusion lists for the atomistic simulations can be created automatically, which is often used for polymer coarse-graining, where it is difficult to separate bonded and non-bonded degrees of freedom~\cite{Tschoep:1998}.

The main tool which can be used to calculate hystograms, cross-correlate coarse-grained variables, and create exclusion lists is \prog{csg_boltzmann}.  Note that it parses the whole trajectory and stores all information bonded interactions in memory. This is useful for interactive analysis, but can be problematic for big systems. In the latter case, one can use \prog{csg_stat} which, however, has a limited number of tasks it can perform.

\prog{csg_map} can also useful to convert an atomistic trajectory to a coarse-grained one. The usage of this program is discussed in sec.~\ref{sec:trajectory}.

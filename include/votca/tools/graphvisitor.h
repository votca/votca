...
class GraphVisitor {
 protected:
  ...
  std::set<long int> explored_;

  /// The vertex the visitor started on
  long int startingVertex_ = 0;

  /// What is done to an individual graph node as it is explored
  virtual void addEdges_(const Graph& graph, long int vertex) = 0;
 ...
 public:
  ...
  std::vector<long int> getUnexploredVertex(const Edge edge) const;
  ...
  void setStartingVertex(long int vertex) { startingVertex_ = vertex; }
  long int getStartingVertex() const { return startingVertex_; }
  ...
  std::set<long int> getExploredVertices() const;

  /// Has the vertex been explored
  bool vertexExplored(const long int vert) const;
};
}  // namespace tools
}  // namespace votca

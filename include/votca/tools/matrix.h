...
inline void matrix::checkBounds_(const byte_t row_or_col) const {
  assert( boost::lexical_cast<int>(row_or_col) > -1 && boost::lexical_cast<int>(row_or_col) < 3);
} 
...
inline matrix &matrix::operator=(const matrix &m) {
  for (size_t i = 0; i < 9; ++i) _m[i] = m._m[i];
  return *this;
}
...
inline void matrix::ZeroMatrix() {
  for (size_t i = 0; i < 9; ++i) _m[i] = 0.;  //(*this) = 0.;
}
...
inline matrix operator*(const double &d, const matrix &m) {
  matrix mm;
  for (size_t i = 0; i < 9; ++i) mm._m[i] = d * m._m[i];
  return mm;
}

inline vec operator*(const matrix &r, const vec &a) { return matrix(r) * a; }

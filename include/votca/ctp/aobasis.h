...
public:
    ...
    void EvalAOspace(ub::matrix_range<ub::matrix<double> >& AOvalues, double x, double y, double z , string type = "");
    void EvalAOgrid(ub::matrix_range<ub::matrix<double> >& AOvalues, double x, double y, double z , const bool doGrad );
    ...
    /*
       inline void AOShell::EvalAOGradspace(ub::matrix<double>& gradAOvalues, double x, double y, double z, string type ){
           ...
           inline void AOShell::EvalAOspace(ub::matrix_range<ub::matrix<double> >& AOvalues, double x, double y, double z, string type ){
            ...
            for (GaussianIterator itr = firstGaussian(); itr != lastGaussian(); ++itr) {
                ...
                for (int i = 0; i < shell_type.length(); ++i) {
                    ...
                    if (single_shell == "S") {
                        ...
                        if (doGrad) {
                            i_act = _i_func+1;
                            AOvalues(i_act, 1) += _contractions[0] * -2.0 * alpha * center_x*_expofactor; // x gradient of s-function
                            AOvalues(i_act, 2) += _contractions[0] * -2.0 * alpha * center_y*_expofactor; // y gradient of s-function
                            AOvalues(i_act, 3) += _contractions[0] * -2.0 * alpha * center_z*_expofactor; // z gradient of s-function
                        }
                    ...
                    if (single_shell == "P") {
                        ...
                        if (doGrad) {
                            i_act = _i_func+1;
                            AOvalues(i_act, 1) += _contractions[1] * 2.0 * sqrt(alpha) * (1.0 - 2.0 * alpha * center_x * center_x) * _expofactor; // x gradient 
                            AOvalues(i_act, 2) += _contractions[1] * -2.0 * sqrt(alpha) * 2.0 * alpha * center_x * center_y*_expofactor; // y gradient 
                            AOvalues(i_act, 3) += _contractions[1] * -2.0 * sqrt(alpha) * 2.0 * alpha * center_x * center_z*_expofactor; // z gradient 

                            // py -functions
                            i_act = _i_func + 2;
                            AOvalues(i_act, 1) += _contractions[1] * -2.0 * sqrt(alpha) * 2.0 * alpha * center_x * center_y*_expofactor; // x gradient 
                            AOvalues(i_act, 2) += _contractions[1] * 2.0 * sqrt(alpha) * (1.0 - 2.0 * alpha * center_y * center_y) * _expofactor; // y gradient 
                            AOvalues(i_act, 3) += _contractions[1] * -2.0 * sqrt(alpha) * 2.0 * alpha * center_y * center_z*_expofactor; // z gradient 

                            // pz -functions
                            i_act = _i_func + 3;
                            AOvalues(i_act, 1) += _contractions[1] * -2.0 * sqrt(alpha) * 2.0 * alpha * center_x * center_z*_expofactor; // x gradient 
                            AOvalues(i_act, 2) += _contractions[1] * -2.0 * sqrt(alpha) * 2.0 * alpha * center_y * center_z *_expofactor; // y gradient 
                            AOvalues(i_act, 3) += _contractions[1] * 2.0 * sqrt(alpha) * (1.0 - 2.0 * alpha * center_z * center_z) * _expofactor; // z gradient      
                    ...
                    if (single_shell == "D") {
                        ...
                        if (doGrad) {
                            i_act = _i_func+1;
                            AOvalues(i_act, 1) += _contractions[2] * 4.0 * alpha * (center_z - 2.0 * alpha * center_x * center_x * center_z) * _expofactor; // x gradient 
                            AOvalues(i_act, 2) += _contractions[2] * -8.0 * alpha * alpha * center_x * center_y * center_z*_expofactor; // y gradient 
                            AOvalues(i_act, 3) += _contractions[2] * 4.0 * alpha * (center_x - 2.0 * alpha * center_x * center_z * center_z) * _expofactor; // z gradient 

                            // dyz function
                            i_act = _i_func + 2;
                            AOvalues(i_act, 1) += _contractions[2] * -8.0 * alpha * alpha * center_x * center_y * center_z*_expofactor; // x gradient 
                            AOvalues(i_act, 2) += _contractions[2] * 4.0 * alpha * (center_z - 2.0 * alpha * center_y * center_y * center_z) * _expofactor; // y gradient 
                            AOvalues(i_act, 3) += _contractions[2] * 4.0 * alpha * (center_y - 2.0 * alpha * center_y * center_z * center_z) * _expofactor; // z gradient 

                            // dxy function
                            i_act = _i_func + 3;
                            AOvalues(i_act, 1) += _contractions[2] * 4.0 * alpha * (center_y - 2.0 * alpha * center_x * center_x * center_y) * _expofactor; // x gradient 
                            AOvalues(i_act, 2) += _contractions[2] * 4.0 * alpha * (center_x - 2.0 * alpha * center_x * center_y * center_y) * _expofactor; // y gradient 
                            AOvalues(i_act, 3) += _contractions[2] * -8.0 * alpha * alpha * center_x * center_y * center_z*_expofactor; // z gradient 

                            // d3z2-r2-function
                            i_act = _i_func + 4;
                            AOvalues(i_act, 1) += _contractions[2] * -4.0 * alpha / sqrt(3.0) * center_x * (1.0 + alpha * (3.0 * center_z * center_z - distsq)) * _expofactor;
                            AOvalues(i_act, 2) += _contractions[2] * -4.0 * alpha / sqrt(3.0) * center_y * (1.0 + alpha * (3.0 * center_z * center_z - distsq)) * _expofactor;
                            AOvalues(i_act, 3) += _contractions[2] * 4.0 * alpha / sqrt(3.0) * center_z * (2.0 - alpha * (3.0 * center_z * center_z - distsq)) * _expofactor;

                            // dx2-y2-function
                            i_act = _i_func + 5;
                            AOvalues(i_act, 1) += _contractions[2] * 4.0 * alpha * center_x * (1.0 - alpha * (center_x * center_x - center_y * center_y)) * _expofactor;
                            AOvalues(i_act, 2) += _contractions[2] * -4.0 * alpha * center_y * (1.0 + alpha * (center_x * center_x - center_y * center_y)) * _expofactor;
                            AOvalues(i_act, 3) += _contractions[2] * -4.0 * alpha * alpha * center_z * (center_x * center_x - center_y * center_y) * _expofactor;
            ...
            inline void AOShell::EvalAOgrid(ub::matrix_range<ub::matrix<double> >& AOvalues, double x, double y, double z, const bool doGrad ){

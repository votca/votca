...
class GW {
 public:
  ...
  struct options {
    ...
    Index reset_3c = 5;  // how often the 3c integrals in iterate should be
                         // rebuild
    double qp_grid_range = 0.0;  // Left, right distance from grid origin
 ...
 private:
  ...
  Eigen::VectorXd SolveQP_Grid(Eigen::VectorXd frequencies);
  Eigen::VectorXd SolveQP_SelfConsistent(Eigen::VectorXd frequencies);
  Eigen::VectorXd IterateQP_FixedPoint() const;

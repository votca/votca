...
class Md2QmEngine {
 ...
 private:
  ...
  // MD <-> QM Maps
  map<string, map<int, map<string, CTP::Atom *> > > _map_mol_resNr_atm_atmType;
  ...
  CTP::Atom *AddAtomType(CTP::Molecule *owner, string residue_name,
                         int residue_number, string md_atom_name,
                         int md_atom_id, bool hasQMPart, int qm_atom_id,
                         vec qmpos, string element, double weight);

  const string &getMoleculeName(const string &mdname);
  CTP::Molecule *getMoleculeType(const string &name);
  CTP::Atom *getAtomType(const string &molMdName, int resNr,
                         const string &mdAtomName);
  void ReadXYZFile(string &file, map<int, pair<string, vec> > &intCoords);
};

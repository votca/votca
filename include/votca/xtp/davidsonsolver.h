...
class DavidsonSolver {
 ...
 public:
  ...
  void set_iter_max(long N) { this->_iter_max = N; }
  void set_max_search_space(long N) { this->_max_search_space = N; }
  ...
  long num_iterations() const { return this->_num_iter; }

  template <typename MatrixReplacement>
  void solve(const MatrixReplacement &A, long neigen,
             long size_initial_guess = 0) {
    ...
    for (long iiter = 0; iiter < _iter_max; iiter++) {
      ...
      // etend the subspace
      long nupdate = extendProjection(rep, proj);
 ...
 private:
  ...
  struct ProjectedSpace {
    ...
    long search_space;   // size of the projection i.e. number of cols in V
    long size_update;    // size update ...
  ...
  void updateProjection(const MatrixReplacement &A, ProjectedSpace &proj,
    ...
    if (iiter == 0 || _davidson_ortho == ORTHO::QR) {
      ...
      long size = proj.V.rows();
      long old_dim = proj.T.cols();
      long new_dim = proj.V.cols();
      long nvec = new_dim - old_dim;
  ...
  long getSizeUpdate(long neigen) const;

  void checkOptions(long operator_size);

  void printOptions(long operator_size) const;
  ...
  void printIterationData(const RitzEigenPair &rep, const ProjectedSpace &proj,
                          long neigen, long iiter) const;

  ArrayXl argsort(const Eigen::VectorXd &V) const;

  Eigen::MatrixXd setupInitialEigenvectors(long size) const;

  Eigen::MatrixXd extract_vectors(const Eigen::MatrixXd &V,
                                  const ArrayXl &idx) const;

  Eigen::MatrixXd orthogonalize(const Eigen::MatrixXd &V, long nupdate);
  Eigen::MatrixXd qr(const Eigen::MatrixXd &A) const;
  Eigen::MatrixXd gramschmidt(const Eigen::MatrixXd &A, long nstart);
  ...
  ProjectedSpace initProjectedSpace(long neigen, long size_initial_guess) const;

  long extendProjection(RitzEigenPair &rep, ProjectedSpace &proj);

  void restart(const RitzEigenPair &rep, ProjectedSpace &proj,
               long size_restart) const;

  void storeConvergedData(const RitzEigenPair &rep, long neigen, long iiter);

  void storeNotConvergedData(const RitzEigenPair &rep,
                             std::vector<bool> &root_converged, long neigen);

  void storeEigenPairs(const RitzEigenPair &rep, long neigen);
};

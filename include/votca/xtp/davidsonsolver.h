            ...
            void solve(MatrixReplacement &A, int neigen, int size_initial_guess = 0)
            {
                ...
                for (int iiter = 0; iiter < iter_max; iiter++ ) {
                    ...
                    CTP_LOG(ctp::logDEBUG, _log)  << ctp::TimeStamp() 
                       << format(" %1$4d \t %2$12d \t %3$4.2e") % iiter % search_space % res_norm << flush;         
                ...
                CTP_LOG(ctp::logDEBUG, _log)  << ctp::TimeStamp()  << "-----------------------------------" << flush;
                if (!has_converged) {
                    CTP_LOG(ctp::logDEBUG, _log)  << ctp::TimeStamp() << "- Warning : Davidson didn't converge ! " <<  flush; 
                    this->_eigenvalues = Eigen::VectorXd::Zero(neigen);
                    this->_eigenvectors = Eigen::MatrixXd::Zero(size,neigen);
                }
                else   {
                    CTP_LOG(ctp::logDEBUG, _log)  << ctp::TimeStamp() << "- Davidson converged " <<  flush; 

...
class Sternheimer {
 public:
  ...
  Eigen::MatrixXcd CalculateGreensfunction(Orbitals& orbitals, double w,
                                           double eta);
  // Calculates the selfenergy of the system
  Eigen::MatrixXcd CalculateSelfEnergy(Orbitals& orbitals, double w, double eta,
                                       Eigen::VectorXd grid);
  // Calculates the spectral function
  double CalculateSpectralfunction(Eigen::MatrixXcd Selfenergy,
                                   Eigen::MatrixXcd deltaSE, double w);

  Eigen::VectorXcd CalculateqpCorrection(Eigen::MatrixXcd Pertubation,
                                         Orbitals& orb);

  Eigen::VectorXcd CalculateqpCorrection(Eigen::MatrixXcd SelfEnergy,
                                         Eigen::MatrixXcd VXC, Orbitals& orb);
 ...
 private:
  ...
  Eigen::MatrixXcd SternheimerLHS(Eigen::MatrixXcd hamiltonian,
                                  ...
                                  std::complex<double> omega, bool pm);
  ...
  Eigen::VectorXcd SternheimerSolve(Eigen::MatrixXcd& LHS,
                                    Eigen::VectorXcd& RHS);
  // Calculates the matrix elements for the pertubation used in the Sternheimer
  // equation
  ...
  Eigen::MatrixXcd AnalyticGreensfunction(Orbitals& orbitals, double w,
                                          double eta);
  ...
  Eigen::MatrixXcd CalculateAnalyticSelfEnergy(Orbitals& orbitals, double w,
                                               double eta, double delta,
                                               Eigen::VectorXd grid);

  Eigen::MatrixXcd CalculateNonAnalyticSelfEnergy(Orbitals& orbitals, double w,
                                                  double delta);

  Eigen::MatrixXcd SelfEnergyC(Orbitals orbitals, Eigen::VectorXd grid,
                               double w, double eta);

  Eigen::MatrixXcd SelfEnergyEx(Orbitals orbitals);

  Eigen::MatrixXcd CalculateKSselfenergie(Eigen::MatrixXcd Selfenergie,
                                          Eigen::MatrixXd XC);

  Eigen::MatrixXd Polarisability(Eigen::MatrixXd delta_n, int x, int y,
                                 double omega);
};
}  // namespace xtp
}  // namespace votca
...
// Eigen::MatrixXcd SternheimerW::Polarisability(std::complex<double> w,
// std::string gridtype){
//
//        clock_t begin = clock();
//
...
//        numint.GridSetup(gridtype, _orbitals.QMAtoms(), dftbasis); //For now
//        use medium grid const int& basis_size = _orbitals.getBasisSetSize();
...
//        "<<double(clock()-begin)/CLOCKS_PER_SEC<<std::endl;
//
//        return Polar;
//    }
//
//    std::vector<Eigen::MatrixXcd> SternheimerW::DeltaNOS(std::complex<double>
//    w, std::string gridtype){
//
//
...
//        equation const Eigen::MatrixXd H=Hamiltonian(); const Eigen::MatrixXd
//        S=OverlapMatrix(); const Eigen::MatrixXd p=DensityMatrix();
...
//
//        Eigen::MatrixXcd delta_c_p=Eigen::MatrixXcd::Zero(basis_size,
//        basis_size); Eigen::MatrixXcd
//        delta_c_m=Eigen::MatrixXcd::Zero(basis_size, basis_size);
//
...
//        Eigen::MatrixXcd delta_n=Eigen::MatrixXcd::Zero(basis_size,
//        basis_size);
//

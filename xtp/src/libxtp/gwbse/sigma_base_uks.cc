...
Eigen::MatrixXd Sigma_base_UKS::CalcExchangeMatrix() const {
...
Eigen::VectorXd Sigma_base_UKS::CalcCorrelationDiag(
...
Eigen::MatrixXd Sigma_base_UKS::CalcCorrelationOffDiag(
    const Eigen::VectorXd& frequencies) const {
  Eigen::MatrixXd result = Eigen::MatrixXd::Zero(qptotal_, qptotal_);
#pragma omp parallel for schedule(dynamic)
  for (Index gw_level1 = 0; gw_level1 < qptotal_; gw_level1++) {

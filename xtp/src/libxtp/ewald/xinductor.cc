...
void XInductor::Evaluate(XJob* job) {
  ...
  if (job->StartFromCPT()) {
    ...
    XTP_LOG(Log::debug, *_log)
        << "Carry out partial depolarization." << std::flush;
    ...
    XTP_LOG(Log::debug, *_log)
        << "Carry out full depolarization." << std::flush;
    std::vector<PolarSeg*>::iterator sit;
    std::vector<APolarSite*>::iterator pit;
  ...
  if (this->_induce)
    ...
    this->EnergyStatic(job);
  boost::timer::cpu_times t2 = cpu_t.elapsed();

  double t_indu = (t1.wall - t0.wall) / 1e9 / 60.;
  double t_ener = (t2.wall - t1.wall) / 1e9 / 60.;
  XTP_LOG(Log::info, *_log)
      << (format("  o Total:     %1$1.2f min") % (t_ener + t_indu))
      << std::flush;
  XTP_LOG(Log::info, *_log)
      << (format("  o Induction: %1$1.2f min") % (t_indu)) << std::flush;
  XTP_LOG(Log::info, *_log)
      << (format("  o Energy:    %1$1.2f min") % (t_ener)) << std::flush;
...
int XInductor::Induce(XJob* job) {
  ...
  XTP_LOG(Log::info, *_log) << "Inductor: Using WSOR = " << wSOR
                            << ", ASHARP = " << _aDamp << std::flush;
  ...
  for (sit1 = _qmm.begin(); sit1 < _qmm.end(); ++sit1) {
    for (sit2 = sit1 + 1; sit2 < _qmm.end(); ++sit2) {
      ...
      double dr = ((*sit1)->getPos() - (*sit2)->getPos()).norm();
      ...
      // Interaction atom <> fragment (cg)
        ...
        for (fit = (*sit2)->PolarFrags().begin();
             fit < (*sit2)->PolarFrags().end(); ++fit) {
          for (pit1 = (*sit1)->begin(); pit1 < (*sit1)->end(); ++pit1) {
  ...
  if (true || !job->StartFromCPT()) {
    ...
    XTP_LOG(Log::info, *_log)
        << "Job started from archive, reusing existing "
        << "state as starting configuration" << std::flush;
  }
  ...
  for (int ridx = 0; ridx < 1; ++ridx) {
    ...
    for (; iter_cg < maxI; ++iter_cg) {
        ...
        for (sit2 = sit1 + 1; sit2 < _qmm.end(); ++sit2) {
          ...
          if (dr > rc_seg) {
            ...
            for (pit1 = (*sit1)->begin(); pit1 < (*sit1)->end(); ++pit1) {
              ...
              _actor.FieldPermAsIndu_At_By(*(*pit1),
                                           *((*sit2)->getInduCgSite()));
            }
            for (pit2 = (*sit2)->begin(); pit2 < (*sit2)->end(); ++pit2) {
              _actor.BiasIndu(*(*pit2), *((*sit1)->getInduCgSite()));
              _actor.FieldPermAsIndu_At_By(*(*pit2),
                                           *((*sit1)->getInduCgSite()));
            }
          }
          // Interaction atom <> fragment (cg)
      ...
      XTP_LOG(Log::info, *_log)
          << (boost::format(
                  "  ITER %1$3d  |  AVG(dU/U) %3$1.7e  EPS %11$1.1e  |  "
                  "NN(0<>%7$1.1f<>%6$1.1f) %8$04d|%9$04d|%10$04d") %
              iter_cg % maxdU_U % avgdU_U % rmsdU % baseN % rc_seg % rc_frag %
              count_atm_atm % count_atm_frag % count_atm_seg % eTOL)
                 .str();
      XTP_LOG(Log::info, *_log)
          << (format("  |  T=%1$1.2fs CG/I/O %2$2.2f%% %3$2.2f%% %4$2.2f%%") %
  ...
  //
  //        XTP_LOG(Log::info,*_log) << (boost::format(
  //            "  Iter %1$3d | max(dU/U) %2$1.7e  avg(dU/U) %3$1.7e  rms(dU)
  //            %4$1.7e  N %5$d") % iter % maxdU_U % avgdU_U % rmsdU %
  //            baseN).str() << std::flush;
  //
  //        // Break if converged
  //        if      (converged) {
  ...
  //            this->setError((boost::format("Did not converge to precision "
  //               "(%1$d steps, AVG(dU:U) = %2$1.3e)") % maxI %
  //               avgdU_U).str());
  //            break;
  //        }
  //    }
...
double XInductor::Energy(XJob* job) {
  ...
  //    std::cout << std::endl << "E_f_intra_0 " << e_f_intra_0*int2eV <<
  //    std::flush; std::cout << std::endl << "E_m_intra_0 " <<
  //    e_m_intra_0*int2eV << std::flush; std::cout << std::endl << "E_f_intra_1
  //    " << e_f_intra_1*int2eV << std::flush; std::cout << std::endl <<
  //    "E_m_intra_1 " << e_m_intra_1*int2eV << std::flush; std::cout <<
  //    std::endl << "E_f_intra_2 " << e_f_intra_2*int2eV << std::flush;
  //    std::cout << std::endl << "E_m_intra_2 " << e_m_intra_2*int2eV <<
  //    std::flush; std::cout << std::endl << "E_m_0 " << e_m_0*int2eV <<
  //    std::flush; std::cout << std::endl << "E_m_1 " << e_m_1*int2eV <<
  //    std::flush; std::cout << std::endl << "E_m_2 " << e_m_2*int2eV <<
  //    std::flush;
  ...
  // ... 0th kind
  E_Tot = E_Pair_Pair + E_Pair_Sph1 + E_Sph1_Sph1 + E_Pair_Sph2 + E_Sph1_Sph2;

  //    XTP_LOG(Log::info,*_log)
  //             << "... E(X) = " << E_Tot * int2eV << " eV "
  //             << std::flush << "...      = (Site, Site) " << E_Pair_Pair *
  //             int2eV
  //             << std::flush << "...      + (Site, Sph1) " << E_Pair_Sph1 *
  //             int2eV
  //             << std::flush << "...      + (Sph1, Sph1) " << E_Sph1_Sph1 *
  //             int2eV
  //             << std::flush << "...      + (Site, Sph2) " << E_Pair_Sph2 *
  //             int2eV
  //             << std::flush << "...      + (Sph1, Sph2) " << E_Sph1_Sph2 *
  //             int2eV
  //             << std::flush;
  ...
  //             << std::flush << "...      = (PP) "    << epp  * int2eV
  //             << std::flush << "...      + (PU) "    << epu  * int2eV
  //             << std::flush << "...      + (UU) "    << euu  * int2eV
  //             << std::flush;

  // ... 2nd kind
  double E_f_m = e_f_c_c + e_f_c_non_c + e_f_c_out + e_f_non_c_non_c +
                 e_f_non_c_out + e_m_c + e_m_non_c + e_m_out;

  XTP_LOG(Log::info, *_log)
      << (format("PP-PU-UU Splitting")).str() << std::flush
      << (format("  + U [Q  -  Q]       = %1$+1.7f eV") % (epp * int2eV)).str()
      << std::flush
      << (format("  + U [Q  - dQ]       = %1$+1.7f eV") % (epu * int2eV)).str()
      << std::flush
      << (format("  + U [dQ - dQ]       = %1$+1.7f eV") % (euu * int2eV)).str()
      << std::flush << (format("  = ------------------------------")).str()
      << std::flush
      << (format("  + SUM(E)            = %1$+1.7f eV") % (E_PPUU * int2eV))
             .str()
      << std::flush;
  XTP_LOG(Log::info, *_log)
      << (format("QM0-MM1-MM2 Splitting")).str() << std::flush
      << (format("  + Field-term [0-0]  = %1$+1.7f eV (%2$+1.7f)") %
          (e_f_c_c * int2eV) % (e_f_intra_0 * int2eV))
             .str()
      << std::flush
      << (format("  + Field-term [0-1]  = %1$+1.7f eV") %
          (e_f_c_non_c * int2eV))
             .str()
      << std::flush
      << (format("  + Field-term [0-2]  = %1$+1.7f eV") % (e_f_c_out * int2eV))
             .str()
      << std::flush
      << (format("  + Field-term [1-1]  = %1$+1.7f eV (%2$+1.7f)") %
          (e_f_non_c_non_c * int2eV) % (e_f_intra_1 * int2eV))
             .str()
      << std::flush
      << (format("  + Field-term [1-2]  = %1$+1.7f eV") %
          (e_f_non_c_out * int2eV))
             .str()
      << std::flush << (format("    ------------------------------")).str()
      << std::flush
      << (format("  + Work-term  [-0-]  = %1$+1.7f eV") % (e_m_c * int2eV))
             .str()
      << std::flush
      << (format("  + Work-term  [-1-]  = %1$+1.7f eV") % (e_m_non_c * int2eV))
             .str()
      << std::flush
      << (format("  + Work-term  [-2-]  = %1$+1.7f eV") % (e_m_out * int2eV))
             .str()
      << std::flush << (format("  = ------------------------------")).str()
      << std::flush
      << (format("    SUM(E)            = %1$+1.7f eV") % (E_f_m * int2eV))
             .str()
      << std::flush;

  //    XTP_LOG(Log::info,*_log)
  //             << "... E(X) = " << E_f_m * int2eV << " eV "
  //             << std::flush << "...      = (f,0-0) " << e_f_c_c          *
  //             int2eV
  //             << std::flush << "...      + (f,0-1) " << e_f_c_non_c      *
  //             int2eV
  //             << std::flush << "...      + (f,0-2) " << e_f_c_out        *
  //             int2eV
  //             << std::flush << "...      + (f,1-1) " << e_f_non_c_non_c  *
  //             int2eV
  //             << std::flush << "...      + (f,1-2) " << e_f_non_c_out    *
  //             int2eV
  //             << std::flush << "...      + (m,-0-) " << e_m_c            *
  //             int2eV
  //             << std::flush << "...      + (m,-1-) " << e_m_non_c        *
  //             int2eV
  //             << std::flush << "...      + (m,-2-) " << e_m_out          *
  //             int2eV
  //             << std::flush;

  // Forward results to job
  job->setEnergy(E_Tot * int2eV, E_Pair_Pair * int2eV, E_Pair_Sph1 * int2eV,
                 E_Pair_Sph2 * int2eV, E_Sph1_Sph1 * int2eV,
                 E_Sph1_Sph2 * int2eV, e_perm * int2eV, eu_inter * int2eV);

  job->setEnergy_PPUU(epp * int2eV, epu * int2eV, euu * int2eV);

  job->setEnergy_f_m(e_f_c_c * int2eV, e_f_c_non_c * int2eV, e_f_c_out * int2eV,
                     e_f_non_c_non_c * int2eV, e_f_non_c_out * int2eV,
                     e_m_c * int2eV, e_m_non_c * int2eV, e_m_out * int2eV);
...
double XInductor::EnergyStatic(XJob* job) {
  ...
  XTP_LOG(Log::info, *_log)
      << (format("QM0-MM1-MM2 Splitting")).str() << std::flush
      << (format("  + Field-term [0-0]  = %1$+1.7f eV") % (e_f_c_c * int2eV))
             .str()
      << std::flush
      << (format("  + Field-term [0-1]  = %1$+1.7f eV") %
          (e_f_c_non_c * int2eV))
      ...
      << (format("  + Field-term [0-2]  = %1$+1.7f eV") % (e_f_c_out * int2eV))
             .str()
      << std::flush
      << (format("  + Field-term [1-1]  = %1$+1.7f eV") %
...
XInductor::XInductor(Topology* top, tools::Property* opt, std::string sfx,
                     int nst, bool mav)
    : _subthreads(nst), _maverick(mav) {

  std::string key = sfx + ".tholemodel";
  ...
  if (opt->exists(key + ".exp_damp")) {
    ...
    std::cout << std::endl
              << "... ... WARNING: No sharpness parameter supplied";
    std::cout << std::endl << "... ... ... Using default a = 0.39";
    _aDamp = 0.39;
  }

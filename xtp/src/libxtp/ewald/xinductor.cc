...
//    _qmm.insert(_qmm.end(),_mm1.begin(),_mm1.end());
    ...
    if (job->StartFromCPT()) {
        ...
        XTP_LOG(Log::debug,*_log) << "Carry out partial depolarization." << std::flush;
        ...
        XTP_LOG(Log::debug,*_log) << "Carry out full depolarization." << std::flush;        
        std::vector< PolarSeg* >   ::iterator sit;
        std::vector< APolarSite* > ::iterator pit;
...
//    this->InitChunks();
    ...
    XTP_LOG(Log::info,*_log) << (format("  o Total:     %1$1.2f min")
        % (t_ener+t_indu)) << std::flush;
    XTP_LOG(Log::info,*_log) << (format("  o Induction: %1$1.2f min")
        % (t_indu)) << std::flush;
    XTP_LOG(Log::info,*_log) << (format("  o Energy:    %1$1.2f min")
        % (t_ener)) << std::flush;
...
//         }}
    ...
    XTP_LOG(Log::info,*_log) << (format("  PERM      |  T=%1$1.2fs ") 
        % (t_perm));
    ...
    if (true || !job->StartFromCPT()) {
        ...
        XTP_LOG(Log::info,*_log) << "Job started from archive, reusing existing "
    ...
    for (int ridx = 0; ridx < 1; ++ridx) {
        ...
        for ( ; iter_cg < maxI; ++iter_cg) {
            ...
            XTP_LOG(Log::info,*_log) << (boost::format(
                ...
                % eTOL).str();
            XTP_LOG(Log::info,*_log) << (format("  |  T=%1$1.2fs CG/I/O %2$2.2f%% %3$2.2f%% %4$2.2f%%") 
...
//            % iter % maxdU_U % avgdU_U % rmsdU % baseN).str() << std::flush;
//
//        // Break if converged
//        if      (converged) { 
//            _isConverged = true;
...
//    std::cout << std::endl << "E_f_intra_0 " << e_f_intra_0*int2eV << std::flush;
//    std::cout << std::endl << "E_m_intra_0 " << e_m_intra_0*int2eV << std::flush;
//    std::cout << std::endl << "E_f_intra_1 " << e_f_intra_1*int2eV << std::flush;
//    std::cout << std::endl << "E_m_intra_1 " << e_m_intra_1*int2eV << std::flush;
//    std::cout << std::endl << "E_f_intra_2 " << e_f_intra_2*int2eV << std::flush;
//    std::cout << std::endl << "E_m_intra_2 " << e_m_intra_2*int2eV << std::flush;
//    std::cout << std::endl << "E_m_0 " << e_m_0*int2eV << std::flush;
//    std::cout << std::endl << "E_m_1 " << e_m_1*int2eV << std::flush;
//    std::cout << std::endl << "E_m_2 " << e_m_2*int2eV << std::flush;
...
//             << "... E(X) = " << E_Tot * int2eV << " eV "
//             << std::flush << "...      = (Site, Site) " << E_Pair_Pair * int2eV
//             << std::flush << "...      + (Site, Sph1) " << E_Pair_Sph1 * int2eV
//             << std::flush << "...      + (Sph1, Sph1) " << E_Sph1_Sph1 * int2eV
//             << std::flush << "...      + (Site, Sph2) " << E_Pair_Sph2 * int2eV
//             << std::flush << "...      + (Sph1, Sph2) " << E_Sph1_Sph2 * int2eV
//             << std::flush;
...
//             << std::flush;
    ...
    XTP_LOG(Log::info,*_log)
        ...
        << std::flush << (format("  + U [Q  -  Q]       = %1$+1.7f eV") % (epp      * int2eV)).str()
        << std::flush << (format("  + U [Q  - dQ]       = %1$+1.7f eV") % (epu      * int2eV)).str()       
        << std::flush << (format("  + U [dQ - dQ]       = %1$+1.7f eV") % (euu      * int2eV)).str()
        << std::flush << (format("  = ------------------------------")).str()
        << std::flush << (format("  + SUM(E)            = %1$+1.7f eV") % (E_PPUU   * int2eV)).str()
        << std::flush;
    XTP_LOG(Log::info,*_log)
        << (format("QM0-MM1-MM2 Splitting")).str()
        << std::flush << (format("  + Field-term [0-0]  = %1$+1.7f eV (%2$+1.7f)") % (e_f_c_c          * int2eV) % (e_f_intra_0*int2eV)).str()
        << std::flush << (format("  + Field-term [0-1]  = %1$+1.7f eV") % (e_f_c_non_c      * int2eV)).str()       
        << std::flush << (format("  + Field-term [0-2]  = %1$+1.7f eV") % (e_f_c_out        * int2eV)).str()
        << std::flush << (format("  + Field-term [1-1]  = %1$+1.7f eV (%2$+1.7f)") % (e_f_non_c_non_c  * int2eV) % (e_f_intra_1*int2eV)).str()
        << std::flush << (format("  + Field-term [1-2]  = %1$+1.7f eV") % (e_f_non_c_out    * int2eV)).str()
        << std::flush << (format("    ------------------------------")).str()
        << std::flush << (format("  + Work-term  [-0-]  = %1$+1.7f eV") % (e_m_c            * int2eV)).str()
        << std::flush << (format("  + Work-term  [-1-]  = %1$+1.7f eV") % (e_m_non_c        * int2eV)).str()
        << std::flush << (format("  + Work-term  [-2-]  = %1$+1.7f eV") % (e_m_out          * int2eV)).str()
        << std::flush << (format("  = ------------------------------")).str()
        << std::flush << (format("    SUM(E)            = %1$+1.7f eV") % (E_f_m               *int2eV)).str()
        << std::flush;
    
    
//    XTP_LOG(Log::info,*_log)
//             << "... E(X) = " << E_f_m * int2eV << " eV " 
//             << std::flush << "...      = (f,0-0) " << e_f_c_c          * int2eV
//             << std::flush << "...      + (f,0-1) " << e_f_c_non_c      * int2eV
//             << std::flush << "...      + (f,0-2) " << e_f_c_out        * int2eV
//             << std::flush << "...      + (f,1-1) " << e_f_non_c_non_c  * int2eV
//             << std::flush << "...      + (f,1-2) " << e_f_non_c_out    * int2eV
//             << std::flush << "...      + (m,-0-) " << e_m_c            * int2eV
//             << std::flush << "...      + (m,-1-) " << e_m_non_c        * int2eV
//             << std::flush << "...      + (m,-2-) " << e_m_out          * int2eV
//             << std::flush;
...
//             << std::flush;
    ...
    XTP_LOG(Log::info,*_log)
        << (format("QM0-MM1-MM2 Splitting")).str()
        << std::flush << (format("  + Field-term [0-0]  = %1$+1.7f eV") % (e_f_c_c          * int2eV)).str()
        << std::flush << (format("  + Field-term [0-1]  = %1$+1.7f eV") % (e_f_c_non_c      * int2eV)).str()       
        << std::flush << (format("  + Field-term [0-2]  = %1$+1.7f eV") % (e_f_c_out        * int2eV)).str()
        << std::flush << (format("  + Field-term [1-1]  = %1$+1.7f eV") % (e_f_non_c_non_c  * int2eV)).str()
        << std::flush << (format("  + Field-term [1-2]  = %1$+1.7f eV") % (e_f_non_c_out    * int2eV)).str()
        << std::flush << (format("    ------------------------------")).str()
        << std::flush << (format("  + Work-term  [-0-]  = %1$+1.7f eV") % (e_m_c            * int2eV)).str()
        << std::flush << (format("  + Work-term  [-1-]  = %1$+1.7f eV") % (e_m_non_c        * int2eV)).str()
...
XInductor::XInductor(Topology *top, tools::Property *opt, 
                     std::string sfx, int nst, bool mav)
    ...
    std::string key = sfx + ".tholemodel";
        ...
        if ( opt->exists(key+".exp_damp") ) {
            ...
            std::cout << std::endl << "... ... WARNING: No sharpness parameter supplied";

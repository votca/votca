#include "votca/tools/save_load_eigen.h"
...
PolarTop::PolarTop() : _top(NULL), _center(vec(0, 0, 0)) {
  ...
  _polarization_iter = -1;
  _polarization_converged = false;
};
...
void PolarTop::PrintInfo(std::ostream& out) {
  out << std::endl
      << "Shells "
      << "|QM0| = " << _qm0.size() << ", "
      << "|MM1| = " << _mm1.size() << ", "
      ...
      << "|FGC| = " << _fgC.size() << ", "
      << "|FGN| = " << _fgN.size() << ", "
      << "|BGN| = " << _bgN.size() << ". " << std::flush;
}

std::string PolarTop::ShellInfoStr() {
...
void PolarTop::PrintPDB(std::string outfile) {
...
void PolarTop::PrintInduState(std::string outfile, std::string format,
...
void PolarTop::PrintInduState(FILE* out, std::string format, bool split_dpl,
                              double dpl_space) {
...
void PolarTop::SaveToDrive(std::string archfile) {
  // Carve into archive
...
void PolarTop::LoadFromDrive(std::string archfile) {

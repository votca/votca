...
void APolarSite::Rotate(const matrix &rot, const vec &refPos) {
    ...
    for (int state = -1; state < 2; state++) {
        ...
        if (_Qs[state+1].size() > 4) {
            ...
            _Qs[state+1][6] = 2 / sqrt(3) * Q_Global(1,2);  // Q21s
            _Qs[state+1][7] = 1 / sqrt(3) *(Q_Global(0,0)
                                          - Q_Global(1,1)); // Q22c
            _Qs[state+1][8] = 2 / sqrt(3) * Q_Global(0,1);  // Q22s
        }
...
void APolarSite::Charge(int state) {
    ...
    matrix polarity;
    polarity.row(0) = vec(Pxx,Pxy,Pxz);
    polarity.row(1) = vec(Pxy,Pyy,Pyz);
    polarity.row(2) = vec(Pxz,Pyz,Pzz);
    /* = matrix( vec(Pxx,Pxy,Pxz),
                              vec(Pxy,Pyy,Pyz),
                              vec(Pxz,Pyz,Pzz) );*/

    Eigen::SelfAdjointEigenSolver<Eigen::Matrix3d> es(polarity);
    //matrix::eigensystem_t eigensystem_polarity;
    //polarity.SolveEigensystem(eigensystem_polarity);
    pax         = es.eigenvectors().col(0);
    pay         = es.eigenvectors().col(1);
    paz         = es.eigenvectors().col(2);
    eigenpxx    = es.eigenvalues()(0);
    eigenpyy    = es.eigenvalues()(1);
    eigenpzz    = es.eigenvalues()(2);
    eigendamp =  (eigenpxx > eigenpyy) ?
                     ((eigenpxx > eigenpzz) ? eigenpxx : eigenpzz)
                   : ((eigenpyy > eigenpzz) ? eigenpyy : eigenpzz);
    //cout << std::endl; << "eigensystem ...";
    //cout << std::endl; << pax << " --- " << eigenpxx;
    //cout << std::endl; << pay << " --- " << eigenpyy;
    //cout << std::endl; << paz << " --- " << eigenpzz;
...
double APolarSite::getProjP(vec &dir) {
    double alpha_proj = std::abs(dir.dot(pax)) * std::abs(dir.dot(pax)) * eigenpxx
         + std::abs(dir.dot(pay)) * std::abs(dir.dot(pay)) * eigenpyy
         + std::abs(dir.dot(paz)) * std::abs(dir.dot(paz)) * eigenpzz;
...
double APolarSite::HistdU() {
    ...
    double abs_U0 = U0.norm();
    double abs_U1 = U1.norm();
    double abs_dU = dU.norm();
...
void APolarSite::PrintInfo(std::ostream &out) {
    ...
    printf("\n%2s %2d POS %+2.3f %+2.3f %+2.3f "
           "POL %+2.3f %+2.3f %+2.3f %+2.3f %+2.3f %+2.3f ",
            _name.c_str(), _id, _pos(0), _pos(1), _pos(2),
...
void APolarSite::WritePdbLine(FILE *out, const std::string &tag) {
    ...
    fprintf(out, "ATOM  %5d %4s%1s%3s %1s%4d%1s   "
         ...
         _pos(0)*10,        // X in Angstroms               %8.3f
         _pos(1)*10,        // Y in Angstroms               %8.3f
         _pos(2)*10,        // Z in Angstroms               %8.3f
         1.0,                   // Occupancy                    %6.2f
...
void APolarSite::WriteChkLine(FILE *out, vec &shift, bool split_dpl,
                             std::string format, double spacing) {
    ...
    if (split_dpl) {
        ...
        if (_rank == 2) {
            ...
            EIGEN.eigenvalues() = es.eigenvalues();
            EIGEN.eigenvectors() = es.eigenvectors();
        }

        double a        = spacing;
        double mag_d    = tot_dpl.norm();
        vec    dir_d_0  = tot_dpl.normalized();
        vec    dir_d    = dir_d_0.normalized();
        ...
        if (format == "xyz" && _rank == 2) {
            vec D1 = pos + 0.5 * a * EIGEN.eigenvectors().col(0);
            vec D2 = pos - 0.5 * a * EIGEN.eigenvectors().col(0);
            vec E1 = pos + 0.5 * a * EIGEN.eigenvectors().col(1);
            vec E2 = pos - 0.5 * a * EIGEN.eigenvectors().col(1);
            vec F1 = pos + 0.5 * a * EIGEN.eigenvectors().col(2);
            vec F2 = pos - 0.5 * a * EIGEN.eigenvectors().col(2);
...
void APolarSite::WriteMpsLine(std::ostream &out, std::string unit = "angstrom") {
...
void APolarSite::WriteXmlLine(std::ostream &out) {
    ...
    out << _id << std::endl;;
    out << _name << std::endl;;
    out << ((_isVirtual) ? "virtual" : "notvirtual") << std::endl;;
    out << "resolution" << (int)_resolution << std::endl;;
    out << _pos << std::endl;;
    out <<  _locX << " "; out <<  _locY << " "; out <<  _locZ << " "; out << std::endl;;
    for (int state = -1; state < 2; ++state) {
        out << "<state>" << std::endl;;
        out << state << std::endl;;
    ...
    out <<  _rank; out << std::endl;;

    out << "<pol>" << std::endl;;
    for (int state = -1; state < 2; ++state) {
        out << "<state>" << std::endl;;
        out << _Ps[state+1](0,0) << " " << _Ps[state+1](0,1) << " " << _Ps[state+1](0,2) << std::endl;;
        out << _Ps[state+1](1,0) << " " << _Ps[state+1](1,1) << " " << _Ps[state+1](1,2) << std::endl;;
        out << _Ps[state+1](2,0) << " " << _Ps[state+1](2,1) << " " << _Ps[state+1](2,2) << std::endl;;
        out << "</state>" << std::endl;;
    }
    out << "</pol>" << std::endl;;
    
    out << "<config>" << std::endl;;
    out <<  Pxx << " "; out <<  Pxy << " "; out <<  Pxz << " "; out << std::endl;;
    out <<  Pyy << " "; out <<  Pyz << " "; out << std::endl;;
    out <<  Pzz << " "; out << std::endl;;

    out <<  pax << " "; out <<  eigenpxx; out << std::endl;;
    out <<  pay << " "; out <<  eigenpyy; out << std::endl;;
    out <<  paz << " "; out <<  eigenpzz; out << std::endl;;

    out <<  eigendamp; out << std::endl;;

    out <<  Q00 << " "; out << std::endl;;
    out <<  Q1x << " "; out <<  Q1y << " ";  out <<  Q1z << " "; out << std::endl;;
    out <<  Q20 << " "; out <<  Q21c << " "; out <<  Q21s << " "; out <<  Q22c << " "; out <<  Q22s << " "; out << std::endl;;
    out <<  Qxx << " "; out <<  Qxy << " ";  out <<  Qxz << " ";  out <<  Qyy << " ";  out <<  Qyz << " "; out <<  Qzz << " "; out << std::endl;;

    out <<  U1x << " "; out <<  U1y << " "; out <<  U1z << " "; out << std::endl;;
    out <<  FPx << " "; out <<  FPy << " "; out <<  FPz << " "; out << std::endl;;
    out <<  FUx << " "; out <<  FUy << " "; out <<  FUz << " "; out << std::endl;;
    out << "</config>" << std::endl;;
    out << "</aps>" << std::endl;;
    return;
}
...
std::vector<APolarSite*> APS_FROM_MPS(std::string filename, int state, QMThread *thread) {
    ...
    if (useDefaultPs) {
        if (thread == NULL)
        std::cout << std::endl << "... ... ... NOTE Using default Thole polarizabilities "
             << "for charge state " << state << ". ";
        
        std::map<std::string,double> polar_table = POLAR_TABLE();
...
//                                       "for polar site type " + elem + ". "); }
            ...
            P1 = alpha*Eigen::Matrix3d::Identity();
            (*pol)->setPs(P1, state);
        }
    }

...
void APolarSite::Rotate(const matrix &rot, const vec &refPos) {
  ...
  for (int state = -1; state < 2; state++) {
    ...
    if (_Qs[state + 1].size() > 4) {
      ...
      _Qs[state + 1][7] =
          1 / sqrt(3) * (Q_Global(0, 0) - Q_Global(1, 1));  // Q22c
...
double APolarSite::getProjP(vec &dir) {
  double alpha_proj =
      std::abs(dir.dot(pax)) * std::abs(dir.dot(pax)) * eigenpxx +
...
void APolarSite::PrintInfo(std::ostream &out) {
  ...
  printf(
      ...
      _name.c_str(), _id, _pos(0), _pos(1), _pos(2), Pxx * 1000, Pxy * 1000,
      Pxz * 1000, Pyy * 1000, Pyz * 1000, Pzz * 1000);
}
...
void APolarSite::WriteChkLine(FILE *out, vec &shift, bool split_dpl,
  ...
  fprintf(out, "%+4.9f %+4.9f %+4.9f %+4.7f \n", pos(0) * int2ext,
          pos(1) * int2ext, pos(2) * int2ext, Q00);
  ...
  if (split_dpl) {
    ...
    fprintf(out, "%+4.9f %+4.9f %+4.9f %+4.7f \n", A(0) * int2ext,
            A(1) * int2ext, A(2) * int2ext, qA);

    if (format == "xyz") {
      fprintf(out, " B ");
    }
    fprintf(out, "%+4.9f %+4.9f %+4.9f %+4.7f \n", B(0) * int2ext,
            B(1) * int2ext, B(2) * int2ext, qB);
    ...
    if (format == "xyz" && _rank == 2) {
      ...
      fprintf(out, " D %+4.9f %+4.9f %+4.9f \n", D1(0) * int2ext,
              D1(1) * int2ext, D1(2) * int2ext);
      fprintf(out, " D %+4.9f %+4.9f %+4.9f \n", D2(0) * int2ext,
              D2(1) * int2ext, D2(2) * int2ext);
      fprintf(out, " E %+4.9f %+4.9f %+4.9f \n", E1(0) * int2ext,
              E1(1) * int2ext, E1(2) * int2ext);
      fprintf(out, " E %+4.9f %+4.9f %+4.9f \n", E2(0) * int2ext,
              E2(1) * int2ext, E2(2) * int2ext);
      fprintf(out, " F %+4.9f %+4.9f %+4.9f \n", F1(0) * int2ext,
              F1(1) * int2ext, F1(2) * int2ext);
      fprintf(out, " F %+4.9f %+4.9f %+4.9f \n", F2(0) * int2ext,
              F2(1) * int2ext, F2(2) * int2ext);
    }
  }
}

void APolarSite::WriteMpsLine(std::ostream &out,
  ...
  out << (boost::format(" %1$2s %2$+1.7f %3$+1.7f %4$+1.7f Rank %5$d\n") %
          _name % (_pos(0) * conv_pos) % (_pos(1) * conv_pos) %
...
void APolarSite::WriteXmlLine(std::ostream &out) {
  ...
  for (int state = -1; state < 2; ++state) {
    out << "<state>" << std::endl;
    ;
    out << _Ps[state + 1](0, 0) << " " << _Ps[state + 1](0, 1) << " "
        << _Ps[state + 1](0, 2) << std::endl;
    ;
    out << _Ps[state + 1](1, 0) << " " << _Ps[state + 1](1, 1) << " "
        << _Ps[state + 1](1, 2) << std::endl;
    ;
    out << _Ps[state + 1](2, 0) << " " << _Ps[state + 1](2, 1) << " "
        << _Ps[state + 1](2, 2) << std::endl;
    ;
    out << "</state>" << std::endl;
    ;
  }
...
std::vector<APolarSite *> APS_FROM_MPS(std::string filename, int state,
  ...
  if (intt.is_open()) {
    ...
    std::cout << std::endl << "ERROR: No such file " << filename << std::endl;
    ;

...
PolarSeg::PolarSeg(PolarSeg *templ, bool do_depolarize) {
  ...
  for (unsigned int i = 0; i < templ->_pfrags.size(); ++i) {
    ...
    for (PolarFrag::iterator pit = ref_frag->begin(); pit != ref_frag->end();
         ++pit) {
...
PolarFrag *PolarSeg::AddFragment(std::string name) {
...
vec PolarSeg::CalcTotD() {
  vec D = vec(0, 0, 0);
  for (unsigned int i = 0; i < this->size(); ++i) {
    if ((*this)[i]->getRank() > 0) D += (*this)[i]->getQ1();
...
void PolarSeg::PrintPolarNbPDB(std::string outfile) {
  FILE *out;
...
void PolarSeg::WriteMPS(std::string mpsfile, std::string tag) {
  std::ofstream ofs;
  ofs.open(mpsfile.c_str(), std::ofstream::out);
  if (!ofs.is_open()) {
    throw std::runtime_error("Bad file handle: " + mpsfile);
  }
  ...
  ofs << (boost::format(
              "! N=%2$d Q[e]=%1$+1.7f D[e*nm]=%3$+1.7e %4$+1.7e %5$+1.7e\n") %
          CalcTotQ() % size() % D(0) % D(1) % D(2));
...
void PolarSeg::GeneratePermInduCgSite(bool do_cg_polarizabilities) {
  ...
  for (PolarSeg::iterator pit = begin(); pit < end(); ++pit) {
    // PERMANENT MOMENTS
...
void PolarSeg::Coarsegrain(bool cg_anisotropic) {
  ...
  for (std::vector<PolarFrag *>::iterator fit = _pfrags.begin();
    ...
    for (PolarFrag::iterator pit = (*fit)->begin(); pit < (*fit)->end();
         ++pit) {
      // PERMANENT MOMENTS
      // Convert real to complex moments
      std::vector<double> Qlm = (*pit)->getQs(0);
      ...
      //            std::vector<double> Qlm_shifted_vector =
      //            Qlm_shifted.ToVector(); DMA::ComplexSphericalMoments
      //            Xlm_back(Qlm_shifted_vector); DMA::RegularSphericalHarmonics
      //            Clm_back(shift); std::vector<DMA::cmplx> Xlm_shifted_back =
      //            mshift.Shift(Xlm_back, Clm_back); DMA::RealSphericalMoments
      //            Qlm_back(Xlm_shifted_back); cout << endl << "restored";
      //            Qlm_back.PrintReal();
    ...
    assert((u1_cg_sum - u1_cg_red).norm() < 1e-9 &&
           "<PolarSeg::CoarseGrain> INDUCED MOMENTS TRAFO - ERROR");
    // Collapse polarizabilities
    MolPolEngine engine = MolPolEngine();
    matrix PCG = engine.CalculateMolPol(*(*fit), Log::verbose() && false);
    if (cg_anisotropic == false) {
      // Reduce to isotropic tensor

      Eigen::SelfAdjointEigenSolver<Eigen::Matrix3d> es(PCG);

      double p11 = es.eigenvalues()(0);
      double p22 = es.eigenvalues()(1);
      double p33 = es.eigenvalues()(2);
      PCG = pow(p11 * p22 * p33, 1. / 3.) * Eigen::Matrix3d::Identity();
    }

...
PoissonGrid::PoissonGrid(const Topology *top, std::vector<PolarSeg *> &fg,
                         std::vector<PolarSeg *> &bg, Logger *log)
    : _log(log) {
  ...
  XTP_LOG(Log::debug, *_log)
      << "Setup <PoissonGrid> @ " << _center << std::flush;
  ...
  for (int i = 1; true; ++i) {
    ...
    if (xi <= x01) {
      ...
      dx = (xi - x01) / (x1 - x01) * dx1 + (x1 - xi) / (x1 - x01) * dx0;
    } else {
      dx = dx1;
    }
    ...
    std::cout << std::endl << dx << " => " << xi << std::flush;
    if (xi > x1) break;
  }
}

...
Ewald3D3D::Ewald3D3D(const Topology *top, PolarTop *ptop, tools::Property *opt, Logger *log) 
  : Ewald3DnD(top, ptop, opt, log) {}


EWD::triple<> Ewald3D3D::ConvergeReciprocalSpaceSum(std::vector<PolarSeg*> &target) {

    const double int2eV = 1 / (4 * EWD::Pi * 8.854187817e-12) * 1.602176487e-19 / 1.000e-9;
    ...
    double shell_dk = (_A.cwiseAbs().maxCoeff() > _B.cwiseAbs().maxCoeff()) ?
        ((_A.cwiseAbs().maxCoeff() > _C.cwiseAbs().maxCoeff()) ? _A.cwiseAbs().maxCoeff() : _C.cwiseAbs().maxCoeff()) 
      : ((_B.cwiseAbs().maxCoeff() > _C.cwiseAbs().maxCoeff()) ? _B.cwiseAbs().maxCoeff() : _C.cwiseAbs().maxCoeff());
    ...
    for (shellit = shell_ks.begin(); shellit < shell_ks.end(); ++shellit, ++N_shells_proc) {
        ...
        for (kit = (*shellit).begin(); kit < (*shellit).end(); ++kit, ++N_K_proc) {
            ...
            XTP_LOG(Log::debug,*_log)
                ...
                % (dEKK_rms/_LxLyLz*int2eV) % N_EKK_memory) << std::flush;

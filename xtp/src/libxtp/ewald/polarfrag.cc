#include <votca/xtp/ewald/dmaspace.h>
...
const vec &PolarFrag::CalcPosCenterOfGeom() {
...
const vec &PolarFrag::CalcPosPolarWeights() {
  ...
  // Else: return weighted 0.5*(center-of-polarity + center-of-charge)
    ...
    for (unsigned int i = 0; i < this->size(); ++i) {
      ...
      if (sum_abs_q < 1e-2) {
        ...
        weight = 0.5 * ((*this)[i]->getIsoP() / sum_iso_p +
...
void PolarFrag::GeneratePermInduCgSite(
  ...
  for (PolarFrag::iterator pit = begin(); pit < end(); ++pit) {
    // PERMANENT MOMENTS
    ...
    uQlm[1] = u1(2);  // NOTE order is z-x-y == 10-11c-11s
    uQlm[2] = u1(0);
    uQlm[3] = u1(1);
  ...
  // Zero induced dipole
  vec u1_cg_red = Eigen::Vector3d::Zero();

...
PEwald3D3D::PEwald3D3D(const Topology *top, PolarTop *ptop,
                       tools::Property *opt, Logger *log)
    : Ewald3DnD(top, ptop, opt, log) {}

void PEwald3D3D::GenerateKVectors(std::vector<PolarSeg *> &ps1,
  ...
  XTP_LOG(Log::info, *_log)
      << std::flush << "Generating K-vectors (Sx*Sy*Sz grading system in place)"
      << std::flush;
  ...
  XTP_LOG(Log::info, *_log)
      << "  o K-lines through origin: Exploring K resonances" << std::flush;
  ...
  if (pow(avg_kx_s1s2 * avg_ky_s1s2 * avg_kz_s1s2, 2. / 3.) > 1e-100)
    kxyz_s1s2_norm = 1. /
                     pow(avg_kx_s1s2 * avg_ky_s1s2 * avg_kz_s1s2, 2. / 3.) *
                     int2eV / _LxLyLz;
  else {
    XTP_LOG(Log::debug, *_log)
        << "    - Symptoms of a neutral system: Use Ark2Expk2 grading."
        << std::flush;
    kxyz_s1s2_norm = int2eV / _LxLyLz;
    neutral_mode = true;
  }
  ...
  XTP_LOG(Log::info, *_log)
      << "  o K-planes through origin: Applying K resonances" << std::flush;
  ...
  for (ky = -_NB_max; ky < _NB_max + 1; ++ky) {
    ...
    for (kz = -_NC_max; kz < _NC_max + 1; ++kz) {
      ...
      double grade = _ewdactor.Ark2Expk2(k) * kx_s1s2[std::abs(kx)] *
                     ...
                     kxyz_s1s2_norm;
      if (neutral_mode) grade = _ewdactor.Ark2Expk2(k) * int2eV / _LxLyLz;
  ...
  //     cout << endl << std::scientific << kvec.getX() << " " << kvec.getY() <<
  //     " " << kvec.getZ() << " grade " << kvec.getGrade() << std::flush;
  // }

  // ZERO COMPONENTS ZERO, THREE NON-ZERO
  XTP_LOG(Log::info, *_log)
      << "  o K-space (off-axis): Applying K resonances" << std::flush;
...
void PEwald3D3D::ScanCutoff() {
  ...
  XTP_LOG(Log::debug,*_log) << std::flush
      << "Scan cutoff (long-range check)" << std::flush;
  ...
  std::vector< std::vector<PolarSeg*> > ::iterator vsit;
  ...
  */
  ...
  std::string scan_file =
      "scan_cut_" + boost::lexical_cast<std::string>(_polar_qm0[0]->getId()) +
      "_" + _jobType + ".tab";
  ofs.open(scan_file.c_str(), std::ofstream::out);
  ...
  for (sit1 = _fg_C.begin(); sit1 != _fg_C.end(); ++sit1) {
    ...
    std::vector<std::vector<TinyNeighbour *> > shelled_nbs;
    ...
    for (tnit = nbs.begin(); tnit != nbs.end(); ++tnit) {
      ...
      double R = ((*sit1)->getPos() - nb->getPos() - (*tnit)->_L).norm();
    ...
    for (int sidx = 0; sidx < N_shells; ++sidx) {
      ...
      int shell_count = 0;
      // Interact with shell
      std::vector<TinyNeighbour *> &nb_shell = shelled_nbs[sidx];
      ...
      XTP_LOG(Log::debug, *_log)
          << (format("  o ID = %5$-4d Rc = %1$+02.7f   |MGN| = %3$5d   ER = "
                     "%2$+1.7f eV   dER2(sum) = %4$+1.3e eV") %
              shell_R % (sum * int2eV) % nb_shell.size() %
              (shell_rms * shell_count) % (*sit1)->getId())
                 .str()
          << std::flush;
      ofs << (format("MST DBG ...  o ID = %5$-4d Rc = %1$+02.7f   |MGN| = "
                     "%3$5d   ER = %2$+1.7f eV   dER2(sum) = %4$+1.3e eV") %
              shell_R % (sum * int2eV) % nb_shell.size() %
              (shell_rms * shell_count) % (*sit1)->getId())
                 .str()
          << std::endl;
  ...
  //            if ( std::abs(na) != N && std::abs(nb) != N && std::abs(nc) != N
  //            ) continue; vec L = na*_a + nb*_b + nc*_c;
  //
  ...
  //    XTP_LOG(Log::debug,*_log) << ax << " " << by << " " << cz << " " << norm
  //    << std::flush;
...
//            double R = votca::tools::maxnorm(dr);
      ...
      for (int sidx = 0; sidx < N_shells; ++sidx) {
          ...
          XTP_LOG(Log::debug,*_log)
              << (format("  o ID = %5$-4d Rc = %1$+02.7f   |MGN| = %3$5d   ER =
%2$+1.7f eV   dER2(sum) = %4$+1.3e eV") % shell_R % (sum*int2eV) %
shell_mg.size() % (shell_rms*shell_count) % (*sit1)->getId()).str() <<
std::flush; ofs
              << (format("MST DBG ...  o ID = %5$-4d Rc = %1$+02.7f   |MGN| =
%3$5d   ER = %2$+1.7f eV   dER2(sum) = %4$+1.3e eV") % shell_R % (sum*int2eV) %
shell_mg.size() % (shell_rms*shell_count) % (*sit1)->getId()).str() << endl;
      }
  }
...
EWD::triple<> PEwald3D3D::ConvergeRealSpaceSum(
  ...
  XTP_LOG(Log::debug, *_log)
      << std::flush << "R-space energy via midground" << std::flush;
  ...
  if (_did_field_pin_R_shell) {
    ...
    _converged_R = true;
    //        cout << endl << "XINTERACTOR " << _actor.getEPP() << " " <<
    //        _actor.getEPU() << " "<< _actor.getEUU() << std::flush;
  }
        ...
        for (sit2 = shell_mg.begin(); sit2 < shell_mg.end(); ++sit2) {
          for (pit1 = (*sit1)->begin(); pit1 < (*sit1)->end(); ++pit1) {
            for (pit2 = (*sit2)->begin(); pit2 < (*sit2)->end(); ++pit2) {
              ...
              //                            _actor.BiasIndu(*(*pit1), *(*pit2));
              //                            double ef = _actor.E_f(*(*pit1),
              //                            *(*pit2)); sum_pp += ef; shell_term
              //                            = ef; shell_sum += shell_term;
              //                            shell_rms += shell_term*shell_term;
              //                            shell_count += 1;
            }
          }
...
EWD::triple<> PEwald3D3D::ConvergeReciprocalSpaceSum(
  ...
  XTP_LOG(Log::info, *_log)
      << (format("  :: RE %1$+1.7e IM %2$+1.7e") % (sum_re / _LxLyLz * int2eV) %
          (sum_im / _LxLyLz * int2eV))
             .str()
      << std::flush;
  ...
  while (!converged12 && kvit < _kvecs_1_0.end()) {
    ...
    while (kvit < _kvecs_1_0.end()) {
      ...
      EWD::triple<EWD::cmplx> ppuu =
          _ewdactor.AS1S2(kvec.getK(), target, _bg_P);
      ...
      //    << " grade " << kvec.getGrade() << " re " <<
      //    (as1s2._re/_LxLyLz*_ewdactor.int2eV) << std::flush;
      ++kvit;
      ++shell_count;
    }
    ...
    if (shell_count > 0) {
      XTP_LOG(Log::debug, *_log)
          << (format("  o M = %1$04d   G = %2$+1.3e   dE(rms) = %3$+1.3e eV") %
              shell_count % crit_grade % (de_this_shell / _LxLyLz * int2eV))
                 .str()
          << std::flush;
    }

    if (shell_count > 10 && de_this_shell / _LxLyLz * int2eV < _crit_dE) {
      XTP_LOG(Log::info, *_log)
          << (format("  :: RE %1$+1.7e IM %2$+1.7e") %
              (sum_re / _LxLyLz * int2eV) % (sum_im / _LxLyLz * int2eV))
                 .str()
          << std::flush;
      converged12 = true;
    }
  ...
  return EWD::triple<>(sum_re_pp / _LxLyLz, sum_re_pu / _LxLyLz,
                       sum_re_uu / _LxLyLz);
}
...
EWD::triple<> PEwald3D3D::CalculateShapeCorrection(
  ...
  XTP_LOG(Log::debug, *_log)
      << std::flush << "Energy correction terms" << std::flush;
  XTP_LOG(Log::debug, *_log) << "  o Shape-correction to energy, using '"
                             << _shape << "'" << std::flush;

  EWD::triple<double> ppuu =
      _ewdactor.U12_ShapeTerm(target, _bg_P, _shape, _LxLyLz, _log);
  ...
  //              for (pit1 = (*sit1)->begin(); pit1 < (*sit1)->end(); ++pit1) {
  //                 for (pit2 = (*sit2)->begin(); pit2 < (*sit2)->end();
  //                 ++pit2) {
  ...
  //        //
  //        >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
  //        EWD::triple<double> ppuu_fast =
  //        _ewdactor.U12_XYSlab_Factorized(_fg_C, _bg_P); double sum_pp_fast =
  //        ppuu_fast._pp; double sum_pu_fast = ppuu_fast._pu; double
  //        sum_uu_fast = ppuu_fast._uu; sum_pp_fast *= -4*M_PI/_LxLyLz;
  //        sum_pu_fast *= -4*M_PI/_LxLyLz;
  //        sum_uu_fast *= -4*M_PI/_LxLyLz;
  //        cout << endl << "Slow: " << sum_pp << " " << sum_pu << " " << sum_uu
  //        << std::flush; cout << endl << "Fast: " << sum_pp_fast << " " <<
  //        sum_pu_fast << " " << sum_uu_fast << std::flush;
  //
  //        EWD::triple<double> ppuu_cube =
  //        _ewdactor.U12_ShapeTerm_Factorized(_fg_C, _bg_P, _shape, _LxLyLz);
  //        double sum_pp_cube = ppuu_cube._pp;
  //        double sum_pu_cube = ppuu_cube._pu;
  //        double sum_uu_cube = ppuu_cube._uu;
  //        cout << endl << "Cube: " << sum_pp_cube << " " << sum_pu_cube << " "
  //        << sum_uu_cube << std::flush;
  //        //
  //        >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
  //    }
  //    else {
...
EWD::triple<> PEwald3D3D::CalculateForegroundCorrection(
  ...
  XTP_LOG(Log::debug, *_log)
      << "  o Foreground-correction to energy via FGN" << std::flush;
...
void PEwald3D3D::Field_ConvergeRealSpaceSum() {
  ...
  XTP_LOG(Log::debug, *_log)
      << std::flush << "R-space fields via midground" << std::flush;
  ...
  for (sit1 = _fg_C.begin(); sit1 != _fg_C.end(); ++sit1) {
    ...
    for (int sidx = 0; sidx < N_shells; ++sidx) {
      ...
      shell_rms = sqrt(shell_rms / shell_count) * int2V_m;
      ...
      if (Log::verbose()) {
        XTP_LOG(Log::debug, *_log)
            << (format("  o ID = %5$-4d Rc = %1$+02.7f   |MGN| = %2$5d   "
                       "dF(rms) = %3$+1.3e V/m   [1eA => %4$+1.3e eV]") %
                shell_R % shell_mg.size() % shell_rms % e_measure %
                ((*sit1)->getId()))
                   .str()
            << std::flush;
      }
  ...
  if (field_converged_count == _fg_C.size()) {
    XTP_LOG(Log::debug, *_log)
        << (format(":::: Converged to precision (%1$d items)") %
            field_converged_count)
        << std::flush;
    _field_converged_R = true;
  } else if (field_converged_count < _fg_C.size()) {
    XTP_LOG(Log::error, *_log)
        << "ERROR Field not converged on "
        << _fg_C.size() - field_converged_count << " counts." << std::flush;
    _field_converged_R = false;
  } else {
    assert(false);
  }
  ...
  //    _field_converged_R = false;
  //    XTP_LOG(Log::debug,*_log) << std::flush
  //        << "R-space fields via midground" << std::flush;
  //
  //    std::vector<PolarSeg*>::iterator sit1;
  //
  //    // FOR EACH FOREGROUND SEGMENT (FGC) ...
  ...
  //        cout << endl << "sit1 = ID " << (*sit1)->getId() << std::flush;
  //
  //
  //    }
  //    _did_field_pin_R_shell = true;
...
void PEwald3D3D::Field_ConvergeReciprocalSpaceSum() {
  ...
  for (kvit = _kvecs_2_0.begin(); kvit < _kvecs_2_0.end(); ++kvit) {
    EWD::KVector kvec = *kvit;
    EWD::cmplx f_as1s2 =
        _ewdactor.FPU12_AS1S2_At_By(kvec.getK(), _fg_C, _bg_P, rV);
    sum_re += sqrt(f_as1s2._re);
    sum_im += f_as1s2._im;
  }

  XTP_LOG(Log::info, *_log) << (format("  :: RE %1$+1.7e IM %2$+1.7e") %
                                (sum_re * int2V_m) % (sum_im * int2V_m))
                                   .str()
                            << std::flush;
  ...
  while (!converged12 && kvit < _kvecs_1_0.end()) {
    ...
    shell_rms = (rms_count > 0) ? sqrt(shell_rms / rms_count) * int2V_m : 0.0;
    double e_measure = shell_rms * 1e-10 * rms_count;

    if (rms_count > 0) {
      XTP_LOG(Log::debug, *_log)
          << (format("  o M = %1$04d   G = %2$+1.3e   dF(rms) = %3$+1.3e V/m   "
                     "[1eA => %4$+1.3e eV]") %
...
void PEwald3D3D::Field_CalculateForegroundCorrection() {
  ...
  XTP_LOG(Log::debug, *_log)
      << "  o Foreground-correction to fields via FGN" << std::flush;
...
void PEwald3D3D::Field_CalculateShapeCorrection() {
  ...
  XTP_LOG(Log::debug, *_log)
      << std::flush << "Field correction terms" << std::flush;
  XTP_LOG(Log::debug, *_log) << "  o Shape-correction to fields, using '"
                             << _shape << "'" << std::flush;
  ...
  ////              for (pit1 = (*sit1)->begin(); pit1 < (*sit1)->end(); ++pit1)
  ///{ /                 for (pit2 = (*sit2)->begin(); pit2 < (*sit2)->end();
  ///++pit2) { /                    rms += _ewdactor.F12_XYSlab_At_By(*(*pit1),
  ///*(*pit2), TwoPi_V); /                    rms_count += 1; / } / } / } / }
  //
...
// ======= MOMENTS TO TRANSFER TO QM ==========
void PEwald3D3D::Moments_ConvergeRealSpaceSum(
    std::vector<PolarSeg *> &target, ewaldcontainer::PotentialData &data) {
  double sum = 0.0;
  [[maybe_unused]] double sum_phi = 0.0;
  _potential_converged_R = false;

  data.setEta(_ewdactor.getAlpha());

  XTP_LOG(Log::debug, *_log)
      << std::flush << "R-space moments via midground" << std::flush;
  ...
  if (target.size() > 1) {
    throw std::runtime_error(
        "More than one segment not supported for moment transfer to QM in "
        "Moments_ConvergeRealSpaceSum.");
  }
    ...
    for (sit1 = target.begin(); sit1 < target.end(); ++sit1) {
      ...
      for (nit = nbs.begin(); nit != nbs.end(); ++nit) {
        ...
        for (pit2 = nb->begin(); pit2 < nb->end(); ++pit2) {
          ...
          data.addCharge((*pit2)->getQ00(), (*pit2)->getPos());
          // add permanent and induced dipoles, maybe needs checks! UNITS!!!!!
          data.addDipole((*pit2)->getQ1() + (*pit2)->getU1(),
                         (*pit2)->getPos());
        }
      }
      if (Log::verbose()) {
  ...
  // ENERGY - REGENERATE NEIGHBOURS ?
    ...
    // FOR EACH target SEGMENT in QM0
    unsigned int energy_converged_count = 0;
    for (sit1 = target.begin(); sit1 != target.end(); ++sit1) {
      (*sit1)->ClearPolarNbs();
      ...
      for (int sidx = 0; sidx < N_shells; ++sidx) {
        ...
        for (sit2 = shell_mg.begin(); sit2 < shell_mg.end(); ++sit2) {
          for (pit2 = (*sit2)->begin(); pit2 < (*sit2)->end(); ++pit2) {
            ...
            for (pit1 = (*sit1)->begin(); pit1 < (*sit1)->end(); ++pit1) {
              double phi = _ewdactor.PhiPU12_ERFC_At_By(*(*pit1), *(*pit2));
              // std::cout << "check at first segment " << phi << std::endl;
              sum_phi += phi;
              shell_term = phi;
              shell_sum += shell_term;
              shell_rms += shell_term * shell_term;
...
void PEwald3D3D::Moments_ConvergeReciprocalSpaceSum(
    std::vector<PolarSeg *> &target, ewaldcontainer::PotentialData &data) {
  ...
  for (kvit = _kvecs_2_0.begin(); kvit < _kvecs_2_0.end(); ++kvit) {
    EWD::KVector kvec = *kvit;
    EWD::cmplx f_as1s2 =
        _ewdactor.PhiPU12_AS1S2_At_By(kvec.getK(), data, target, _bg_P, rV);
    sum_re += sqrt(f_as1s2._re);
    sum_im += f_as1s2._im;
  }
...
void PEwald3D3D::Moments_CalculateForegroundCorrection(
    std::vector<PolarSeg *> &target, ewaldcontainer::PotentialData &data) {
  ...
  for (sit1 = target.begin(); sit1 < target.end(); ++sit1) {
    for (sit2 = _fg_N.begin(); sit2 < _fg_N.end(); ++sit2) {
      for (pit2 = (*sit2)->begin(); pit2 < (*sit2)->end(); ++pit2) {
        ...
        data.addCharge((*pit2)->getQ00(), (*pit2)->getPos());
        // add permanent and induced dipoles, maybe needs checks! UNITS!!!!!
        data.addDipole((*pit2)->getQ1() + (*pit2)->getU1(), (*pit2)->getPos());
        for (pit1 = (*sit1)->begin(); pit1 < (*sit1)->end(); ++pit1) {
          rms += _ewdactor.PhiPU12_ERF_At_By(*(*pit1), *(*pit2));
          rms_count += 1;
        }
...
void PEwald3D3D::Moments_CalculateShapeCorrection(
    std::vector<PolarSeg *> &target, ewaldcontainer::PotentialData &data) {
XTP_LOG(Log::debug, *_log)
      << std::flush << "Potential correction terms" << std::flush;
  XTP_LOG(Log::debug, *_log) << "  o Shape-correction to potentials, using '"
  ...
  _ewdactor.PhiPU12_ShapeField_At_By(data, _bg_P, _shape, _LxLyLz);
  return;
}
...
void PEwald3D3D::Potential_CalculateForegroundCorrection_Grid(
  ...
  for (Index i = 0; i < grid.getBoxesSize(); ++i) {
    ...
    for (Index p = 0; p < box.size(); p++) {
      for (sit2 = _fg_N.begin(); sit2 < _fg_N.end(); ++sit2) {
        for (pit2 = (*sit2)->begin(); pit2 < (*sit2)->end(); ++pit2) {
          double phi = _ewdactor.PhiPU12_ERF_At_By(
              points[p] * tools::conv::bohr2nm, *(*pit2));
          rms -= phi;

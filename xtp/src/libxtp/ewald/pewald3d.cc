...
PEwald3D3D::PEwald3D3D(const Topology *top, PolarTop *ptop, tools::Property *opt, Logger *log) 
  : Ewald3DnD(top, ptop, opt, log) {}


void PEwald3D3D::GenerateKVectors(std::vector<PolarSeg*> &ps1, std::vector<PolarSeg*> &ps2) {
    ...
    XTP_LOG(Log::info,*_log) << std::flush
        << "Generating K-vectors (Sx*Sy*Sz grading system in place)" << std::flush;
    ...
    if (pow(avg_kx_s1s2*avg_ky_s1s2*avg_kz_s1s2,2./3.) > 1e-100)
        kxyz_s1s2_norm = 1./pow(avg_kx_s1s2*avg_ky_s1s2*avg_kz_s1s2,2./3.) * int2eV / _LxLyLz;
    else {
        XTP_LOG(Log::debug,*_log)
            << "    - Symptoms of a neutral system: Use Ark2Expk2 grading." << std::flush;
    ...
    //    cout << endl << std::scientific << kvec.getX() << " " << kvec.getY() << " " << kvec.getZ() << " grade " << kvec.getGrade() << std::flush;
    //}
...
void PEwald3D3D::ScanCutoff() {
    ...
    */
    ...
    std::ofstream ofs;
    std::string scan_file = "scan_cut_"
       ...
       //                 shell_rms += shell_term*shell_term;
        //                shell_count += 1;
            ...
            XTP_LOG(Log::debug,*_log)
                ...
                % shell_R % (sum*int2eV) % nb_shell.size() % (shell_rms*shell_count) % (*sit1)->getId()).str() << std::flush;
            ofs
                << (format("MST DBG ...  o ID = %5$-4d Rc = %1$+02.7f   |MGN| = %3$5d   ER = %2$+1.7f eV   dER2(sum) = %4$+1.3e eV") 
                % shell_R % (sum*int2eV) % nb_shell.size() % (shell_rms*shell_count) % (*sit1)->getId()).str() << std::endl;
        }
    }
...
//    XTP_LOG(Log::debug,*_log) << ax << " " << by << " " << cz << " " << norm << std::flush;
...
//            double R = votca::tools::maxnorm(dr);
        ...
        for (int sidx = 0; sidx < N_shells; ++sidx) {
            ...
            XTP_LOG(Log::debug,*_log)
                << (format("  o ID = %5$-4d Rc = %1$+02.7f   |MGN| = %3$5d   ER = %2$+1.7f eV   dER2(sum) = %4$+1.3e eV") 
                % shell_R % (sum*int2eV) % shell_mg.size() % (shell_rms*shell_count) % (*sit1)->getId()).str() << std::flush;
            ofs
                << (format("MST DBG ...  o ID = %5$-4d Rc = %1$+02.7f   |MGN| = %3$5d   ER = %2$+1.7f eV   dER2(sum) = %4$+1.3e eV") 
                % shell_R % (sum*int2eV) % shell_mg.size() % (shell_rms*shell_count) % (*sit1)->getId()).str() << endl;
        }
    }
...
EWD::triple<> PEwald3D3D::ConvergeRealSpaceSum(std::vector<PolarSeg*> &target) {
    ...
    XTP_LOG(Log::debug,*_log) << std::flush 
        << "R-space energy via midground" << std::flush;
...
//                        sum_pp += ef;
            ...
            if (Log::verbose()){ XTP_LOG(Log::debug,*_log)
                << (format("  o Id = %5$-4d Rc = %1$+02.7f   |MGN| = %2$5d   dF(rms) = %3$+1.3e V/m   [1eA => %4$+1.3e eV]") 
                % -1.0 % nbs.size() % -1.0  % -1.0 % ((*sit1)->getId())).str() << std::flush;
	    }
        }
        _converged_R = true;
//        cout << endl << "XINTERACTOR " << _actor.getEPP() << " " << _actor.getEPU() << " "<< _actor.getEUU() << std::flush;
    }
...
//        cout << endl << "XINTERACTOR " << _actor.getEPP() << " " << _actor.getEPU() << " "<< _actor.getEUU() << std::flush;
        ...
        if (energy_converged_count == target.size()) {
            XTP_LOG(Log::debug,*_log)  
                ...
                % energy_converged_count) << std::flush;
            _converged_R = true;
        }
        else if (energy_converged_count < target.size()) {
            XTP_LOG(Log::error,*_log) << "ERROR Energy not converged on " 
...
EWD::triple<> PEwald3D3D::ConvergeReciprocalSpaceSum(std::vector<PolarSeg*> &target) {
    ...
    // TWO COMPONENTS ZERO, ONE NON-ZERO
    XTP_LOG(Log::info,*_log) << std::flush 
        << "K-lines through origin: Checking K resonances" << std::flush;
    ...
    XTP_LOG(Log::info,*_log)
        << (format("  :: RE %1$+1.7e IM %2$+1.7e")
            % (sum_re/_LxLyLz*int2eV)
            % (sum_im/_LxLyLz*int2eV)).str() << std::flush;
    ...
    while (!converged12 && kvit < _kvecs_1_0.end()) {
        ...
        while (kvit < _kvecs_1_0.end()) {
            ...
            //   << " grade " << kvec.getGrade() << " re " << (as1s2._re/_LxLyLz*_ewdactor.int2eV) << std::flush;            
            ++kvit;
            ++shell_count;
        }
        de_this_shell = (de_this_shell < 0.) ? -de_this_shell : de_this_shell;
        
        if (shell_count > 0){ XTP_LOG(Log::debug,*_log)
             << (format("  o M = %1$04d   G = %2$+1.3e   dE(rms) = %3$+1.3e eV")
             % shell_count
             % crit_grade
             % (de_this_shell/_LxLyLz*int2eV)).str() << std::flush;
	}
...
EWD::triple<> PEwald3D3D::CalculateShapeCorrection(std::vector<PolarSeg*> &target) {
    
    XTP_LOG(Log::debug,*_log) << std::flush
        << "Energy correction terms" << std::flush;
    XTP_LOG(Log::debug,*_log)
        << "  o Shape-correction to energy, using '" << _shape << "'" << std::flush;
...
//        cout << endl << "Slow: " << sum_pp << " " << sum_pu << " " << sum_uu << std::flush;
//        cout << endl << "Fast: " << sum_pp_fast << " " << sum_pu_fast << " " << sum_uu_fast << std::flush;
//        
...
//        cout << endl << "Cube: " << sum_pp_cube << " " << sum_pu_cube << " " << sum_uu_cube << std::flush;
//        // >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
//    }
//    else {
...
EWD::triple<> PEwald3D3D::CalculateForegroundCorrection(std::vector<PolarSeg*> &target) {
...
//                        shell_count += 1;
            ...
            if (Log::verbose()){ XTP_LOG(Log::debug,*_log)
                << (format("  o ID = %5$-4d Rc = %1$+02.7f   |MGN| = %2$5d   dF(rms) = %3$+1.3e V/m   [1eA => %4$+1.3e eV]") 
                % shell_R % shell_mg.size() % shell_rms  % e_measure % ((*sit1)->getId())).str() << std::flush;
	    }
    ...
    if (field_converged_count == _fg_C.size()) {
        XTP_LOG(Log::debug,*_log)  
            << (format(":::: Converged to precision (%1$d items)") 
            % field_converged_count) << std::flush;
        _field_converged_R = true;
    }
    else if (field_converged_count < _fg_C.size()) {
        XTP_LOG(Log::error,*_log) << "ERROR Field not converged on " 
...
void PEwald3D3D::Field_ConvergeReciprocalSpaceSum() {
    ...
    while (!converged12 && kvit < _kvecs_1_0.end()) {
        ...
        if (rms_count > 0){ XTP_LOG(Log::debug,*_log)
             << (format("  o M = %1$04d   G = %2$+1.3e   dF(rms) = %3$+1.3e V/m   [1eA => %4$+1.3e eV]")
             % rms_count
             % crit_grade
             % shell_rms
             % e_measure).str() << std::flush;
	}
...
void PEwald3D3D::Field_CalculateShapeCorrection() {
    ...
    XTP_LOG(Log::debug,*_log) << std::flush
        << "Field correction terms" << std::flush;
    XTP_LOG(Log::debug,*_log)
        << "  o Shape-correction to fields, using '" << _shape << "'" << std::flush;
...
void PEwald3D3D::Potential_ConvergeRealSpaceSum(std::vector<PolarSeg*> &target) {
    ...
    XTP_LOG(Log::debug,*_log) << std::flush 
        << "R-space potentials via midground" << std::flush;
...
void PEwald3D3D::Potential_ConvergeReciprocalSpaceSum(std::vector<PolarSeg*> &target) {
...
void PEwald3D3D::Potential_CalculateForegroundCorrection(std::vector<PolarSeg*> &target) {
    XTP_LOG(Log::debug,*_log)
...
void PEwald3D3D::Potential_CalculateShapeCorrection(std::vector<PolarSeg*> &target) {
    XTP_LOG(Log::debug,*_log) << std::flush
        << "Potential correction terms" << std::flush;
    XTP_LOG(Log::debug,*_log)
        << "  o Shape-correction to potentials, using '" << _shape << "'" << std::flush;

...
Ewald3DnD::Ewald3DnD(const Topology *top, PolarTop *ptop, tools::Property *opt,
                     Logger *log)
  ...
  if (opt->exists(pfx + ".multipoles.polar_bg")) {
    std::string ptop_file =
        opt->get(pfx + ".multipoles.polar_bg").as<std::string>();
    if (ptop_file != "")
      _started_from_archived_indu_state = true;
    else
  ...
  // Ewald parameters
  std::string cmethod =
      opt->get(pfx + ".coulombmethod.method").as<std::string>();
  ...
  if (opt->exists(pfx + ".coarsegrain.cg_radius")) {
    _coarse_cg_radius = opt->get(pfx + ".coarsegrain.cg_radius").as<double>();
  } else
  ...
  if (opt->exists(pfx + ".tasks.evaluate_energy")) {
    _task_evaluate_energy = opt->get(pfx + ".tasks.evaluate_energy").as<bool>();
  } else
    _task_evaluate_energy = false;
  if (opt->exists(pfx + ".tasks.apply_radial")) {
    _task_apply_radial = opt->get(pfx + ".tasks.apply_radial").as<bool>();
  } else
    _task_apply_radial = false;
  if (opt->exists(pfx + ".tasks.solve_poisson")) {
    _task_solve_poisson = opt->get(pfx + ".tasks.solve_poisson").as<bool>();
  } else
    _task_solve_poisson = false;
  if (opt->exists(pfx + ".tasks.scan_cutoff")) {
    _task_scan_cutoff = opt->get(pfx + ".tasks.scan_cutoff").as<bool>();
  } else
  ...
  this->CoarseGrainDensities(_coarse_do_cg_background, _coarse_do_cg_foreground,
                             _coarse_cg_radius);
  ...
  XTP_LOG(Log::info, *_log) << "Generate periodic images. ";
  ...
  if (iQ1 == +1)
    ...
    _jobType = "electron-like";
  else if (iQ1 == 0 && iQ2 == 0)
    _jobType = "neutral";
  else if (iQ1 == 0 && iQ2 > 0)
    _jobType = "charge-transfer-like";
  else
    _jobType = "bipolaron-like";

  XTP_LOG(Log::info, *_log)
      << (format("Net ground charge and size:")).str() << std::flush
      << (format("  o Q(FGC) = %1$+1.3fe |FGC| = %2$+5d") % Q_fg_C %
          _fg_C.size())
             .str()
      << std::flush
      << (format("  o Q(FGN) = %1$+1.3fe |FGN| = %2$+5d") % Q_fg_N %
          _fg_N.size())
             .str()
      << std::flush
      << (format("  o Q(MGN) = %1$+1.3fe |MGN| ~ %2$+5d") % Q_mg_N %
          _mg_N.size())
             .str()
      << std::flush
      << (format("  o Q(BGN) = %1$+1.3fe |BGN| = %2$+5d") % Q_bg_N %
          _bg_N.size())
             .str()
      << std::flush
      << (format("  o Q(BGP) = %1$+1.3fe |BGP| = %2$+5d") % Q_bg_P %
          _bg_P.size())
             .str()
      << std::flush
      << (format("  o Job type '%3$s' (iQ1=%1$d, iQ2=%2$d)") % iQ1 % iQ2 %
          _jobType)
             .str()
      << std::flush;
  ...
  for (sit = _fg_C.begin(); sit < _fg_C.end(); ++sit) {
    ...
    for (pit = pseg->begin(); pit < pseg->end(); ++pit) {
      ...
      if ((*pit)->getRank() > 0) netdpl_fgC += (*pit)->getQ1();
    }
  }
  ...
  XTP_LOG(Log::info, *_log)
      << std::flush
      << (format("  o D(FGC) [e*nm]           = %1$+1.3f %2$+1.3f %3$+1.3f  ") %
          netdpl_fgC(0) % netdpl_fgC(1) % netdpl_fgC(2))
             .str();
  XTP_LOG(Log::info, *_log)
      << std::flush
...
void Ewald3DnD::ExpandForegroundReduceBackground(double polar_R_co) {
  ...
  XTP_LOG(Log::debug, *_log)
      << "  o Max. int. distance (QM0): " << _max_int_dist_qm0 << "nm"
      << std::flush;
    ...
    for (int na = -polar_na_max; na < polar_na_max + 1; ++na) {
      for (int nb = -polar_nb_max; nb < polar_nb_max + 1; ++nb) {
        for (int nc = -polar_nc_max; nc < polar_nc_max + 1; ++nc) {
          ...
          for (sit2 = _fg_C.begin(), sit3 = _fg_N.begin(); sit2 < _fg_C.end();
               ++sit2, ++sit3) {
            ...
            double dR_L = (seg_bg->getPos() + L - (*sit3)->getPos()).norm();
...
void Ewald3DnD::CoarseGrainDensities(bool cg_bg, bool cg_fg, double cg_radius) {
  ...
  XTP_LOG(Log::debug, *_log) << "Coarse-graining agenda" << std::flush;
  XTP_LOG(Log::debug, *_log)
      << "  o Coarse-grain background:   "
      << ((_coarse_do_cg_background) ? "yes" : "no") << std::flush;
  XTP_LOG(Log::debug, *_log)
      << "  o Coarse-grain foreground:   "
      << ((_coarse_do_cg_foreground) ? "yes" : "no") << std::flush;
  XTP_LOG(Log::debug, *_log)
      << "  o Anisotropic P-tensor:      "
      << ((_coarse_cg_anisotropic) ? "yes" : "no") << std::flush;
  XTP_LOG(Log::debug, *_log)
      << "  o Coarse-graining radius:    " << _coarse_cg_radius << " nm"
      << std::flush;

  // COARSE-GRAIN BACKGROUND
  if (cg_bg) {
    XTP_LOG(Log::debug, *_log) << "Coarse-grain background" << std::flush;
    ...
    for (std::vector<PolarSeg *>::iterator sit = _bg_P.begin();
      ...
      if ((*sit)->size() > (*sit)->PolarFrags().size()) {
        // std::cout << "\rMST DBG ...   o BGP ID = " << (*sit)->getId()
        //      << "   " << std::flush;
    ...
    for (std::vector<PolarSeg *>::iterator sit = _fg_N.begin();
      ...
      if ((*sit)->size() > (*sit)->PolarFrags().size()) {
        // std::cout << "\rMST DBG ...   o FGN ID = " << (*sit)->getId()
        //      << "   " << std::flush;
        (*sit)->Coarsegrain(_coarse_cg_anisotropic);
        ++count_fgn;
      }
    ...
    XTP_LOG(Log::debug, *_log)
        << "  o Coarse-grained " << count_bgp << "/" << _bg_P.size()
        << " (BGP) " << count_fgn << "/" << _fg_N.size() << " (FGN) "
        << "with " << count_bgp_id_in_fg << " FG-table counts" << std::flush;
  }
  // COARSE-GRAIN FOREGROUND
  if (cg_fg) {
    XTP_LOG(Log::debug, *_log) << "Coarse-grain foreground" << std::flush;
    ...
    for (std::vector<PolarSeg *>::iterator sit = _fg_C.begin();
      ...
      for (std::vector<PolarSeg *>::iterator qit = _polar_qm0.begin();
        ...
        double R = ((*sit)->getPos() - (*qit)->getPos()).norm();
        if (R < min_R) min_R = R;
      }

      if (!(min_R <= _polar_cutoff + 1e-3)) {
        std::cout << std::endl << "ASSERTION IMMINENT" << std::endl;
      }
      ...
      if (min_R > cg_radius) {
        // std::cout << "\rMST DBG ...   o FGC ID = " << (*sit)->getId()
        //      << "   " << std::flush;
        (*sit)->Coarsegrain(_coarse_cg_anisotropic);
        ++count_fgc;
      }
    ...
    XTP_LOG(Log::debug, *_log) << "  o Coarse-grained " << count_fgc << "/"
                               << _fg_C.size() << " (FGC) " << std::flush;
  }
  return;
}
        ...
        for (int nc = -_nc_max; nc < _nc_max + 1; ++nc) {
          ...
          if (!is_in_fg) {
            ...
            for (sit2 = _fg_N.begin(); sit2 < _fg_N.end(); ++sit2) {
              ...
              double dR_L = ((*sit2)->getPos() - pos_L).norm();
              if (dR_L <= R_co) {
...
void Ewald3DnD::WriteDensitiesPDB(std::string pdbfile) {
...
void Ewald3DnD::WriteDensitiesPtop(std::string fg, std::string bg) {
  ...
  //
  //    std::string fg_pdb = fg + ".pdb";
  //    std::string mg_pdb = mg + ".pdb";
  //    std::string bg_pdb = bg + ".pdb";
  //    std::vector<PolarSeg*>::iterator sit;
  //    std::vector<APolarSite*> ::iterator pit;
  //    FILE *out;
  ...
  std::ofstream ofs;
  ofs.open((fg + bg).c_str(), std::ofstream::out);
  ofs << "<ptop>\n";
  ...
  for (sit = _fg_C.begin(); sit < _fg_C.end(); ++sit) {
    ...
    ofs << (format("\t\t<name>%1$s</name>\n") %
            _top->getSegment((*sit)->getId()).getType());
...
void Ewald3DnD::WriteInductionStateTable() {
  ...
  if (false && Log::verbose()) {
    std::string tabfile =
        "polarized_" +
        boost::lexical_cast<std::string>(_polar_qm0[0]->getId()) + "_" +
        _jobType + ".tab";
    std::ofstream ofs;
    ...
    for (sit1 = _fg_C.begin(), sit2 = _fg_N.begin(); sit1 < _fg_C.end();
      ...
      for (pit1 = pseg_h->begin(), pit2 = pseg_n->begin(); pit1 < pseg_h->end();
        ...
        double frac_prj_dfp = dfp.dot(dr) / dR / dfp.norm();
        double frac_prj_dfu = dfu.dot(dr) / dR / dfu.norm();
        double frac_prj_dft = dft.dot(dr) / dR / dft.norm();

        ofs << (format("dR2 %1$+1.7f fput456 %2$+1.7e %3$+1.7e %4$+1.7e "
                       "fprjput8910 %5$+1.7e %6$+1.7e %7$+1.7e\n") %
                dR % ((dfp).norm() * int2V_m) % ((dfu).norm() * int2V_m) %
                ((dft).norm() * int2V_m) % frac_prj_dfp % frac_prj_dfu %
                frac_prj_dft);
      }
    }
    ofs.close();
  ...
  if (true && Log::verbose()) {
    std::ofstream ofs;
    std::string tabfile =
        this->IdentifyMethod() + ".indu_state_" +
        boost::lexical_cast<std::string>(_polar_qm0[0]->getId()) + "_" +
        _jobType + ".tab";
    ofs.open(tabfile.c_str(), std::ofstream::out);
  ...
  if (false && Log::verbose()) {
      ...
      for (int na = -_na_max; na < _na_max + 1; ++na) {
        for (int nb = -_nb_max; nb < _nb_max + 1; ++nb) {
          for (int nc = -_nc_max; nc < _nc_max + 1; ++nc) {
            ...
            // In foreground ?
            bool is_in_fg =
                _fg_table->IsInForeground(pseg->getId(), na, nb, nc);
            if (!is_in_fg) {
              for (PolarSeg::iterator pit1 = pseg->begin(); pit1 < pseg->end();
                   ++pit1) {
                ...
                ofs << (format("U1n %1$+1.7e %2$+1.7e %3$+1.7e   ") %
                        (u1_n(0)) % (u1_n(1)) % (u1_n(2)))
                           .str();
                ofs << (format("U1c %1$+1.7e %2$+1.7e %3$+1.7e   ") %
                        (u1_c(0)) % (u1_c(1)) % (u1_c(2)))
                           .str();
                ofs << "BG" << std::endl;
              }
            } else
    ...
    for (sit1 = _fg_C.begin(), sit2 = _fg_N.begin(); sit1 < _fg_C.end();
      ...
      for (pit1 = pseg_h->begin(), pit2 = pseg_n->begin(); pit1 < pseg_h->end();
        ...
        ofs << (format("XYZ %1$+1.7e %2$+1.7e %3$+1.7e    ") % (pos(0)) %
                (pos(1)) % (pos(2)))
                   .str();
        ofs << (format("U1n %1$+1.7e %2$+1.7e %3$+1.7e   ") % (u1_n(0)) %
                (u1_n(1)) % (u1_n(2)))
                   .str();
        ofs << (format("U1c %1$+1.7e %2$+1.7e %3$+1.7e   ") % (u1_c(0)) %
                (u1_c(1)) % (u1_c(2)))
                   .str();
        ofs << "FG" << std::endl;
      }
...
void Ewald3DnD::ShowAgenda(Logger *log) {
  ...
  XTP_LOG(Log::debug, *log)
      << "System & Ewald parameters (" << IdentifyMethod() << ")" << std::flush;
  XTP_LOG(Log::debug, *log) << "  o Real-space unit cell:      " << _a << " x "
                            << _b << " x " << _c << std::flush;
  XTP_LOG(Log::debug, *log)
      << "  o Real-space c/o (guess):    " << _R_co << " nm" << std::flush;
  XTP_LOG(Log::debug, *log) << "  o na(max), nb(max), nc(max): " << _na_max
                            << ", " << _nb_max << ", " << _nc_max << std::flush;
  XTP_LOG(Log::debug, *log) << "  o 1st Brillouin zone:        " << _A << " x "
                            << _B << " x " << _C << std::flush;
  XTP_LOG(Log::debug, *log)
  ...
  XTP_LOG(Log::debug, *log) << "  o kx(max), ky(max), kz(max): " << _NA_max
                            << ", " << _NB_max << ", " << _NC_max << std::flush;

  XTP_LOG(Log::debug, *log)
      << "Tasks to perform (" << IdentifyMethod() << ")" << std::flush;
  XTP_LOG(Log::debug, *log)
      << "  o Scan interaction range:    "
      << ((_task_scan_cutoff) ? "yes" : "no") << std::flush;
  XTP_LOG(Log::debug, *log)
      << "  o Calculate fg fields:       "
      << ((_task_calculate_fields) ? "yes" : "no") << std::flush;
  XTP_LOG(Log::debug, *log)
      << "  o Polarize foreground:       "
      << ((_task_polarize_fg) ? "yes" : "no") << std::flush;
  XTP_LOG(Log::debug, *log)
      << "  o Evaluate energy:           "
      << ((_task_evaluate_energy) ? "yes" : "no") << std::flush;
  XTP_LOG(Log::debug, *log)
      << "  o Apply radial correction:   "
      << ((_task_apply_radial) ? "yes" : "no") << std::flush;
...
void Ewald3DnD::ShowFieldsTeaser(std::vector<PolarSeg *> &target, Logger *log) {
  ...
  int fieldCount = 0;
  for (std::vector<PolarSeg *>::iterator sit1 = target.begin();
       sit1 < target.end(); ++sit1) {
    PolarSeg *pseg = *sit1;
    Segment seg = _top->getSegment(pseg->getId());
    XTP_LOG(Log::debug, *log) << "ID = " << pseg->getId() << " ("
                              << seg.getType() << ") " << std::flush;
    for (PolarSeg::iterator pit1 = pseg->begin(); pit1 < pseg->end(); ++pit1) {
      ...
      XTP_LOG(Log::debug, *log)
          << (format("FPU = (%1$+1.7e %2$+1.7e %3$+1.7e) V/m    ") %
              (fp(0) * int2V_m + fu(0) * int2V_m) %
              (fp(1) * int2V_m + fu(1) * int2V_m) %
              (fp(2) * int2V_m + fu(2) * int2V_m))
                 .str();
      XTP_LOG(Log::debug, *log)
          << (format("U1* = (%1$+1.7e %2$+1.7e %3$+1.7e) e*nm") % (u1(0)) %
              (u1(1)) % (u1(2)))
                 .str()
          << std::flush;
      fieldCount += 1;
      if (fieldCount > 10) {
        XTP_LOG(Log::debug, *log) << "FPU = ... ... ..." << std::flush;
        break;
      }
    }
...
void Ewald3DnD::ShowEnergySplitting() {
  ...
  XTP_LOG(Log::debug, *_log) << std::flush;
  XTP_LOG(Log::info, *_log)
      << (format("Interaction FGC -> ***")).str() << std::flush
      << (format("  + EPP(FGC->MGN)  = %1$+1.7e = %2$+1.7e  %3$+1.7e  %4$+1.7e "
                 "eV") %
          _ER.Sum() % _ER._pp % _ER._pu % _ER._uu)
             .str()
      << std::flush
      << (format("  + EKK(FGC->BGP)  = %1$+1.7e = %2$+1.7e  %3$+1.7e  %4$+1.7e "
                 "eV") %
          _EK.Sum() % _EK._pp % _EK._pu % _EK._uu)
             .str()
      << std::flush
      << (format("  - EPP(FGC->FGN)  = %1$+1.7e = %2$+1.7e  %3$+1.7e  %4$+1.7e "
                 "eV") %
          _EC.Sum() % _EC._pp % _EC._pu % _EC._uu)
             .str()
      << std::flush
      << (format("  + EK0(FGC->BGP)  = %1$+1.7e = %2$+1.7e  %3$+1.7e  %4$+1.7e "
                 "eV") %
          _E0.Sum() % _E0._pp % _E0._pu % _E0._uu)
             .str()
      << std::flush
      << (format("  + EDQ(FGC->MGN)  = %1$+1.7e = %2$+1.7e  %3$+1.7e  %4$+1.7e "
                 "eV") %
          _EDQ.Sum() % _EDQ._pp % _EDQ._pu % _EDQ._uu)
             .str()
      << std::flush
      << (format("  + EJ(shape-dep.) = %1$+1.7e = %2$+1.7e  %3$+1.7e  %4$+1.7e "
                 "eV") %
          _EJ.Sum() % _EJ._pp % _EJ._pu % _EJ._uu)
             .str()
      << std::flush
      << (format("  = "
                 "-------------------------------------------------------------"
             ...
             .str()
      << std::flush
      << (format("  + SUM(E) (0,Q,J) = %1$+1.7e = %2$+1.7e  %3$+1.7e  %4$+1.7e "
                 "eV") %
          _ET.Sum() % _ET._pp % _ET._pu % _ET._uu)
             .str()
      << std::flush;

  XTP_LOG(Log::info, *_log)
      << (format("Interaction FGC <> FGC")).str() << std::flush
      << (format("  + EF [00 01 11]  = %1$+1.7e = %2$+1.7e  %3$+1.7e  %4$+1.7e "
                 "eV") %
          (_polar_EF00 + _polar_EF01 + _polar_EF11) % _polar_EF00 %
          _polar_EF01 % _polar_EF11)
             .str()
      << std::flush
      << (format("  + EF [02 12 22]  = %1$+1.7e = *****ZERO*****  "
                 "*****ZERO*****  *****ZERO***** eV") %
          0.0)
             .str()
      << std::flush
      << (format("  + EM [0  1  2 ]  = %1$+1.7e = %2$+1.7e  %3$+1.7e  "
                 "*****ZERO***** eV") %
          (_polar_EM0 + _polar_EM1) % _polar_EM0 % _polar_EM1)
             .str()
      << std::flush
      << (format("  o E  [PP PU UU]  = %1$+1.7e = %2$+1.7e  %3$+1.7e  %4$+1.7e "
                 "eV") %
          (_polar_EPP + _polar_EPU + _polar_EUU) % _polar_EPP % _polar_EPU %
          _polar_EUU)
             .str()
      << std::flush
      << (format("  = "
             ...
             .str()
      << std::flush
      << (format("  + SUM(E)         = %1$+1.7e = %2$+1.7e  %3$+1.7e  %4$+1.7e "
                 "eV") %
  ...
  XTP_LOG(Log::info, *_log)
      << (format("Interaction FGC <> FGC(i) u ***(o)")).str() << std::flush
      << (format("  + Ei [pp+pu+iw]  = %1$+1.7e = %2$+1.7e  %3$+1.7e  %4$+1.7e "
                 "eV") %
          _inner % _inner_epp % _inner_eppu % _inner_ework)
             .str()
      << std::flush
      << (format("  + Eo [pp+pu+iw]  = %1$+1.7e = %2$+1.7e  %3$+1.7e  "
                 "************** eV") %
      ...
      << (format("  = "
             ...
             .str()
      << std::flush
      << (format("  + E  [stat+ind]  = %1$+1.7e = %2$+1.7e  %3$+1.7e eV") %
...
//    }
  ...
  */
  ...
  // TEASER OUTPUT PERMANENT FIELDS
  XTP_LOG(Log::debug, *_log)
      << std::flush << "Foreground fields (FGC):" << std::flush;
  ...
  _inner_eppu = _polar_EF00 + _polar_EF01 + _polar_EF02 + _polar_EF11 +
                _polar_EF12 - _polar_EPP;
  ...
  if (Log::verbose()) {
    std::ofstream ofs;
    ofs.open("indu_state", std::ofstream::out);
    for (std::vector<PolarSeg *>::iterator sit1 = _bg_P.begin();
         sit1 < _bg_P.end(); ++sit1) {
      PolarSeg *pseg = *sit1;
      // Segment *seg = _top->getSegment(pseg->getId());
  ...
  XTP_LOG(Log::debug, *_log)
      << std::flush << (format("Timing (T = %1$1.2f min)") % (_t_total))
      << std::flush;
  XTP_LOG(Log::debug, *_log) << (format("  o Usage <Coarsegrain> = %1$2.2f%%") %
                                 (100 * _t_coarsegrain / _t_total))
                             << std::flush;
  XTP_LOG(Log::debug, *_log) << (format("  o Usage <Fields>      = %1$2.2f%%") %
                                 (100 * _t_fields / _t_total))
                             << std::flush;
  XTP_LOG(Log::debug, *_log) << (format("  o Usage <Induction>   = %1$2.2f%%") %
                                 (100 * _t_induction / _t_total))
                             << std::flush;
  XTP_LOG(Log::debug, *_log) << (format("  o Usage <Energy>      = %1$2.2f%%") %
                                 (100 * _t_energy / _t_total))
                             << std::flush;
  XTP_LOG(Log::debug, *_log) << std::flush;
...
void Ewald3DnD::EvaluateFields(bool do_depolarize_fgc) {
  ...
  for (sit1 = _fg_C.begin(); sit1 < _fg_C.end(); ++sit1) {
    ...
    for (pit1 = pseg->begin(); pit1 < pseg->end(); ++pit1) {
      ...
      fieldCount += 1;
      if (fieldCount > 10) {
        XTP_LOG(Log::debug, *_log) << "F = ... ... ..." << std::flush;
        break;
      }
    }
    if (fieldCount > 10) break;
...
void Ewald3DnD::EvaluateInduction() {
  ...
  XTP_LOG(Log::debug, *_log) << std::flush;
  XTP_LOG(Log::debug, *_log)
      << format("Call inductor on FGC = QM0 u MM1") << std::flush;
  XTP_LOG(Log::debug, *_log)
      << (format("  o |QM0|, |MM1|, |MM2|        %1$d %2$d %3$d") %
          _ptop->QM0().size() % _ptop->MM1().size() % _ptop->MM2().size())
             .str()
      << std::flush;
  XTP_LOG(Log::debug, *_log)
      << (format("  o Polarization cut-off:      ")).str() << _polar_cutoff
      << " nm " << std::flush;
  XTP_LOG(Log::debug, *_log) << (format("  o With induction:            %1$s") %
                                 ((_polar_do_induce) ? "yes" : "no"))
                             << std::flush;
  XTP_LOG(Log::debug, *_log)
      << (format("  o Thole sharpness parameter: ")).str() << _polar_aDamp
      << std::flush;
  XTP_LOG(Log::debug, *_log)
      << (format("  o SOR mixing factor:         ")).str() << _polar_wSOR_N
      << " (N) " << _polar_wSOR_C << " (C) " << std::flush;
  XTP_LOG(Log::debug, *_log)
      << (format("  o Iterations (max):          512")).str() << std::flush;
  XTP_LOG(Log::debug, *_log)
      << (format("  o Tolerance (dU/U):          %1$1.3e") % _polar_epstol)
             .str()
      << std::flush;
  XTP_LOG(Log::debug, *_log)
      << (format("  o Induce within QM0:         yes")).str() << std::flush;
  XTP_LOG(Log::debug, *_log)
      << (format("  o Subthreads:                single")).str() << std::flush;
  XTP_LOG(Log::debug, *_log)
      << (format("  o Started from archive:      %1$s") %
          ((_started_from_archived_indu_state) ? "yes" : "no"))
      << std::flush;

  if (_started_from_archived_indu_state) {
    XTP_LOG(Log::debug, *_log)
        << "Reusing induction state from archive." << std::flush;
    std::vector<PolarSeg *>::iterator sit1, sit2;
    PolarSeg::iterator pit1, pit2;
    for (sit1 = _fg_C.begin(), sit2 = _fg_N.begin(); sit1 < _fg_C.end();
         ++sit1, ++sit2) {
  ...
  XTP_LOG(Log::debug, *_log) << "Creating inductor object" << std::flush;

  XInductor polar_xind =
      XInductor(_polar_do_induce, polar_induce_intra_pair, polar_subthreads,
                _polar_wSOR_N, _polar_wSOR_C, _polar_epstol, polar_maxIter,
                _polar_aDamp, polar_maverick, _top);
  XTP_LOG(Log::debug, *_log) << "success." << std::flush;
...
bool Ewald3DnD::EvaluateInductionQMMM(bool do_reset, bool do_reuse_bgp_state,
  ...
  XTP_LOG(Log::debug, *_log) << std::flush;
  XTP_LOG(Log::debug, *_log) << "Started MM Polarization" << std::flush;

  // DEPOLARIZATION STAGE
  if (do_reset) {
    XTP_LOG(Log::debug, *_log)
        << " o Carry out complete depolarization on FGC" << std::flush;
    for (sit = _fg_C.begin(); sit < _fg_C.end(); ++sit) {
    ...
    XTP_LOG(Log::debug, *_log)
        << " o Carry out partial depolarization on FGC" << std::flush;
    for (sit = _fg_C.begin(); sit < _fg_C.end(); ++sit) {
      for (pit = (*sit)->begin(); pit != (*sit)->end(); ++pit) {
        (*pit)->ResetFieldU();
      }
  ...
  if (do_calc_perm_fields) {
    XTP_LOG(Log::debug, *_log) << std::flush;
    XTP_LOG(Log::debug, *_log)
        << "Compute fields for aperiodic embedding" << std::flush;

    if (do_calc_perm_bg_fields) {
      // FIELDS ON 'A' GENERATED BY APERIODIC BACKGROUND : FP[Ql,U1 e B*~] -> X
      // u P
      XTP_LOG(Log::debug, *_log) << " o FP[Ql,U1 e B*~] -> X u P" << std::flush;
    ...
    if (do_calc_perm_fg_fields) {
      ...
      XTP_LOG(Log::debug, *_log) << " o FP[Ql e P]      -> P" << std::flush;
      ...
      XTP_LOG(Log::debug, *_log) << " o FP[Ql e P]      -> X" << std::flush;
      for (sit1 = _polar_mm1.begin(); sit1 != _polar_mm1.end(); ++sit1) {
  ...
  XTP_LOG(Log::debug, *_log) << std::flush;
  XTP_LOG(Log::debug, *_log)
      << "Start classical SCF with background fields" << std::flush;
  XTP_LOG(Log::debug, *_log) << std::flush;
  _log->DisablePreface();
  ...
  for (; n_iter < max_iter + 1; ++n_iter) {
    ...
    XTP_LOG(Log::debug, *_log)
        << (boost::format("o %1$2d") % n_iter) << std::flush;
    if (n_iter % 10 == 0) {
      _log->EnablePreface();
      XTP_LOG(Log::debug, *_log) << std::flush;
      _log->DisablePreface();
...
void Ewald3DnD::EvaluateEnergy(std::vector<PolarSeg *> &target) {
  ...
  EWD::triple<> EDQ_fgC_mgN =
      CalculateHigherRankCorrection(target);  // ! OBSOLETE !
...
void Ewald3DnD::EvaluateEnergyQMMM() {
  ...
  if (Log::verbose()) {
    XTP_LOG(Log::info, *_log) << std::flush;
    XTP_LOG(Log::info, *_log)
        << "R(MM1) " << EPP_mm1_mgN * int2eV << std::flush;
    XTP_LOG(Log::info, *_log)
        << "R(MM1) " << EPP_qm0_mgN * int2eV << std::flush;
  }
  ...
  if (Log::verbose()) {
    XTP_LOG(Log::info, *_log) << std::flush;
    XTP_LOG(Log::info, *_log) << "K " << EKK_mm1_bgP * int2eV << std::flush;
    XTP_LOG(Log::info, *_log) << "K " << EKK_qm0_bgP * int2eV << std::flush;
  }
  ...
  if (Log::verbose()) {
    XTP_LOG(Log::info, *_log) << std::flush;
    XTP_LOG(Log::info, *_log) << "0 " << EK0_mm1_bgP * int2eV << std::flush;
    XTP_LOG(Log::info, *_log) << "0 " << EK0_qm0_bgP * int2eV << std::flush;
  }
  ...
  if (Log::verbose()) {
    XTP_LOG(Log::info, *_log) << std::flush;
    XTP_LOG(Log::info, *_log) << "J " << EJ_mm1_bgP * int2eV << std::flush;
    XTP_LOG(Log::info, *_log) << "J " << EJ_qm0_bgP * int2eV << std::flush;
  }
  ...
  if (Log::verbose()) {
    XTP_LOG(Log::info, *_log) << std::flush;
    XTP_LOG(Log::info, *_log) << "C " << EPP_mm1_fgN * int2eV << std::flush;
    XTP_LOG(Log::info, *_log) << "C " << EPP_qm0_fgN * int2eV << std::flush;
  }

  _log->setReportLevel(Log::debug);

  // STORE ENERGIES RESOLVED ACCORDING TO MM1 AND QM0
  _ER_MM1 = EPP_mm1_mgN * int2eV;
  _ER_QM0 = EPP_qm0_mgN * int2eV;

  _EK_MM1 = EKK_mm1_bgP * int2eV;
  _EK_QM0 = EKK_qm0_bgP * int2eV;

  _E0_MM1 = EK0_mm1_bgP * int2eV;
  _E0_QM0 = EK0_qm0_bgP * int2eV;

  _EJ_MM1 = EJ_mm1_bgP * int2eV;
  _EJ_QM0 = EJ_qm0_bgP * int2eV;

  _EC_MM1 = EPP_mm1_fgN * int2eV;
  _EC_QM0 = EPP_qm0_fgN * int2eV;
  ...
  if (true || Log::verbose()) {
    XTP_LOG(Log::info, *_log) << std::flush;
    XTP_LOG(Log::info, *_log)
        << "QM-MM splitting for background interaction (eV)" << std::flush;
    XTP_LOG(Log::info, *_log) << "  o E(MM1)       = " << _ET_MM1 << std::flush;
    XTP_LOG(Log::info, *_log) << "  o E(QM0)       = " << _ET_QM0 << std::flush;
    XTP_LOG(Log::info, *_log)
        << "  o E(QM0 u MM1) = " << _ET_MM1 + _ET_QM0 << std::flush;
  }
...
void Ewald3DnD::EvaluateRadialCorrection() {
  ...
  XTP_LOG(Log::info, *_log) << std::flush;
  XTP_LOG(Log::info, *_log)
      << "Apply dielectric radial correction" << std::flush;
  XTP_LOG(Log::info, *_log)
      << "  o Radial screening constant: " << _polar_radial_corr_epsilon
      << std::flush;
  XTP_LOG(Log::info, *_log)
      << "  o Radial extension: " << _polar_cutoff << "nm <> "
      << _polar_cutoff + _R_co << "nm" << std::flush;
  ...
  XTP_LOG(Log::debug, *_log) << "Steps I-IV" << std::flush;
  // Depolarize midground
  XTP_LOG(Log::debug, *_log) << "  o Depolarize midground" << std::flush;
  ...
  XTP_LOG(Log::debug, *_log)
      << "  o Add screened field contribution (QM0)" << std::flush;
  for (sit1 = _mg_N.begin(); sit1 != _mg_N.end(); ++sit1) {
    for (pit1 = (*sit1)->begin(); pit1 < (*sit1)->end(); ++pit1) {
      for (sit2 = _polar_qm0.begin(); sit2 != _polar_qm0.end(); ++sit2) {
        for (pit2 = (*sit2)->begin(); pit2 != (*sit2)->end(); ++pit2) {
          _actor.BiasStat(*(*pit1), *(*pit2));
          _actor.FieldPerm_At_By(*(*pit1), *(*pit2),
                                 _polar_radial_corr_epsilon);
        }
      }
    }
  ...
  // Induce
  XTP_LOG(Log::debug, *_log) << "  o Induce (direct)" << std::flush;
  for (sit1 = _mg_N.begin(); sit1 != _mg_N.end(); ++sit1) {
    for (pit1 = (*sit1)->begin(); pit1 != (*sit1)->end(); ++pit1) {
      (*pit1)->InduceDirect();
    }
  ...
  // Evaluate Energy
  XTP_LOG(Log::debug, *_log)
      << "  o Evaluate stabilization energy" << std::flush;
  _polar_ERC = 0.0;
  for (sit1 = _mg_N.begin(); sit1 != _mg_N.end(); ++sit1) {
    for (pit1 = (*sit1)->begin(); pit1 < (*sit1)->end(); ++pit1) {
      for (sit2 = _polar_qm0.begin(); sit2 != _polar_qm0.end(); ++sit2) {
  ...
  _polar_ERC *= int2eV;
  XTP_LOG(Log::info, *_log) << "=> ERC = " << _polar_ERC << "eV" << std::flush;
  return;
}
...
void Ewald3DnD::EvaluatePotentialGrid(std::vector<PolarSeg *> &target,Vxc_Grid &grid,
  ...
  if (add_bg) {
    ...
    // RECIPROCAL-SPACE CONTRIBUTION (3D2D && 3D3D) -> Kvec criterion based on FGC-BGP interaction
    ...
    for (sit1 = _polar_mm1.begin(); sit1 != _polar_mm1.end(); ++sit1) {
      ...
      for (Index i = 0; i < grid.getBoxesSize(); ++i) {
        ...
        for (Index p = 0; p < box.size(); p++) {
            values[p]+=_actor.Potential_At_By(points[p]* tools::conv::bohr2nm, *(*pit1)) * tools::conv::nm2bohr;
          }
        }
      }
    }
  ...
  XTP_LOG(Log::info, *_log)
      << std::flush << "Potential calculated on numerical integration grid " << std::flush;
...
EWD::triple<> Ewald3DnD::ConvergeRealSpaceSum(std::vector<PolarSeg *> &target) {
  ...
  for (int i = 0; i < 1000; ++i) {
    ...
    XTP_LOG(Log::debug, *_log)
        << (format("Rc = %1$+1.7f   |MGN| = %3$5d nm   ER = %2$+1.7f eV   "
                   "dER(rms) = %4$+1.7f") %
            Rc % (this_ER * int2eV) % _mg_N.size() % dER_rms)
               .str()
        << std::flush;
    if (i > 0 && dER_rms < _crit_dE) {
      _converged_R = true;
      XTP_LOG(Log::debug, *_log)
          << (format(":::: Converged to precision as of Rc = %1$+1.3f nm") % Rc)
          << std::flush;
      break;
    }
    prev_ER = this_ER;
...
std::string Ewald3DnD::GenerateErrorstring() {
  std::string rstr;
  rstr += (format("Converged R-sum = %1$s, converged K-sum = %2$s, ") %
           ((_converged_R) ? "true" : "false") %
           ((_converged_K) ? "true" : "false"))
              .str();
  rstr += (format("converged induction = %1$s") %
           ((_polar_converged) ? "true" : "false"))
              .str();
  return rstr;
}

tools::Property Ewald3DnD::GenerateOutputstring() {
  ...
  next->add("total", (format("%1$+1.7f") % _Eppuu).str())
      .setAttribute("unit", "eV");
  next->add("estat", (format("%1$+1.7f") % _Estat).str())
      .setAttribute("unit", "eV");
  next->add("eindu", (format("%1$+1.7f") % _Eindu).str())
      .setAttribute("unit", "eV");
  ...
  next->add("F-00-01-11", (format("%1$+1.5e %2$+1.5e %3$+1.5e") % _polar_EF00 %
                           _polar_EF01 % _polar_EF11)
                              .str());
  next->add("M-00-11---",
            (format("%1$+1.5e %2$+1.5e") % _polar_EM0 % _polar_EM1).str());
  next->add("E-PP-PU-UU", (format("%1$+1.5e %2$+1.5e %3$+1.5e") % _polar_EPP %
                           _polar_EPU % _polar_EUU)
                              .str());

  next = &out.add("terms_o", "");
  next->add("R-pp-pu-uu", (format("%1$+1.5e = %2$+1.5e %3$+1.5e %4$+1.5e") %
                           _ER.Sum() % _ER._pp % _ER._pu % _ER._uu)
                              .str());
  next->add("K-pp-pu-uu", (format("%1$+1.5e = %2$+1.5e %3$+1.5e %4$+1.5e") %
                           _EK.Sum() % _EK._pp % _EK._pu % _EK._uu)
                              .str());
  next->add("O-pp-pu-uu", (format("%1$+1.5e = %2$+1.5e %3$+1.5e %4$+1.5e") %
                           _E0.Sum() % _E0._pp % _E0._pu % _E0._uu)
                              .str());
  next->add("J-pp-pu-uu", (format("%1$+1.5e = %2$+1.5e %3$+1.5e %4$+1.5e") %
                           _EJ.Sum() % _EJ._pp % _EJ._pu % _EJ._uu)
                              .str());
  next->add("C-pp-pu-uu", (format("%1$+1.5e = %2$+1.5e %3$+1.5e %4$+1.5e") %
                           _EC.Sum() % _EC._pp % _EC._pu % _EC._uu)
                              .str());
  next->add("Q-pp-pu-uu", (format("%1$+1.5e = %2$+1.5e %3$+1.5e %4$+1.5e") %
                           _EDQ.Sum() % _EDQ._pp % _EDQ._pu % _EDQ._uu)
                              .str());
  ...
  next->add("t_wload",
            (format("%1$1.2f %2$1.2f %3$1.2f %4$1.2f") % (_t_coarsegrain) %
             (_t_fields) % (_t_induction) % (_t_energy))
                .str())
      .setAttribute("unit", "min");

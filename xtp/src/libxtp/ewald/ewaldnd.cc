...
Ewald3DnD::Ewald3DnD(const Topology *top, PolarTop *ptop, tools::Property *opt, Logger *log) 
    : _top(top), _ptop(ptop), _log(log), _fg_table(0) {
    
    // EVALUATE OPTIONS
    std::string pfx = "options.ewald";
    // Multipoles: started from archive?
    if (opt->exists(pfx+".multipoles.polar_bg")) {
        std::string ptop_file = opt->get(pfx+".multipoles.polar_bg").as<std::string>();
    ...
    // Ewald parameters
    std::string cmethod = opt->get(pfx+".coulombmethod.method").as<std::string>();
    ...
    if (opt->exists(pfx+".convergence.rfactor"))
        ...
        _rfactor = 6.;
    // Polar parameters
    std::string pmethod = opt->get(pfx+".coulombmethod.method").as<std::string>();
  ...
  _NC_max = int(ceil(_K_co / _C.cwiseAbs().maxCoeff()));
    ...
    if (_do_compensate_net_dipole) {
        ...
        XTP_LOG(Log::info,*_log) << "  o System Q1 compensation: " << system_dpl 
            << "  (apply to " << charged_count << " polar sites)" << std::flush;
    ...
    XTP_LOG(Log::info,*_log)
        ...
        << std::flush << (format("  o Q(FGC) = %1$+1.3fe |FGC| = %2$+5d") % Q_fg_C % _fg_C.size()).str()
        << std::flush << (format("  o Q(FGN) = %1$+1.3fe |FGN| = %2$+5d") % Q_fg_N % _fg_N.size()).str()
        << std::flush << (format("  o Q(MGN) = %1$+1.3fe |MGN| ~ %2$+5d") % Q_mg_N % _mg_N.size()).str()
        << std::flush << (format("  o Q(BGN) = %1$+1.3fe |BGN| = %2$+5d") % Q_bg_N % _bg_N.size()).str()
        << std::flush << (format("  o Q(BGP) = %1$+1.3fe |BGP| = %2$+5d") % Q_bg_P % _bg_P.size()).str()
        << std::flush << (format("  o Job type '%3$s' (iQ1=%1$d, iQ2=%2$d)") % iQ1 % iQ2 % _jobType).str()
        << std::flush;
    
    if (std::abs(Q_bg_P) > 1e-2) {
        std::cout << std::endl;
        std::cout << std::endl << format("***************************** ERROR ******************************");
        std::cout << std::endl << format("       Background charge |Q(BGP)| is larger than 0.01e.");
        std::cout << std::endl << format("       Be more precise: e.g. rounding error?");
        std::cout << std::endl << format("       Or think again: e.g. erroneous parametrization?");
    ...
    XTP_LOG(Log::info,*_log)
        ...
        << std::flush << (format("  o D(BGP) [e*nm]           = %1$+1.3f %2$+1.3f %3$+1.3f  ") 
        % netdpl_bgP(0) % netdpl_bgP(1) % netdpl_bgP(2)).str();
    XTP_LOG(Log::info,*_log)
        << std::flush << (format("  o D(FGC) [e*nm]           = %1$+1.3f %2$+1.3f %3$+1.3f  ") 
        % netdpl_fgC(0) % netdpl_fgC(1) % netdpl_fgC(2)).str();
    XTP_LOG(Log::info,*_log)
        << std::flush << (format("  o Sigma q|z|**2 [e*nm**2] = %1$+1.7f   ")
        % qzz_bgP) << std::flush;
...
void Ewald3DnD::ExpandForegroundReduceBackground(double polar_R_co) {
    ...
    XTP_LOG(Log::debug,*_log) << "  o Max. int. distance (QM0): " << 
        _max_int_dist_qm0 << "nm" << std::flush;
    ...
    for (sit1 = _bg_P.begin(); sit1 < _bg_P.end(); ++sit1) {
        ...
        for (int nc = -polar_nc_max; nc < polar_nc_max+1; ++nc) {            
            ...
            for (sit2 = _fg_C.begin(), sit3 = _fg_N.begin(); 
                ...
                double dR_L = (
                    seg_bg->getPos() + L - (*sit3)->getPos()).norm();
...
void Ewald3DnD::CoarseGrainDensities(bool cg_bg, bool cg_fg, double cg_radius) {
    ...
    XTP_LOG(Log::debug,*_log) << "  o Coarse-grain background:   " << ((_coarse_do_cg_background) ? "yes" : "no") << std::flush;
    XTP_LOG(Log::debug,*_log) << "  o Coarse-grain foreground:   " << ((_coarse_do_cg_foreground) ? "yes" : "no") << std::flush;
    XTP_LOG(Log::debug,*_log) << "  o Anisotropic P-tensor:      " << ((_coarse_cg_anisotropic) ? "yes" : "no") << std::flush;
    XTP_LOG(Log::debug,*_log) << "  o Coarse-graining radius:    " << _coarse_cg_radius << " nm" << std::flush;
    ...
    if (cg_bg) {
        ...
        XTP_LOG(Log::debug,*_log) << "  o Coarse-grained "
    ...
    if (cg_fg) {
        ...
        XTP_LOG(Log::debug,*_log) << "  o Coarse-grained "
             << count_fgc << "/" << _fg_C.size() << " (FGC) " << std::flush;
    }    
    return;
}
...
void Ewald3DnD::WriteDensitiesPtop(std::string fg, std::string mg, std::string bg) {
...
void Ewald3DnD::WriteInductionStateTable() {
    ...
    if (false && Log::verbose()) {
        std::string tabfile = "polarized_"
            + boost::lexical_cast<std::string>(_polar_qm0[0]->getId())
            + "_" + _jobType + ".tab";
        std::ofstream ofs;
        ...
        for (sit1 = _fg_C.begin(), sit2 = _fg_N.begin(); 
            ...
            for (pit1 = pseg_h->begin(), pit2 = pseg_n->begin(); 
                ...
                ofs << (format("dR2 %1$+1.7f fput456 %2$+1.7e %3$+1.7e %4$+1.7e "
                    ...
                    % dR % ((dfp).norm()*int2V_m) % ((dfu).norm()*int2V_m) % ((dft).norm()*int2V_m)
    ...
    if (true && Log::verbose()) {
        std::ofstream ofs;
        std::string tabfile = this->IdentifyMethod() + ".indu_state_"
            + boost::lexical_cast<std::string>(_polar_qm0[0]->getId())
            + "_" + _jobType + ".tab";
        ofs.open(tabfile.c_str(), std::ofstream::out);
...
void Ewald3DnD::ShowAgenda(Logger *log) {
    ...
    XTP_LOG(Log::debug,*log) << "System & Ewald parameters (" << IdentifyMethod() << ")" << std::flush;
    XTP_LOG(Log::debug,*log) << "  o Real-space unit cell:      " << _a << " x " << _b << " x " << _c << std::flush;
    XTP_LOG(Log::debug,*log) << "  o Real-space c/o (guess):    " << _R_co << " nm" << std::flush;
    XTP_LOG(Log::debug,*log) << "  o na(max), nb(max), nc(max): " << _na_max << ", " << _nb_max << ", " << _nc_max << std::flush;
    XTP_LOG(Log::debug,*log) << "  o 1st Brillouin zone:        " << _A << " x " << _B << " x " << _C << std::flush;
    XTP_LOG(Log::debug,*log) << "  o Reciprocal-space c/o:      " << _K_co << " 1/nm" << std::flush;
    XTP_LOG(Log::debug,*log) << "  o R-K switching param.       " << _alpha << " 1/nm" << std::flush;
    XTP_LOG(Log::debug,*log) << "  o Unit-cell volume:          " << _LxLyLz << " nm**3" << std::flush;
    XTP_LOG(Log::debug,*log) << "  o LxLy (for 3D2D EW):        " << _LxLy << " nm**2" << std::flush;
    XTP_LOG(Log::debug,*log) << "  o kx(max), ky(max), kz(max): " << _NA_max << ", " << _NB_max << ", " << _NC_max << std::flush;
    
    XTP_LOG(Log::debug,*log) << "Tasks to perform (" << IdentifyMethod() << ")" << std::flush;
    XTP_LOG(Log::debug,*log) << "  o Scan interaction range:    " << ((_task_scan_cutoff) ? "yes" : "no") << std::flush;
    XTP_LOG(Log::debug,*log) << "  o Calculate fg fields:       " << ((_task_calculate_fields) ? "yes" : "no") << std::flush;
    XTP_LOG(Log::debug,*log) << "  o Polarize foreground:       " << ((_task_polarize_fg) ? "yes" : "no") << std::flush;
    XTP_LOG(Log::debug,*log) << "  o Evaluate energy:           " << ((_task_evaluate_energy) ? "yes" : "no") << std::flush;
    XTP_LOG(Log::debug,*log) << "  o Apply radial correction:   " << ((_task_apply_radial) ? "yes" : "no") << std::flush;
...
void Ewald3DnD::ShowFieldsTeaser(std::vector<PolarSeg*> &target, Logger *log) {
    ...
    for (std::vector<PolarSeg*>::iterator sit1 = target.begin(); sit1 < target.end(); ++sit1) {
        PolarSeg *pseg = *sit1;
        Segment seg = _top->getSegment(pseg->getId());
        XTP_LOG(Log::debug,*log) << "ID = " << pseg->getId() << " (" << seg.getType() << ") " << std::flush;
        for (PolarSeg::iterator pit1 = pseg->begin(); pit1 < pseg->end(); ++pit1) {
            vec fp = (*pit1)->getFieldP();
            vec fu = (*pit1)->getFieldU();
            vec u1 = (*pit1)->getU1();
...
void Ewald3DnD::ShowEnergySplitting(Logger *log) {
    ...
    XTP_LOG(Log::info,*_log)
        ...
        << std::flush << (format("  + EPP(FGC->MGN)  = %1$+1.7e = %2$+1.7e  %3$+1.7e  %4$+1.7e eV") 
            % _ER.Sum() % _ER._pp % _ER._pu % _ER._uu).str()
        << std::flush << (format("  + EKK(FGC->BGP)  = %1$+1.7e = %2$+1.7e  %3$+1.7e  %4$+1.7e eV") 
            % _EK.Sum() % _EK._pp % _EK._pu % _EK._uu).str()       
        << std::flush << (format("  - EPP(FGC->FGN)  = %1$+1.7e = %2$+1.7e  %3$+1.7e  %4$+1.7e eV") 
            % _EC.Sum() % _EC._pp % _EC._pu % _EC._uu).str()
        << std::flush << (format("  + EK0(FGC->BGP)  = %1$+1.7e = %2$+1.7e  %3$+1.7e  %4$+1.7e eV") 
            % _E0.Sum() % _E0._pp % _E0._pu % _E0._uu).str() 
        << std::flush << (format("  + EDQ(FGC->MGN)  = %1$+1.7e = %2$+1.7e  %3$+1.7e  %4$+1.7e eV") 
            % _EDQ.Sum() % _EDQ._pp % _EDQ._pu % _EDQ._uu).str()
        << std::flush << (format("  + EJ(shape-dep.) = %1$+1.7e = %2$+1.7e  %3$+1.7e  %4$+1.7e eV") 
            % _EJ.Sum() % _EJ._pp % _EJ._pu % _EJ._uu).str()
        << std::flush << (format("  = -----------------------------------------------------------------------------------")).str()
        << std::flush << (format("  + SUM(E) (0,Q,J) = %1$+1.7e = %2$+1.7e  %3$+1.7e  %4$+1.7e eV") 
            % _ET.Sum() % _ET._pp % _ET._pu % _ET._uu).str()
        << std::flush;
    
    XTP_LOG(Log::info,*_log)
        << (format("Interaction FGC <> FGC")).str()
        << std::flush << (format("  + EF [00 01 11]  = %1$+1.7e = %2$+1.7e  %3$+1.7e  %4$+1.7e eV") 
            % (_polar_EF00+_polar_EF01+_polar_EF11) % _polar_EF00 % _polar_EF01 % _polar_EF11).str()
        << std::flush << (format("  + EF [02 12 22]  = %1$+1.7e = *****ZERO*****  *****ZERO*****  *****ZERO***** eV") 
            % 0.0).str()
        << std::flush << (format("  + EM [0  1  2 ]  = %1$+1.7e = %2$+1.7e  %3$+1.7e  *****ZERO***** eV") 
            % (_polar_EM0+_polar_EM1) % _polar_EM0 % _polar_EM1).str()
        << std::flush << (format("  o E  [PP PU UU]  = %1$+1.7e = %2$+1.7e  %3$+1.7e  %4$+1.7e eV") 
            % (_polar_EPP+_polar_EPU+_polar_EUU) % _polar_EPP % _polar_EPU % _polar_EUU).str()
        << std::flush << (format("  = -----------------------------------------------------------------------------------")).str()
        << std::flush << (format("  + SUM(E)         = %1$+1.7e = %2$+1.7e  %3$+1.7e  %4$+1.7e eV")
            % _inner % _inner_epp % _inner_eppu % _inner_ework).str()
        << std::flush;
    
    XTP_LOG(Log::info,*_log)            
        << (format("Interaction FGC <> FGC(i) u ***(o)")).str()
        << std::flush << (format("  + Ei [pp+pu+iw]  = %1$+1.7e = %2$+1.7e  %3$+1.7e  %4$+1.7e eV")
            % _inner % _inner_epp % _inner_eppu % _inner_ework).str()
        << std::flush << (format("  + Eo [pp+pu+iw]  = %1$+1.7e = %2$+1.7e  %3$+1.7e  ************** eV")
            % _outer % _outer_epp % _outer_eppu).str()
        << std::flush << (format("  = ===================================================================================")).str()
        << std::flush << (format("  + E  [stat+ind]  = %1$+1.7e = %2$+1.7e  %3$+1.7e eV")
...
void Ewald3DnD::Evaluate() {
    ...
    XTP_LOG(Log::debug,*_log) << std::flush << "Background fields (FGN):" << std::flush;
...
//    EvaluateEnergyQMMM();
    ...
    */
    ...
    // TEASER OUTPUT PERMANENT FIELDS
    XTP_LOG(Log::debug,*_log) << std::flush << "Foreground fields (FGC):" << std::flush;
    ...
    XTP_LOG(Log::debug,*_log) << std::flush << (format("Timing (T = %1$1.2f min)") % (_t_total)) << std::flush;
    XTP_LOG(Log::debug,*_log) << (format("  o Usage <Coarsegrain> = %1$2.2f%%") % (100*_t_coarsegrain/_t_total)) << std::flush;
    XTP_LOG(Log::debug,*_log) << (format("  o Usage <Fields>      = %1$2.2f%%") % (100*_t_fields/_t_total)) << std::flush;
    XTP_LOG(Log::debug,*_log) << (format("  o Usage <Induction>   = %1$2.2f%%") % (100*_t_induction/_t_total)) << std::flush;
    XTP_LOG(Log::debug,*_log) << (format("  o Usage <Energy>      = %1$2.2f%%") % (100*_t_energy/_t_total)) << std::flush;    
    XTP_LOG(Log::debug,*_log) << std::flush;
...
void Ewald3DnD::EvaluateFields(bool do_depolarize_fgc) {
    ...
    XTP_LOG(Log::debug,*_log) << std::flush << "Foreground fields:" << std::flush;
    int fieldCount = 0;
    for (sit1 = _fg_C.begin(); sit1 < _fg_C.end(); ++sit1) {        
        PolarSeg* pseg = *sit1;        
...
void Ewald3DnD::EvaluateInduction() {
    ...
    XTP_LOG(Log::debug,*_log) << std::flush;
    XTP_LOG(Log::debug,*_log) << format("Call inductor on FGC = QM0 u MM1") << std::flush;
    XTP_LOG(Log::debug,*_log) << (format("  o |QM0|, |MM1|, |MM2|        %1$d %2$d %3$d") 
            % _ptop->QM0().size() % _ptop->MM1().size() % _ptop->MM2().size()).str() << std::flush;
    XTP_LOG(Log::debug,*_log) << (format("  o Polarization cut-off:      ")).str() << _polar_cutoff << " nm " << std::flush;
    XTP_LOG(Log::debug,*_log) << (format("  o With induction:            %1$s") % ((_polar_do_induce) ? "yes" : "no")) << std::flush;
    XTP_LOG(Log::debug,*_log) << (format("  o Thole sharpness parameter: ")).str() << _polar_aDamp << std::flush;
    XTP_LOG(Log::debug,*_log) << (format("  o SOR mixing factor:         ")).str() << _polar_wSOR_N << " (N) " << _polar_wSOR_C << " (C) "  << std::flush;
    XTP_LOG(Log::debug,*_log) << (format("  o Iterations (max):          512")).str() << std::flush;
    XTP_LOG(Log::debug,*_log) << (format("  o Tolerance (dU/U):          %1$1.3e") % _polar_epstol).str() << std::flush;
    XTP_LOG(Log::debug,*_log) << (format("  o Induce within QM0:         yes")).str() << std::flush;
    XTP_LOG(Log::debug,*_log) << (format("  o Subthreads:                single")).str() << std::flush;
    XTP_LOG(Log::debug,*_log) << (format("  o Started from archive:      %1$s") % ((_started_from_archived_indu_state) ? "yes" : "no")) << std::flush;
    
    
    if (_started_from_archived_indu_state) {
        XTP_LOG(Log::debug,*_log) << "Reusing induction state from archive." << std::flush;
        std::vector<PolarSeg*>::iterator sit1, sit2;
        PolarSeg::iterator pit1, pit2;
        for (sit1 = _fg_C.begin(), sit2 = _fg_N.begin(); 
            sit1 < _fg_C.end();
...
bool Ewald3DnD::EvaluateInductionQMMM(
    ...
    if (do_reset) {
        XTP_LOG(Log::debug,*_log) << " o Carry out complete depolarization on FGC" << std::flush;
        for (sit = _fg_C.begin(); sit < _fg_C.end(); ++sit) {
        ...
        XTP_LOG(Log::debug,*_log) << " o Carry out partial depolarization on FGC" << std::flush;
        for (sit = _fg_C.begin(); sit < _fg_C.end(); ++sit) {
            for (pit = (*sit)->begin(); pit != (*sit)->end(); ++pit) {
                (*pit)->ResetFieldU();
            }
    ...
    if (do_reuse_bgp_state && _started_from_archived_indu_state) {
        XTP_LOG(Log::debug,*_log) << " o Reuse induction state from archive" << std::flush;
        std::vector<PolarSeg*>::iterator sit1, sit2;
        PolarSeg::iterator pit1, pit2;
    ...
    if (do_calc_perm_fields) {
        XTP_LOG(Log::debug,*_log) << std::flush;
        XTP_LOG(Log::debug,*_log) << "Compute fields for aperiodic embedding" << std::flush;
    ...
    XTP_LOG(Log::debug,*_log) << std::flush;
    XTP_LOG(Log::debug,*_log) << "Start classical SCF with background fields" << std::flush;
    XTP_LOG(Log::debug,*_log) << std::flush;
    _log->DisablePreface();
    ...
    for ( ; n_iter < max_iter+1; ++n_iter) {
        ...
        XTP_LOG(Log::debug,*_log) << (boost::format("o %1$2d") % n_iter) << std::flush;
        if (n_iter % 10 == 0) {
            _log->EnablePreface();
            XTP_LOG(Log::debug,*_log) << std::flush;
            _log->DisablePreface();
...
void Ewald3DnD::EvaluateEnergyQMMM() {    
    ...
    if (Log::verbose()) {
        XTP_LOG(Log::info,*_log) << std::flush;
        XTP_LOG(Log::info,*_log) << "R(MM1) " << EPP_mm1_mgN*int2eV << std::flush;
        XTP_LOG(Log::info,*_log) << "R(MM1) " << EPP_qm0_mgN*int2eV << std::flush;
    }    
    ...
    if (true || Log::verbose()) {
        XTP_LOG(Log::info,*_log) << std::flush;
        XTP_LOG(Log::info,*_log) << "QM-MM splitting for background interaction (eV)" << std::flush;
        XTP_LOG(Log::info,*_log) << "  o E(MM1)       = " << _ET_MM1 << std::flush;
        XTP_LOG(Log::info,*_log) << "  o E(QM0)       = " << _ET_QM0 << std::flush;
        XTP_LOG(Log::info,*_log) << "  o E(QM0 u MM1) = " << _ET_MM1+_ET_QM0 << std::flush;
    }
...
void Ewald3DnD::EvaluateRadialCorrection(std::vector<PolarSeg*> &target) {
    ...
    XTP_LOG(Log::info,*_log) << std::flush;
    XTP_LOG(Log::info,*_log) << "Apply dielectric radial correction" << std::flush;
    XTP_LOG(Log::info,*_log) << "  o Radial screening constant: " 
        << _polar_radial_corr_epsilon << std::flush;
    XTP_LOG(Log::info,*_log) << "  o Radial extension: " << _polar_cutoff << "nm <> "
    ...
    XTP_LOG(Log::debug,*_log) << "  o Add screened field contribution (QM0)" << std::flush;
    for (sit1 = _mg_N.begin(); sit1 != _mg_N.end(); ++sit1) {
        for (pit1 = (*sit1)->begin(); pit1 < (*sit1)->end(); ++pit1) {
            for (sit2 = _polar_qm0.begin(); sit2 != _polar_qm0.end(); ++sit2) {
                for (pit2 = (*sit2)->begin(); pit2 != (*sit2)->end(); ++pit2) {
    ...
    // Evaluate Energy
    XTP_LOG(Log::debug,*_log) << "  o Evaluate stabilization energy" << std::flush;
    _polar_ERC = 0.0;
    for (sit1 = _mg_N.begin(); sit1 != _mg_N.end(); ++sit1) {
        for (pit1 = (*sit1)->begin(); pit1 < (*sit1)->end(); ++pit1) {
            for (sit2 = _polar_qm0.begin(); sit2 != _polar_qm0.end(); ++sit2) {
...
void Ewald3DnD::EvaluatePotential(std::vector<PolarSeg*> &target, bool add_bg, 
    ...
    XTP_LOG(Log::info,*_log) << std::flush << "Potential q*phi " << q_phi*int2eV << std::flush;
...
EWD::triple<> Ewald3DnD::ConvergeRealSpaceSum(std::vector<PolarSeg*> &target) {
    ...
    for (int i = 0; i < 1000; ++i) {
        ...
        XTP_LOG(Log::debug,*_log)
            ...
            % Rc % (this_ER*int2eV) % _mg_N.size() % dER_rms).str() << std::flush;
...
tools::Property Ewald3DnD::GenerateOutputstring() {
    ...
    next->add("xyz", (format("%1$+1.7f %2$+1.7f %3$+1.7f") 
        % _center(0) % _center(1) % _center(2)).str())

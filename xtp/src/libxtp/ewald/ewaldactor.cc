...
EWD::cmplx EwdInteractor::PhiU12_AS1S2_At_By(const vec &k,
                                             std::vector<PolarSeg *> &s1,
                                             std::vector<PolarSeg *> &s2,
                                             double &rV) {
  // ATTENTION Increments *permanent* potential PhiP of s1 only
...
EWD::triple<double> EwdInteractor::U12_ShapeTerm(std::vector<PolarSeg *> &s1,
  ...
  // Traces
  double TrQ2_S1 = Q2_S1.trace();  // Q2_S1(0,0)+Q2_S1.get(1,1)+Q2_S1.get(2,2);
  ...
  if (log != NULL) {
    ...
    XTP_LOG(Log::debug, *log) << "  Q1   = "
                              << (boost::format("%1$+1.7e %2$+1.7e %3$+1.7e") %
                                  Q1_S1(0) % Q1_S1(1) % Q1_S1(2))
                              << std::flush;
    XTP_LOG(Log::debug, *log) << "  U1   = "
                              << (boost::format("%1$+1.7e %2$+1.7e %3$+1.7e") %
                                  U1_S1(0) % U1_S1(1) % U1_S1(2))
                              << std::flush;

    XTP_LOG(Log::debug, *log) << "  Q2   = "
                              << (boost::format("%1$+1.7e %2$+1.7e %3$+1.7e") %
                                  Q2_S1(0, 0) % Q2_S1(0, 1) % Q2_S1(0, 2))
                              << std::flush;
    XTP_LOG(Log::debug, *log) << "         "
                              << (boost::format("%1$+1.7e %2$+1.7e %3$+1.7e") %
                                  Q2_S1(1, 0) % Q2_S1(1, 1) % Q2_S1(1, 2))
                              << std::flush;
    XTP_LOG(Log::debug, *log) << "         "
                              << (boost::format("%1$+1.7e %2$+1.7e %3$+1.7e") %
                                  Q2_S1(2, 0) % Q2_S1(2, 1) % Q2_S1(2, 2))
                              << std::flush;

    XTP_LOG(Log::debug, *log) << "  U2   = "
                              << (boost::format("%1$+1.7e %2$+1.7e %3$+1.7e") %
                                  U2_S1(0, 0) % U2_S1(0, 1) % U2_S1(0, 2))
                              << std::flush;
    XTP_LOG(Log::debug, *log) << "         "
                              << (boost::format("%1$+1.7e %2$+1.7e %3$+1.7e") %
                                  U2_S1(1, 0) % U2_S1(1, 1) % U2_S1(1, 2))
                              << std::flush;
    XTP_LOG(Log::debug, *log) << "         "
                              << (boost::format("%1$+1.7e %2$+1.7e %3$+1.7e") %
                                  U2_S1(2, 0) % U2_S1(2, 1) % U2_S1(2, 2))
                              << std::flush;
    ...
    XTP_LOG(Log::debug, *log) << "  Q1   = "
                              << (boost::format("%1$+1.7e %2$+1.7e %3$+1.7e") %
                                  Q1_S2(0) % Q1_S2(1) % Q1_S2(2))
                              << std::flush;
    XTP_LOG(Log::debug, *log) << "  U1   = "
                              << (boost::format("%1$+1.7e %2$+1.7e %3$+1.7e") %
                                  U1_S2(0) % U1_S2(1) % U1_S2(2))
                              << std::flush;

    XTP_LOG(Log::debug, *log) << "  Q2   = "
                              << (boost::format("%1$+1.7e %2$+1.7e %3$+1.7e") %
                                  Q2_S2(0, 0) % Q2_S2(0, 1) % Q2_S2(0, 2))
                              << std::flush;
    XTP_LOG(Log::debug, *log) << "         "
                              << (boost::format("%1$+1.7e %2$+1.7e %3$+1.7e") %
                                  Q2_S2(1, 0) % Q2_S2(1, 1) % Q2_S2(1, 2))
                              << std::flush;
    XTP_LOG(Log::debug, *log) << "         "
                              << (boost::format("%1$+1.7e %2$+1.7e %3$+1.7e") %
                                  Q2_S2(2, 0) % Q2_S2(2, 1) % Q2_S2(2, 2))
                              << std::flush;

    XTP_LOG(Log::debug, *log) << "  U2   = "
                              << (boost::format("%1$+1.7e %2$+1.7e %3$+1.7e") %
                                  U2_S2(0, 0) % U2_S2(0, 1) % U2_S2(0, 2))
                              << std::flush;
    XTP_LOG(Log::debug, *log) << "         "
                              << (boost::format("%1$+1.7e %2$+1.7e %3$+1.7e") %
                                  U2_S2(1, 0) % U2_S2(1, 1) % U2_S2(1, 2))
                              << std::flush;
    XTP_LOG(Log::debug, *log) << "         "
                              << (boost::format("%1$+1.7e %2$+1.7e %3$+1.7e") %
                                  U2_S2(2, 0) % U2_S2(2, 1) % U2_S2(2, 2))
                              << std::flush;
  ...
  if (shape == "xyslab") {
    ...
    pu =
        Q0_S1 * TrU2_S2 + Q0_S2 * TrU2_S1 - Q1_S1.dot(U1_S2) - Q1_S2.dot(U1_S1);
    uu = -U1_S1.dot(U1_S2);
    pp *= -4 * M_PI / (3 * V);
    pu *= -4 * M_PI / (3 * V);
    uu *= -4 * M_PI / (3 * V);

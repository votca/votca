...
void EwdInteractor::PhiPU12_ShapeField_At_By(std::vector<PolarSeg *> &s1,
  ...
  if (shape == "xyslab") {
    ...
    for (sit = s1.begin(); sit != s1.end(); ++sit) {
      for (pit = (*sit)->begin(); pit != (*sit)->end(); ++pit) {
        (*pit)->PhiP += prefac * (Q2_S2(2, 2) - (*pit)->getPos()(2) * Q1_S2(2));
        (*pit)->PhiU += prefac * (U2_S2(2, 2) - (*pit)->getPos()(2) * U1_S2(2));
      }
    }
  } else if (shape == "cube" || shape == "sphere") {
    double prefac = -4 * M_PI / (3 * V);
    for (sit = s1.begin(); sit != s1.end(); ++sit) {
      for (pit = (*sit)->begin(); pit != (*sit)->end(); ++pit) {
        (*pit)->PhiP += prefac * (TrQ2_S2 - (*pit)->getPos().dot(Q1_S2));
        (*pit)->PhiU += prefac * (TrU2_S2 - (*pit)->getPos().dot(U1_S2));
      }
    }
  } else if (shape == "none") {
    ;  // Nothing to do here
...
void EwdInteractor::PhiPU12_ShapeField_At_By(std::vector<PolarSeg *> &s1,
  ...
  // Increment potentials
  if (shape == "xyslab") {
    double prefac = -4 * M_PI / V * tools::conv::nm2bohr;
    ...
    for (Index i = 0; i < grid.getBoxesSize(); ++i) {
      ...
      for (Index p = 0; p < box.size(); p++) {
        ...
        values[p] += prefac * ((Q2_S2(2, 2) - gridpoint(2) * Q1_S2(2)) +
                               (U2_S2(2, 2) - gridpoint(2) * U1_S2(2)));
      }
    }
  } else if (shape == "cube" || shape == "sphere") {
    double prefac = -4 * M_PI / (3 * V) * tools::conv::nm2bohr;
    ...
    for (Index i = 0; i < grid.getBoxesSize(); ++i) {
      ...
      for (Index p = 0; p < box.size(); p++) {
        ...
        values[p] += prefac * ( (TrQ2_S2 - gridpoint.dot(Q1_S2)) + (TrU2_S2 - gridpoint.dot(U1_S2)));
      }
    }
  }
    else if (shape == "none") {
  ...
  void EwdInteractor::PhiP12_ShapeField_At_By(std::vector<PolarSeg *> & s1,
    ...
    if (shape == "xyslab") {
      ...
      for (sit = s1.begin(); sit != s1.end(); ++sit) {
        for (pit = (*sit)->begin(); pit != (*sit)->end(); ++pit) {
          (*pit)->PhiP +=
              prefac * (Q2_S2(2, 2) - (*pit)->getPos()(2) * Q1_S2(2));
        }
      }
    } else if (shape == "cube" || shape == "sphere") {
      double prefac = -4 * M_PI / (3 * V);
  ...
  void EwdInteractor::PhiU12_ShapeField_At_By(std::vector<PolarSeg *> & s1,
    ...
    if (shape == "xyslab") {
      ...
      for (sit = s1.begin(); sit != s1.end(); ++sit) {
        for (pit = (*sit)->begin(); pit != (*sit)->end(); ++pit) {
          (*pit)->PhiU +=
              prefac * (U2_S2(2, 2) - (*pit)->getPos()(2) * U1_S2(2));
        }
      }
    } else if (shape == "cube" || shape == "sphere") {
      double prefac = -4 * M_PI / (3 * V);
  ...
  //     for (sit = by.begin(); sit < by.end(); ++sit) {
  //         for (pit = (*sit)->begin(); pit < (*sit)->end(); ++pit) {
  //             fz += 2*( (*pit)->Q00*(*pit)->getPos()(2) + (*pit)->U1z);
  //             if ((*pit)->_rank > 0)
  //                 fz += 2*(*pit)->Q1z;
  //         }
  //     }
  ...
  EWD::cmplx EwdInteractor::FP12_At_ByS2(const vec &k,
  ...
  EWD::cmplx EwdInteractor::FU12_At_ByS2(const vec &k,
                                         std::vector<PolarSeg *> &s1,
                                         const EWD::cmplx &S2, double &rV) {
    // ATTENTION Increments PERMANENT fields of s1
    // ATTENTION Structure factor S2 from PERM & INDU moments of s2, k
  ...
  EWD::cmplx EwdInteractor::FPU12_AS1S2_At_By(
      const vec &k, std::vector<PolarSeg *> &s1, std::vector<PolarSeg *> &s2,
      double &rV) {
    // ATTENTION Increments PERMANENT fields of s1
  ...
  EWD::cmplx EwdInteractor::FP12_AS1S2_At_By(
      const vec &k, std::vector<PolarSeg *> &s1, std::vector<PolarSeg *> &s2,
      double &rV) {
    // ATTENTION Increments PERMANENT fields of s1
  ...
  EWD::cmplx EwdInteractor::FU12_AS1S2_At_By(
      const vec &k, std::vector<PolarSeg *> &s1, std::vector<PolarSeg *> &s2,
      double &rV) {
...
EWD::cmplx EwdInteractor::PhiPU12_AS1S2_At_By(
  ...
  EWD::cmplx EwdInteractor::PhiP12_AS1S2_At_By(
      const vec &k, std::vector<PolarSeg *> &s1, std::vector<PolarSeg *> &s2,
      double &rV) {
    // ATTENTION Increments *permanent* potential PhiP of s1 only
  ...
  EWD::cmplx EwdInteractor::PhiU12_AS1S2_At_By(
      const vec &k, std::vector<PolarSeg *> &s1, std::vector<PolarSeg *> &s2,
      double &rV) {
    // ATTENTION Increments *permanent* potential PhiP of s1 only
    // ATTENTION Structure factors include PERMANENT moments of s2 only
  ...
  EWD::triple<EWD::cmplx> EwdInteractor::AS1S2(
      const vec &k, std::vector<PolarSeg *> &s1, std::vector<PolarSeg *> &s2) {
    // NOTE : w/o 1/V
    ApplyBiasK(k);
    ...
    double pu_re_AS1S2 = AK * (u_re_S1 * re_S2 + re_S1 * u_re_S2 -
                               u_im_S1 * im_S2 - im_S1 * u_im_S2);
    double pu_im_AS1S2 = AK * (u_re_S1 * im_S2 + re_S1 * u_im_S2 +
                               u_im_S1 * re_S2 + im_S1 * u_re_S2);
  ...
  EWD::triple<EWD::cmplx> EwdInteractor::S1S2(
      const vec &k, std::vector<PolarSeg *> &s1, std::vector<PolarSeg *> &s2) {
    // NOTE : w/o 1/V
    ApplyBiasK(k);
    ...
    double pu_re_S1S2 =
        (u_re_S1 * re_S2 + re_S1 * u_re_S2 - u_im_S1 * im_S2 - im_S1 * u_im_S2);
    double pu_im_S1S2 =
        (u_re_S1 * im_S2 + re_S1 * u_im_S2 + u_im_S1 * re_S2 + im_S1 * u_re_S2);
  ...
  EWD::triple<double> EwdInteractor::U12_ShapeTerm(
      std::vector<PolarSeg *> & s1, std::vector<PolarSeg *> & s2,
      std::string shape, double V, Logger *log) {
    ...
    vec U1_S1 = vec(0, 0, 0);
    matrix Q2_S1;
    Q2_S1 = Eigen::Matrix3d::Zero();
    matrix U2_S1;
    U2_S1 = Eigen::Matrix3d::Zero();
    ...
    for (sit = s1.begin(); sit != s1.end(); ++sit) {
      for (pit = (*sit)->begin(); pit != (*sit)->end(); ++pit) {
        ...
        // Dipole
        Q1_S1 += q0 * r;
        if ((*pit)->getRank() > 0) Q1_S1 += q1;
        U1_S1 += u1;
        // Quadrupole
        Q2_S1 += 0.5 * q0 * r * r.transpose();
        if ((*pit)->getRank() > 0) {
          Q2_S1 += q1 * r.transpose();
          if ((*pit)->getRank() > 1) {
            Q2_S1 += q2;
          }
        }
        U2_S1 += u1 * r.transpose();
      }
    }
    ...
    // Traces
    double TrQ2_S1 =
        Q2_S1.trace();  // Q2_S1(0,0)+Q2_S1.get(1,1)+Q2_S1.get(2,2);
    double TrU2_S1 = U2_S1.trace();  //(0,0)+U2_S1.get(1,1)+U2_S1.get(2,2);
    double TrQ2_S2 = Q2_S2.trace();  //(0,0)+Q2_S2.get(1,1)+Q2_S2.get(2,2);
    double TrU2_S2 = U2_S2.trace();  //(0,0)+U2_S2.get(1,1)+U2_S2.get(2,2);

    if (log != NULL) {
      XTP_LOG(Log::debug, *log) << "S1 moments: " << std::flush;
      XTP_LOG(Log::debug, *log)
          << "  Q0   = " << (boost::format("%1$+1.7e") % Q0_S1) << std::flush;
      XTP_LOG(Log::debug, *log)
          << "  Q1   = "
          << (boost::format("%1$+1.7e %2$+1.7e %3$+1.7e") % Q1_S1(0) %
              Q1_S1(1) % Q1_S1(2))
          << std::flush;
      XTP_LOG(Log::debug, *log)
          << "  U1   = "
          << (boost::format("%1$+1.7e %2$+1.7e %3$+1.7e") % U1_S1(0) %
              U1_S1(1) % U1_S1(2))
          << std::flush;

      XTP_LOG(Log::debug, *log)
          << "  Q2   = "
          << (boost::format("%1$+1.7e %2$+1.7e %3$+1.7e") % Q2_S1(0, 0) %
              Q2_S1(0, 1) % Q2_S1(0, 2))
          << std::flush;
      XTP_LOG(Log::debug, *log)
          << "         "
          << (boost::format("%1$+1.7e %2$+1.7e %3$+1.7e") % Q2_S1(1, 0) %
              Q2_S1(1, 1) % Q2_S1(1, 2))
          << std::flush;
      XTP_LOG(Log::debug, *log)
          << "         "
          << (boost::format("%1$+1.7e %2$+1.7e %3$+1.7e") % Q2_S1(2, 0) %
              Q2_S1(2, 1) % Q2_S1(2, 2))
          << std::flush;

      XTP_LOG(Log::debug, *log)
          << "  U2   = "
          << (boost::format("%1$+1.7e %2$+1.7e %3$+1.7e") % U2_S1(0, 0) %
              U2_S1(0, 1) % U2_S1(0, 2))
          << std::flush;
      XTP_LOG(Log::debug, *log)
          << "         "
          << (boost::format("%1$+1.7e %2$+1.7e %3$+1.7e") % U2_S1(1, 0) %
              U2_S1(1, 1) % U2_S1(1, 2))
          << std::flush;
      XTP_LOG(Log::debug, *log)
          << "         "
          << (boost::format("%1$+1.7e %2$+1.7e %3$+1.7e") % U2_S1(2, 0) %
              U2_S1(2, 1) % U2_S1(2, 2))
          << std::flush;

      XTP_LOG(Log::debug, *log) << "  TrQ2 = " << TrQ2_S1 << std::flush;
      XTP_LOG(Log::debug, *log) << "  TrU2 = " << TrU2_S1 << std::flush;

      XTP_LOG(Log::debug, *log) << "S2 moments: " << std::flush;
      XTP_LOG(Log::debug, *log)
          << "  Q0   = " << (boost::format("%1$+1.7e") % Q0_S2) << std::flush;
      XTP_LOG(Log::debug, *log)
          << "  Q1   = "
          << (boost::format("%1$+1.7e %2$+1.7e %3$+1.7e") % Q1_S2(0) %
              Q1_S2(1) % Q1_S2(2))
          << std::flush;
      XTP_LOG(Log::debug, *log)
          << "  U1   = "
          << (boost::format("%1$+1.7e %2$+1.7e %3$+1.7e") % U1_S2(0) %
              U1_S2(1) % U1_S2(2))
          << std::flush;

      XTP_LOG(Log::debug, *log)
          << "  Q2   = "
          << (boost::format("%1$+1.7e %2$+1.7e %3$+1.7e") % Q2_S2(0, 0) %
              Q2_S2(0, 1) % Q2_S2(0, 2))
          << std::flush;
      XTP_LOG(Log::debug, *log)
          << "         "
          << (boost::format("%1$+1.7e %2$+1.7e %3$+1.7e") % Q2_S2(1, 0) %
              Q2_S2(1, 1) % Q2_S2(1, 2))
          << std::flush;
      XTP_LOG(Log::debug, *log)
          << "         "
          << (boost::format("%1$+1.7e %2$+1.7e %3$+1.7e") % Q2_S2(2, 0) %
              Q2_S2(2, 1) % Q2_S2(2, 2))
          << std::flush;

      XTP_LOG(Log::debug, *log)
          << "  U2   = "
          << (boost::format("%1$+1.7e %2$+1.7e %3$+1.7e") % U2_S2(0, 0) %
              U2_S2(0, 1) % U2_S2(0, 2))
          << std::flush;
      XTP_LOG(Log::debug, *log)
          << "         "
          << (boost::format("%1$+1.7e %2$+1.7e %3$+1.7e") % U2_S2(1, 0) %
              U2_S2(1, 1) % U2_S2(1, 2))
          << std::flush;
      XTP_LOG(Log::debug, *log)
          << "         "
          << (boost::format("%1$+1.7e %2$+1.7e %3$+1.7e") % U2_S2(2, 0) %
              U2_S2(2, 1) % U2_S2(2, 2))
          << std::flush;
      XTP_LOG(Log::debug, *log) << "  TrQ2 = " << TrQ2_S2 << std::flush;
      XTP_LOG(Log::debug, *log) << "  TrU2 = " << TrU2_S2 << std::flush;
    }
    ...
    if (shape == "xyslab") {
      pp = Q0_S1 * Q2_S2(2, 2) + Q0_S2 * Q2_S1(2, 2) - Q1_S1(2) * Q1_S2(2);
      pu = Q0_S1 * U2_S2(2, 2) + Q0_S2 * U2_S1(2, 2) - Q1_S1(2) * U1_S2(2) -
           Q1_S2(2) * U1_S1(2);
      uu = -U1_S1(2) * U1_S2(2);
      pp *= -4 * M_PI / V;
      pu *= -4 * M_PI / V;
      ...
      double dd =
          -Q1_S1(2) * Q1_S2(2) - Q1_S1(2) * U1_S2(2) - Q1_S2(2) * U1_S1(2);
      double qQ = Q0_S1 * Q2_S2(2, 2) + Q0_S2 * Q2_S1(2, 2) +
                  Q0_S1 * U2_S2(2, 2) + Q0_S2 * U2_S1(2, 2);
      dd *= -4 * M_PI / V * int2eV;
      qQ *= -4 * M_PI / V * int2eV;

      XTP_LOG(Log::debug, *log)
          << (boost::format("  DD %1$+1.7e eV qQ %2$+1.7e eV") % dd % qQ)
          << std::flush;

    } else if (shape == "cube" || shape == "sphere") {
      pp = Q0_S1 * TrQ2_S2 + Q0_S2 * TrQ2_S1 - Q1_S1.dot(Q1_S2);
      pu = Q0_S1 * TrU2_S2 + Q0_S2 * TrU2_S1 - Q1_S1.dot(U1_S2) -
           Q1_S2.dot(U1_S1);
      uu = -U1_S1.dot(U1_S2);
      pp *= -4 * M_PI / (3 * V);
      pu *= -4 * M_PI / (3 * V);
      ...
      if (log != NULL) {
        ...
        double pp_dd = -Q1_S1.dot(Q1_S2);
        double pu_qQ = Q0_S1 * TrU2_S2 + Q0_S2 * TrU2_S1;
        double pu_dd = -Q1_S1.dot(U1_S2) - Q1_S2.dot(U1_S1);
        double uu_dd = -U1_S1.dot(U1_S2);
        pp_qQ *= -4 * M_PI / (3 * V) * int2eV;
        pp_dd *= -4 * M_PI / (3 * V) * int2eV;
        pu_qQ *= -4 * M_PI / (3 * V) * int2eV;
        pu_dd *= -4 * M_PI / (3 * V) * int2eV;
        uu_dd *= -4 * M_PI / (3 * V) * int2eV;
        XTP_LOG(Log::debug, *log)
            << (boost::format("Energy-moment decomposition")) << std::flush;
        XTP_LOG(Log::debug, *log)
            << (boost::format("  qQ (pp) = %1$+1.7e") % pp_qQ) << std::flush;
        XTP_LOG(Log::debug, *log)
            << (boost::format("  dd (pp) = %1$+1.7e") % pp_dd) << std::flush;
        XTP_LOG(Log::debug, *log)
            << (boost::format("  qQ (pu) = %1$+1.7e") % pu_qQ) << std::flush;
        XTP_LOG(Log::debug, *log)
            << (boost::format("  dd (pu) = %1$+1.7e") % pu_dd) << std::flush;
        XTP_LOG(Log::debug, *log)
            << (boost::format("  dd (uu) = %1$+1.7e") % uu_dd) << std::flush;
      }
    } else if (shape == "none") {
      XTP_LOG(Log::debug, *log)
          << (boost::format("Assuming isotropic limit")) << std::flush;
      pp = 0.0;
      pu = 0.0;
      uu = 0.0;
    } else {
    ...
    // cout << std::endl << "S1 " << Q0_S1 << " " << Q1_S1 << " " << TrQ2_S1 <<
    // std::flush; cout << std::endl << "S2 " << Q0_S2 << " " << Q1_S2 << " " <<
    // TrQ2_S2 << std::flush;

...
void EwdInteractor::FPU12_ShapeField_At_By(std::vector<PolarSeg*> &at,
    std::vector<PolarSeg*> &by, std::string shape, double V) {
...
void EwdInteractor::PhiPU12_ShapeField_At_By(std::vector<PolarSeg*> &s1, 
    std::vector<PolarSeg*> &s2, std::string shape, double V) {
...
void EwdInteractor::PhiU12_ShapeField_At_By(std::vector<PolarSeg*> &s1, 
    ...
    if (shape == "xyslab") {
        ...
        for (sit = s1.begin(); sit != s1.end(); ++sit) {
            for (pit = (*sit)->begin(); pit != (*sit)->end(); ++pit) {           
                (*pit)->PhiU += prefac*(U2_S2.get(2,2) - (*pit)->getPos()(2)*U1_S2(2));  
            }
        }
    }
    else if (shape == "cube" || shape == "sphere") {
...
EWD::triple<double> EwdInteractor::U12_ShapeTerm(std::vector<PolarSeg*> &s1,
    std::vector<PolarSeg*> &s2, std::string shape, double V, Logger *log) {
    ...
    if (log != NULL) {
        ...
        CTP_LOG(logDEBUG, *log) << "  Q1   = " 
            << (boost::format("%1$+1.7e %2$+1.7e %3$+1.7e") 
                % Q1_S1(0) % Q1_S1(1) % Q1_S1(2)) << flush;
        CTP_LOG(logDEBUG, *log) << "  U1   = " 
            << (boost::format("%1$+1.7e %2$+1.7e %3$+1.7e") 
                % U1_S1(0) % U1_S1(1) % U1_S1(2)) << flush;
        ...
        CTP_LOG(logDEBUG, *log) << "  Q1   = " 
            << (boost::format("%1$+1.7e %2$+1.7e %3$+1.7e") 
                % Q1_S2(0) % Q1_S2(1) % Q1_S2(2)) << flush;
        CTP_LOG(logDEBUG, *log) << "  U1   = " 
            << (boost::format("%1$+1.7e %2$+1.7e %3$+1.7e") 
                % U1_S2(0) % U1_S2(1) % U1_S2(2)) << flush;
    ...
    //cout << std::endl << "S1 " << Q0_S1 << " " << Q1_S1 << " " << TrQ2_S1 << flush;
    //cout << std::endl << "S2 " << Q0_S2 << " " << Q1_S2 << " " << TrQ2_S2 << flush;

...
PolarBackground::PolarBackground(Topology *top, PolarTop *ptop, tools::Property opt, 
    ...
    XTP_LOG(Log::info,*_log)
        ...
        << std::flush << (format("  o Q(BGP) = %1$+1.3fe |BGP| = %2$+5d") % Q_bg_P % _bg_P.size())
        << std::flush << (format("  o Activity <qdQ %1$d/%3$d> <P %2$d/%3$d>") % estat_count % polar_count % _bg_P.size())
        << std::flush;
    ...
    if (_do_compensate_net_dipole) {
        XTP_LOG(Log::info,*_log) << (format("  o Dpl. compensation: type '%1$s', "
                "direction '%2$s' ") % _dipole_compensation_type 
                % _dipole_compensation_direction) << std::flush;
        ...
        XTP_LOG(Log::info,*_log) << "    - System Q1: " << - system_dpl 
...
void PolarBackground::Checkpoint(int iter, bool converged) {
    XTP_LOG(Log::debug,*_log) << "  o Checkpointing (iteration " << iter << ") ... ";
...
void PolarBackground::Polarize(int n_threads = 1) {
    ...
    XTP_LOG(Log::debug,*_log) << "  o Real-space unit cell:      " << _a << " x " << _b << " x " << _c << std::flush;
    XTP_LOG(Log::debug,*_log) << "  o Real-space c/o (guess):    " << _R_co << " nm" << std::flush;
    XTP_LOG(Log::debug,*_log) << "  o na(max), nb(max), nc(max): " << _na_max << ", " << _nb_max << ", " << _nc_max << std::flush;
    XTP_LOG(Log::debug,*_log) << "  o 1st Brillouin zone:        " << _A << " x " << _B << " x " << _C << std::flush;
    XTP_LOG(Log::debug,*_log) << "  o Reciprocal-space c/o:      " << _K_co << " 1/nm" << std::flush;
    XTP_LOG(Log::debug,*_log) << "  o R-K switching param.       " << _alpha << " 1/nm" << std::flush;
    XTP_LOG(Log::debug,*_log) << "  o Unit-cell volume:          " << _LxLyLz << " nm**3" << std::flush;
    XTP_LOG(Log::debug,*_log) << "  o LxLy (for 3D2D EW):        " << _LxLy << " nm**2" << std::flush;
    XTP_LOG(Log::debug,*_log) << "  o kx(max), ky(max), kz(max): " << _NA_max << ", " << _NB_max << ", " << _NC_max << std::flush;    
    ...
    */
    ...
    if (!_do_restart) {
        ...
        if (!_do_use_cutoff) {
            ...
            // I.C Shape fields
            XTP_LOG(Log::debug,log) << "  o Shape fields ('" << _shape << "')" << std::flush;
        ...
        for (sit1 = _bg_P.begin()+16; sit1 < _bg_P.end(); ++sit1) {
            ...
            XTP_LOG(Log::debug,*_log) << "ID = " << pseg->getId() << " (" << seg.getId() << ") " << std::flush;
        ...
        XTP_LOG(Log::debug,log) << std::flush << "Induce to first order" << std::flush;
        for (sit1 = _bg_P.begin(); sit1 < _bg_P.end(); ++sit1) {
            for (pit1 = (*sit1)->begin(); pit1 < (*sit1)->end(); ++pit1) {
                (*pit1)->InduceDirect();
            }
        ...
        XTP_LOG(Log::debug,log) << std::flush;
        XTP_LOG(Log::debug,log) << "Restarting from checkpoint => "
    ...
    XTP_LOG(Log::debug,log) << "  o Setup real-space neighbours at iteration " << setup_nbs_iter << std::flush;
    XTP_LOG(Log::debug,log) << "  o Generate k-vectors at iteration " << generate_kvecs_iter << std::flush;
    ...
    for ( ; iter != max_iter; ++iter) {
        ...
        XTP_LOG(Log::debug,log) << "  o (Re-)generate induction fields" << std::flush;
    ...
    if (iter == max_iter) {
    	XTP_LOG(Log::debug,log) << std::flush;
    	XTP_LOG(Log::debug,log) << "Reached maximum number of iterations. Stop here." << std::flush;
    }
...
void PolarBackground::RThread::FU_FieldCalc() {
    ...
    if (_do_setup_nbs) {
        ...
        // CLEAR POLAR NEIGHBOR-LIST BEFORE SET-UP
        if (Log::verbose()) { XTP_LOG(Log::debug,*(_master->_log)) << "   - Clearing polar nb-list" << std::endl; }
        for (sit1 = _part_bg_P.begin(); sit1 < _part_bg_P.end(); ++sit1) {
            (*sit1)->ClearPolarNbs();
        }
        ...
        for (sit1 = _part_bg_P.begin(); sit1 < _part_bg_P.end(); ++sit1) {
            PolarSeg *pseg1 = *sit1;
            if (Log::verbose()) { XTP_LOG(Log::debug,*(_master->_log)) << "\rMST DBG     - Progress " << pseg1->getId() << "/" << _full_bg_P.size() << std::flush; }
...
void PolarBackground::FX_RealSpace(std::string mode, bool do_setup_nbs) {
    ...
    for (tfit = tforce.begin(); tfit != tforce.end(); ++tfit) {
        XTP_LOG(Log::debug,*_log) << (format("%1$1.2f%% ") % (*tfit)->Workload(mode));
    }
    XTP_LOG(Log::debug,*_log) << "]" << std::flush;
    
    // Start & wait
    XTP_LOG(Log::debug,*_log) << "    - Start & wait until done" << std::flush << std::flush;
    ...
    if (not_converged_count == 0) XTP_LOG(Log::debug,*_log) << "    - Converged" << std::flush;
    else XTP_LOG(Log::error,*_log) << "    - ERROR " << not_converged_count 
    ...
    XTP_LOG(Log::debug,*_log) << "    - Real-space nb-list set: <R(c/o)> = " 
        << avg_R_co << std::flush;
    int total_nbs_count = 0;
    std::vector<PolarSeg*>::iterator sit1;
    for (sit1 = _bg_P.begin(); sit1 < _bg_P.end(); ++sit1)
        total_nbs_count += (*sit1)->PolarNbs().size();
    XTP_LOG(Log::debug,*_log) << "    - Real-space nb-list set: <nbs/seg> = " 
...
void PolarBackground::FX_ReciprocalSpace(std::string mode1, std::string mode2, 
    ...
    if (generate_kvecs) {
        XTP_LOG(Log::debug,*_log)
            << "  o Generate k-vectors" << std::flush;
        _log->setPreface(Log::debug, "\nMST DBG     - ");
    ...
    if (_kvecs_2_0.size() > 0) { XTP_LOG(Log::debug,*_log)
...
void PolarBackground::GenerateKVectors(std::vector<PolarSeg*> &ps1, 
    ...
    double kxyz_s1s2_norm = 1./pow(avg_kx_s1s2*avg_ky_s1s2*avg_kz_s1s2,2./3.) * int2eV / _LxLyLz;

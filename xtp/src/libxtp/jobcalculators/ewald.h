...
Job::JobResult Ewald<EwaldMethod>::EvalJob(const Topology& top, Job& job,
    ...
    std::vector<std::string> xjobmps = xjob.getSegMps(); // this is a nasty hack to get the state of the foreground
    std::cout << "MPS FILE OF THIS JOB: " << xjobmps[0] << std::endl;
    std::cout << "!!!!! STILL NEED TO DETERMINE QMSTATE FROM HERE !!!!! " <<  std::endl; 
    ...
    if (_polar_bg_arch == "") {
        XTP_LOG(Log::info,log) << "Mps-Mapper: Generate FGC FGN BGN" << std::flush;
        BackgroundRegion BGN(0, log);
        BackgroundRegion FGN(1, log);
        BackgroundRegion FGC(2, log);
        ...
        for (auto segment : top.Segments()) {
            ...
            if (xjob.isInCenter(segment.getId())){
                FGN.push_back(mol);
                PolarSegment mol_x = polmap.map(segment,SegId(seg_index, "h")); // HOLE HERE TODO DETERMINE FROM INPUT
                FGC.push_back(mol_x); //do we need another mapped molecule here?
            }
            else {
                BGN.push_back(mol);
            }
    ...
    ptop.setFGN(fgN);
    ptop.setBGN(bgN);    
    ptop.setSegsFGC(segs_fgC);
    ptop.setSegsFGN(segs_fgN);
    ptop.setSegsBGN(segs_bgN);    
    // Center polar topology
    vec center = xjob.Center();
    ptop.CenterAround(center);
    xjob.setPolarTop(&ptop);

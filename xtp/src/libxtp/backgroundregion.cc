...
void BackgroundRegion::Initialize(const tools::Property& prop) {
  max_iter_ = prop.get("max_iter").as<Index>();
  deltaD_ = prop.get("tolerance_dipole").as<double>();
  deltaE_ = prop.get("tolerance_energy").as<double>();
  exp_damp_ = prop.get("exp_damp").as<double>();
}

bool BackgroundRegion::Converged() const {
...
/*double BakgroundRegion::StaticInteraction() {
...
void BackgroundRegion::AppendResult(tools::Property& prop) const {
...
void BackgroundRegion::WriteToCpt(CheckpointWriter& w) const {

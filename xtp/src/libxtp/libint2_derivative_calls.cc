...
// once.
[[noreturn, maybe_unused]] void ThrowNoDerivativeSupport(
    const char* function_name, const char* configure_flag) {
...
// engine.impl.h).
//
// CRITICAL, confirmed via a real crash: these macros are NOT bare
// on/off flags -- they are defined WITH THE ACTUAL CONFIGURED
// DERIVATIVE ORDER as their value (confirmed directly: libint2's own
// test suite uses "#if INCLUDE_ERI2 >= 1" and
// "if (deriv_order > INCLUDE_ERI2) return true;", checking the VALUE,
// not just definedness). LIBINT2_ENABLE_ONEBODY defaults to 0 (not
// -1/off) if never explicitly set -- meaning one-body integrals ARE
// built, but only at deriv_order=0 (energies only, no first
// derivatives) -- exactly the situation the standard Homebrew libint
// formula leaves onebody in, since it only ever sets
// -DLIBINT2_ENABLE_ERI2=1/_ERI3=1/_ERI=1, never _ONEBODY. In that
// configuration LIBINT_INCLUDE_ONEBODY is defined AS 0, so
// "defined(LIBINT_INCLUDE_ONEBODY)" alone is TRUE (wrongly implying
// support) while the correct check, "LIBINT_INCLUDE_ONEBODY >= 1", is
// FALSE. The former passed this exact guard once, wrongly, on a real
// CI run -- letting the real, deriv_order=1 implementation compile in
// and then hit a null function-pointer assertion deep inside
// libint2's own dispatch table at runtime (a hard SIGABRT, not a C++
// exception -- uncatchable, exactly like the null-buffer segfault
// this guard was originally added to prevent). Checking the value,
// not just definedness, is what actually distinguishes "not
// configured at all" from "configured, but only for energies."
//
...
// default configuration leaves ERI2/ERI3 off while ONEBODY is on; the
// standard Homebrew libint formula is the mirror image -- ERI2/ERI3/ERI
// on, ONEBODY at its default-0/energies-only state).
#if defined(LIBINT2_MAX_DERIV_ORDER) && LIBINT2_MAX_DERIV_ORDER >= 1 && \
    (LIBINT_INCLUDE_ONEBODY >= 1)
...
std::vector<ThreeCenterDerivative> ComputeThreeCenterDerivatives(
  ...
  ThrowNoDerivativeSupport("ComputeThreeCenterDerivatives", "--enable-eri3=1");
}
...
#if defined(LIBINT2_MAX_DERIV_ORDER) && LIBINT2_MAX_DERIV_ORDER >= 1 && \
    (LIBINT_INCLUDE_ONEBODY >= 1) && (LIBINT_INCLUDE_ERI2 >= 1) &&      \
    (LIBINT_INCLUDE_ERI3 >= 1)
  return true;
#else
  return false;
#endif

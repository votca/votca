...
void AOReactionField::Fill(const AOBasis& aobasis) {
    ...
    // something to change later to option
    std::string grid_accuracy = "fine";

    // prepare the integration grid
    Vxc_Grid grid;
    grid.GridSetup(grid_accuracy, atoms_, aobasis);
    DensityIntegration<Vxc_Grid> numint(grid); 


    // go over all basis functions
    for (Index col = 0 ; col < aobasis.AOBasisSize(); col++){
       // calculate the reaction field for this basis function
       // how to access a single function? aobasis.
        //numint.IntegrateBasisFunction();
        //Apply field of this function to polar segments

       // integrate the reaction field for beta with beta'

...
BOOST_AUTO_TEST_CASE(tb_output) {
  ...
  for (const std::string& carrier :
       std::vector<std::string>{"hole", "electron"}) {
  ...
  auto hole_mosA =
      output.Select("dftcoupling.hole.monomer_energies.fragmentA.mo");
  ...
  auto elec_mosA =
      output.Select("dftcoupling.electron.monomer_energies.fragmentA.mo");
  ...
  auto hole_mosB =
      output.Select("dftcoupling.hole.monomer_energies.fragmentB.mo");
  ...
  // -------------------------------------------------------------------------
  {
    ...
    BOOST_CHECK_CLOSE(max_hab, 26.35484999999999,
                      1.0);  // within 1%, eV — PLACEHOLDER
  }

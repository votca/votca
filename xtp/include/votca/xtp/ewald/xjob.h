...
class XJob {
 public:
  XJob(int id, std::string tag, std::vector<const Segment *> &qmSegs,
  ...
  std::vector<const Segment *> &getSegments() { return _qmSegs; }
  ...
  tools::Property GenerateOutputProperty();
  ...
  void setInduIter(int iter) { _iter = iter; }
  ...
  void setEnergy(double E_Tot, double E_Pair_Pair, double E_Pair_Sph1,
                 double E_Pair_Sph2, double E_Sph1_Sph1, double E_Sph1_Sph2,
  ...
  void setEnergy_f_m(double e_f_c_c, double e_f_c_non_c, double e_f_c_out,
                     double e_f_non_c_non_c, double e_f_non_c_out, double e_m_c,
                     double e_m_non_c, double e_m_out) {
  ...
  void setEnergy_QMMM(double energy_QM, double energy_GWBSE, double energy_SF,
                      double energy_QMMM) {
 ...
 private:
  ...
  std::vector<const Segment *> _qmSegs;
...
inline bool XJob::isWithinDist(const vec &pt, double dist, Topology *top) {
  ...
  for (unsigned int i = 0; i < _qmSegs.size(); ++i) {
    ...
    double dR = (top->PbShortestConnect(seg->getPos(), pt)).norm();
    if (dR <= dist) {
      inCenter = true;
      break;
    }

...
class XInductor {
 public:
  ...
  XInductor(Topology *top, tools::Property *opt, std::string sfx, int nst,
            bool mav);
  ...
  class InduWorker : public tools::Thread {
   public:
    InduWorker(int id, const Topology *top, XInductor *forker)
        : _id(id), /*_top(top),*/ _forker(forker), _qmm(NULL), _mm2(NULL) {
    ...
    void InterEnergy() {
      ...
      for (int i = _nx1; i < _nx2; ++i) {
        for (int j = (i >= _ny1) ? i + 1 : _ny1; j < _ny2; ++j) {
          ...
          bool i_inCenter = _forker->_job->isInCenter((*_qmm)[i]->getId());
          bool j_inCenter = _forker->_job->isInCenter((*_qmm)[j]->getId());

          // Pair-non-pair interaction
          if (i_inCenter ^ j_inCenter) {

            for (pit1 = (*_qmm)[i]->begin(); pit1 < (*_qmm)[i]->end(); ++pit1) {
              for (pit2 = (*_qmm)[j]->begin(); pit2 < (*_qmm)[j]->end();
                   ++pit2) {
        ...
        for (unsigned int j = i; j < _xy_done[i].size(); ++j) {
          if (!_xy_done[i][j]) {
            ...
            if (_chunks_avail[i] && _chunks_avail[j]) {
              ...
              _indus[indu_id]->Setnx12ny12(_nx1[i][j], _nx2[i][j], _ny1[i][j],
                                           _ny2[i][j]);

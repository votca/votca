...
class PolarTop {
 ...
 public:
  ...
  void setQM0(std::vector<PolarSeg *> &qm0, bool clean = true) {
    _qm0 = qm0;
    _clean_qm0 = clean;
  }
  void setMM1(std::vector<PolarSeg *> &mm1, bool clean = true) {
    _mm1 = mm1;
    _clean_mm1 = clean;
  }
  void setMM2(std::vector<PolarSeg *> &mm2, bool clean = true) {
    _mm2 = mm2;
    _clean_mm2 = clean;
  }
  ...
  void setBGN(std::vector<PolarSeg *> &bgN, bool clean = true) {
    _bgN = bgN;
    _clean_bgN = clean;
  }
  void setFGN(std::vector<PolarSeg *> &fgN, bool clean = true) {
    _fgN = fgN;
    _clean_fgN = clean;
  }
  void setFGC(std::vector<PolarSeg *> &fgC, bool clean = true) {
    _fgC = fgC;
    _clean_fgC = clean;
  }
  ...
  void PrintInduState(std::string out, std::string format, bool split,
                      double space);
  void PrintInduState(FILE *out, std::string format, bool split, double space);

  // POLARIZATION STATE
  void setPolarizationIter(int iter, bool converged) {
    _polarization_iter = iter;
    _polarization_converged = converged;
  }

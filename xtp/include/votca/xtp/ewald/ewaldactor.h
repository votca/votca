...
class EwdInteractor {
 public:
  ...
  // / 1.000e-9; static const double int2V_m = 1/(4*M_PI*8.854187817e-12)
  // * 1.602176487e-19 / 1.000e-18;
  ...
  inline double L7() {
    return 1 - (1 + ta1 * tu3 + 0.6 * ta2 * tu3 * tu3) * exp(-ta1 * tu3);
  }
  inline double L9() {
    return 1 - (1 + ta1 * tu3 +
                (18 * ta2 * tu3 * tu3 + 9 * ta3 * tu3 * tu3 * tu3) / 35) *
                   exp(-ta1 * tu3);
  }
  ...
  inline double gB1() {
    return rR2 * (gB0() + 2 * a1 * rSqrtPi * exp(-a2 * R2));
  }
  inline double gB2() {
    return rR2 * (3 * gB1() + 4 * a3 * rSqrtPi * exp(-a2 * R2));
  }
  inline double gB3() {
    return rR2 * (5 * gB2() + 8 * a5 * rSqrtPi * exp(-a2 * R2));
  }
  inline double gB4() {
    return rR2 * (7 * gB3() + 16 * a7 * rSqrtPi * exp(-a2 * R2));
  }
  // Make sure to set R1, R2, ... and rR1, rR2, ... before using {gC0, ...}
  inline void UpdateAllCls();
  inline double gC0() { return erf(a1 * R1) * rR1; }
  inline double gC1() {
    return rR2 * (gC0() - 2 * a1 * rSqrtPi * exp(-a2 * R2));
  }
  inline double gC2() {
    return rR2 * (3 * gC1() - 4 * a3 * rSqrtPi * exp(-a2 * R2));
  }
  inline double gC3() {
    return rR2 * (5 * gC2() - 8 * a5 * rSqrtPi * exp(-a2 * R2));
  }
  inline double gC4() {
    return rR2 * (7 * gC3() - 16 * a7 * rSqrtPi * exp(-a2 * R2));
  }
  ...
  void FPU12_ShapeField_At_By(std::vector<PolarSeg *> &at,
                              std::vector<PolarSeg *> &by, std::string shape,
                              double volume);
  void FP12_ShapeField_At_By(std::vector<PolarSeg *> &at,
                             std::vector<PolarSeg *> &by, std::string shape,
                             double volume);
  void FU12_ShapeField_At_By(std::vector<PolarSeg *> &at,
                             std::vector<PolarSeg *> &by, std::string shape,
                             double volume);
  ...
  void PhiP12_ShapeField_At_By(std::vector<PolarSeg *> &at,
                               std::vector<PolarSeg *> &by, std::string shape,
                               double volume);
  void PhiU12_ShapeField_At_By(std::vector<PolarSeg *> &at,
                               std::vector<PolarSeg *> &by, std::string shape,
                               double volume);
  ...
  // Reciprocal-space K=0 shape correction term P1 <> P2
  EWD::triple<double> U12_ShapeTerm(std::vector<PolarSeg *> &s1,
                                    std::vector<PolarSeg *> &s2,
                                    std::string shape, double volume,
                                    Logger *log = NULL);
  ...
  // Fields
  EWD::cmplx FPU12_AS1S2_At_By(const vec &k, std::vector<PolarSeg *> &s1,
                               std::vector<PolarSeg *> &s2, double &rV);
  EWD::cmplx FP12_AS1S2_At_By(const vec &k, std::vector<PolarSeg *> &s1,
                              std::vector<PolarSeg *> &s2, double &rV);
  EWD::cmplx FU12_AS1S2_At_By(const vec &k, std::vector<PolarSeg *> &s1,
                              std::vector<PolarSeg *> &s2, double &rV);

  EWD::cmplx FP12_At_ByS2(const vec &k, std::vector<PolarSeg *> &s1,
                          const EWD::cmplx &S2, double &rV);
  EWD::cmplx FU12_At_ByS2(const vec &k, std::vector<PolarSeg *> &s1,
                          const EWD::cmplx &S2, double &rV);
  ...
  // Energies
  EWD::triple<EWD::cmplx> AS1S2(const vec &k, std::vector<PolarSeg *> &s1,
                                std::vector<PolarSeg *> &s2);
 ...
 private:
  ...
  double ta1, ta2, ta3;
  double tu3 = 0;
  double l3 = 0, l5 = 0, l7 = 0,
         l9 = 0;  // (damps fields originating in indu. m'poles)
  double lp3 = 0, lp5 = 0, lp7 = 0,
         lp9 = 0;  // (damps fields originating in perm. m'poles)
  ...
  const double int2eV =
      1 / (4 * M_PI * 8.854187817e-12) * 1.602176487e-19 / 1.000e-9;

  // ========================== RECIPROCAL SPACE ========================== //

  // k-space vector
  vec k12 = Eigen::Vector3d::Zero();
  double K = 0.0;
  double AK = 0.0;
...
inline void EwdInteractor::ApplyBiasPolar(APolarSite &p1, APolarSite &p2) {
  ...
  tu3 = R3 / sqrt(1. / 3. *
                  (p1.Pxx * p2.Pxx + p1.Pxy * p2.Pxy + p1.Pxz * p2.Pxz +
...
inline void EwdInteractor::UpdateAllGls(APolarSite &p1, APolarSite &p2) {
  ...
  if (p1._rank > 0) {
    ...
    if (p1._rank > 1) {
      Q1__R = (p1.Qxx * rxx + 2 * p1.Qxy * rxy + 2 * p1.Qxz * rxz +
               p1.Qyy * ryy + 2 * p1.Qyz * ryz + p1.Qzz * rzz);
  ...
  if (p1._rank > 0) {
    ...
    if (p1._rank > 1) {
      ...
      if (p2._rank > 0) {
        ppG2 += -2 * (p1.Qxx * p2.Q1x * rx + p1.Qxy * p2.Q1x * ry +
                      p1.Qxz * p2.Q1x * rz + p1.Qxy * p2.Q1y * rx +
                      p1.Qyy * p2.Q1y * ry + p1.Qyz * p2.Q1y * rz +
                      p1.Qxz * p2.Q1z * rx + p1.Qyz * p2.Q1z * ry +
                      p1.Qzz * p2.Q1z * rz);
        puG2 += -2 * (p1.Qxx * p2.U1x * rx + p1.Qxy * p2.U1x * ry +
                      p1.Qxz * p2.U1x * rz + p1.Qxy * p2.U1y * rx +
                      p1.Qyy * p2.U1y * ry + p1.Qyz * p2.U1y * rz +
                      p1.Qxz * p2.U1z * rx + p1.Qyz * p2.U1z * ry +
                      p1.Qzz * p2.U1z * rz);
        ppG3 += mu2_r * Q1__R;
        puG3 += u_mu2_r * Q1__R;
        ...
        if (p2._rank > 1) {
          ppG2 +=
              2 * (p1.Qxx * p2.Qxx + 2 * p1.Qxy * p2.Qxy + 2 * p1.Qxz * p2.Qxz +
                   p1.Qyy * p2.Qyy + 2 * p1.Qyz * p2.Qyz + p1.Qzz * p2.Qzz);
  ...
  if (p2._rank > 0) {
    ...
    if (p2._rank > 1) {
      ...
      if (p1._rank > 0) {
        ppG2 += 2 * (p2.Qxx * p1.Q1x * rx + p2.Qxy * p1.Q1x * ry +
                     p2.Qxz * p1.Q1x * rz + p2.Qxy * p1.Q1y * rx +
                     p2.Qyy * p1.Q1y * ry + p2.Qyz * p1.Q1y * rz +
                     p2.Qxz * p1.Q1z * rx + p2.Qyz * p1.Q1z * ry +
                     p2.Qzz * p1.Q1z * rz);
        puG2 += 2 * (p2.Qxx * p1.U1x * rx + p2.Qxy * p1.U1x * ry +
                     p2.Qxz * p1.U1x * rz + p2.Qxy * p1.U1y * rx +
                     p2.Qyy * p1.U1y * ry + p2.Qyz * p1.U1y * rz +
                     p2.Qxz * p1.U1z * rx + p2.Qyz * p1.U1z * ry +
                     p2.Qzz * p1.U1z * rz);
        ppG3 += -mu1_r * Q2__R;
        puG3 += -u_mu1_r * Q2__R;
...
//     double &TwoPi_V) {
//     assert(false); // This function is correct, but not necessarily
//     efficient,
...
inline double EwdInteractor::PhiPU12_ERFC_At_By(APolarSite &p1,
                                                APolarSite &p2) {
...
inline EWD::triple<double> EwdInteractor::U12_ERFC(APolarSite &p1,
  ...
  if (R1 < 1e-1) {
    std::cout << std::endl
              << "small small " << p1.getPos() << " == " << p2.getPos()
              << std::flush;
  }

  double pp = ppG0 * B0 + ppG1 * B1 + ppG2 * B2 + ppG3 * B3 + ppG4 * B4;
  double pu = (ta1 * tu3 < 40)
                  ? puG1 * lp3 * B1 + puG2 * lp5 * B2 + puG3 * lp7 * B3
                  : puG1 * B1 + puG2 * B2 + puG3 * B3;
  double uu = (ta1 * tu3 < 40) ? uuG1 * l3 * B1 + uuG2 * l5 * B2
                               : uuG1 * B1 + uuG2 * B2;
...
inline EWD::triple<double> EwdInteractor::U12_ERF(APolarSite &p1,
  ...
  if (R1 < 1e-2) {
    // cout << endl << "small small " << p1.getPos() << " == " << p2.getPos() <<
    // flush;
    pp += 2. * a1 * rSqrtPi * (p1.Q00 * p2.Q00);
    uu += 4. / 3. * a3 * rSqrtPi *
          (p1.U1x * p2.U1x + p1.U1y * p2.U1y + p1.U1z * p2.U1z);
    if (p1._rank > 0 && p2._rank > 0) {
      pp += 4. / 3. * a3 * rSqrtPi *
            (p1.Q1x * p2.Q1x + p1.Q1y * p2.Q1y + p1.Q1z * p2.Q1z);
      if (p1._rank > 1 && p2._rank > 1) {
        pp += 16. / 5. * a5 * rSqrtPi *
              (p1.Qxx * p2.Qxx + 2 * p1.Qxy * p2.Qxy + 2 * p1.Qxz * p2.Qxz +
               p1.Qyy * p2.Qyy + 2 * p1.Qyz * p2.Qyz + p1.Qzz * p2.Qzz);
      }
    }
    if (p1._rank > 0) {
      pu += 4. / 3. * a3 * rSqrtPi *
            (p1.Q1x * p2.U1x + p1.Q1y * p2.U1y + p1.Q1z * p2.U1z);
    }
    if (p2._rank > 0) {
      pu += 4. / 3. * a3 * rSqrtPi *
            (p1.U1x * p2.Q1x + p1.U1y * p2.Q1y + p1.U1z * p2.Q1z);
    }

    //        u12 =  2.   *a1*EWD::rSqrtPi * (p1.Q00*p2.Q00)
    //            +  4./3.*a3*EWD::rSqrtPi * (p1.Q1x*p2.Q1x + p1.Q1y*p2.Q1y +
    //            p1.Q1z*p2.Q1z)
    //            + 16./5.*a5*EWD::rSqrtPi * (p1.Qxx*p2.Qxx + 2*p1.Qxy*p2.Qxy +
    //            2*p1.Qxz*p2.Qxz
    //                                                 +   p1.Qyy*p2.Qyy +
    //                                                 2*p1.Qyz*p2.Qyz
    //                                                                   +
...
inline double EwdInteractor::gG2(APolarSite &p1, APolarSite &p2) {
  return 2 * (p1.Qxx * p2.Qxx + 2 * p1.Qxy * p2.Qxy + 2 * p1.Qxz * p2.Qxz +
         ...
         - 2 * (p1.Qxx * p2.Q1x * rx + p1.Qxy * p2.Q1x * ry +
                p1.Qxz * p2.Q1x * rz + p1.Qxy * p2.Q1y * rx +
                p1.Qyy * p2.Q1y * ry + p1.Qyz * p2.Q1y * rz +
                p1.Qxz * p2.Q1z * rx + p1.Qyz * p2.Q1z * ry +
                p1.Qzz * p2.Q1z * rz) +
         2 * (p2.Qxx * p1.Q1x * rx + p2.Qxy * p1.Q1x * ry +
              p2.Qxz * p1.Q1x * rz + p2.Qxy * p1.Q1y * rx +
              p2.Qyy * p1.Q1y * ry + p2.Qyz * p1.Q1y * rz +
              p2.Qxz * p1.Q1z * rx + p2.Qyz * p1.Q1z * ry +
              p2.Qzz * p1.Q1z * rz);
...
inline double EwdInteractor::gG3(APolarSite &p1, APolarSite &p2) {
  return -4 * ((p1.Qxx * rx + p1.Qxy * ry + p1.Qxz * rz) *
                   (p2.Qxx * rx + p2.Qxy * ry + p2.Qxz * rz) +
               (p1.Qxy * rx + p1.Qyy * ry + p1.Qyz * rz) *
                   (p2.Qxy * rx + p2.Qyy * ry + p2.Qyz * rz) +
               (p1.Qxz * rx + p1.Qyz * ry + p1.Qzz * rz) +
               (p2.Qxz * rx + p2.Qyz * ry + p2.Qzz * rz))
...
inline double EwdInteractor::gG4(APolarSite &p1, APolarSite &p2) {
  return (p1.Qxx * rxx + 2 * p1.Qxy * rxy + 2 * p1.Qxz * rxz + p1.Qyy * ryy +
          2 * p1.Qyz * ryz + p1.Qzz * rzz) *
         (p2.Qxx * rxx + 2 * p2.Qxy * rxy + 2 * p2.Qxz * rxz + p2.Qyy * ryy +
          2 * p2.Qyz * ryz + p2.Qzz * rzz);
}
...
inline void EwdInteractor::PUApplyBiasK(APolarSite &p) {
  ...
  if (p._rank > 0) {
    ...
    if (p._rank > 1) {
      Qk = p.Qxx * kxx + 2 * p.Qxy * kxy + 2 * p.Qxz * kxz + p.Qyy * kyy +
           2 * p.Qyz * kyz + p.Qzz * kzz;
    } else
      Qk = 0.0;
  } else {
    dk = 0.0;
  ...
  kr = kx * p.getPos()(0) + ky * p.getPos()(1) + kz * p.getPos()(2);
  coskr = cos(kr);
  sinkr = sin(kr);

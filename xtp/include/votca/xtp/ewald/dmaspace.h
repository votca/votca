...
class RegularSphericalHarmonics {
 public:
  RegularSphericalHarmonics(vec r)
      : _x(r(0)), _y(r(1)), _z(r(2)), _r2(r.dot(r)) {}
  ...
  inline cmplx C1_1() {
    return std::sqrt(0.5) * cmplx(+1 * R11c(), -1 * R11s());
  }
  ...
  inline cmplx C2_2() {
    return std::sqrt(0.5) * cmplx(+1 * R22c(), -1 * R22s());
  }
  inline cmplx C2_1() {
    return std::sqrt(0.5) * cmplx(+1 * R21c(), -1 * R21s());
  }
...
class ComplexSphericalMoments {
 public:
  ...
  inline cmplx X1_1() {
    return std::sqrt(0.5) * cmplx(+1 * Q11c(), -1 * Q11s());
  }
  ...
  inline cmplx X2_2() {
    return std::sqrt(0.5) * cmplx(+1 * Q22c(), -1 * Q22s());
  }
  inline cmplx X2_1() {
    return std::sqrt(0.5) * cmplx(+1 * Q21c(), -1 * Q21s());
  }
...
class RealSphericalMoments {
 public:
  ...
  inline double Q11s() {
    cmplx q = std::sqrt(0.5) * cmplx(0, 1) * (X1_1() + X1x1());
    assert(q.Im() < 1e-8);
    return q.Re();
  }
  inline double Q20() { return X20().Re(); }
  inline double Q21c() {
    cmplx q = std::sqrt(0.5) * cmplx(1, 0) * (X2_1() - X2x1());
    assert(q.Im() < 1e-8);
    return q.Re();
  }
  inline double Q21s() {
    cmplx q = std::sqrt(0.5) * cmplx(0, 1) * (X2_1() + X2x1());
    assert(q.Im() < 1e-8);
    return q.Re();
  }
  inline double Q22c() {
    cmplx q = std::sqrt(0.5) * cmplx(1, 0) * (X2_2() + X2x2());
    assert(q.Im() < 1e-8);
    return q.Re();
  }
  inline double Q22s() {
    cmplx q = std::sqrt(0.5) * cmplx(0, 1) * (X2_2() - X2x2());
    assert(q.Im() < 1e-8);
    return q.Re();
  }

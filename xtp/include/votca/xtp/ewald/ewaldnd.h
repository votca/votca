...
class Ewald3DnD {
 ...
 public:
  Ewald3DnD(const Topology *top, PolarTop *ptop, tools::Property *opt,
            Logger *log);
  virtual ~Ewald3DnD();
  ...
  virtual EWD::triple<> ConvergeReciprocalSpaceSum(
      std::vector<PolarSeg *> &target) = 0;
  ...
  void EvaluatePotentialGrid(std::vector<PolarSeg *> &target, Vxc_Grid &grid, bool add_bg,
                         bool add_mm1, bool add_qm0);

  virtual void Potential_ConvergeRealSpaceSum_Grid(std::vector<PolarSeg *> &, Vxc_Grid &) {
    ;
  }
   ...
   public:
    ...
    ForegroundTable(int n_segs_cell, int na_max, int nb_max, int nc_max)
        : _na_max(na_max),
          ...
          _nc_max(nc_max),
          _id_na_nb_nc__inFg(
              fgtable_t(boost::extents[n_segs_cell][2 * na_max + 1]
                                      [2 * nb_max + 1][2 * nc_max + 1])) {
    ...
    bool IsInForeground(int segid, int na, int nb, int nc) {
      bool is_in_fg;
      if (std::abs(na) > _na_max || std::abs(nb) > _nb_max ||
          std::abs(nc) > _nc_max) {
        is_in_fg = false;
      } else {
        idx_t i = segid;

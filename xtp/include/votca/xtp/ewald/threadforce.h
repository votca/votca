#ifndef VOTCA_CTP_THREADFORCE_H
#define VOTCA_CTP_THREADFORCE_H
...
class ThreadForce : public std::vector<ThreadType *>,
 ...
 public:
  ...
  template <class in_t>
  void AddAtomicInput(std::vector<in_t> &inputs) {
    // Apportion the vector elements equally onto all threads
    typename std::vector<in_t>::iterator iit;
    int in_idx = 0;
    int th_idx = 0;
  ...
  void Disband() {
    // Delete & clear
    it_t tfit;
    for (tfit = this->begin(); tfit < this->end(); ++tfit) delete *tfit;
    this->clear();
    return;
  }
...
class MultiModeThread : public ThreadBase {
 public:
  ...
  const std::string &getMode() { return _current_mode; }
  void setMode(const std::string &mode) { _current_mode = mode; }
  void setVerbose(bool verbose) { _verbose = verbose; }

  void RegisterStart(std::string mode, void (ThreadDerived::*member)()) {
    _mode_startfunct[mode] = static_cast<StartFunct>(member);
  }

  void RegisterReset(std::string mode, void (ThreadDerived::*member)()) {
    _mode_resetfunct[mode] = static_cast<ResetFunct>(member);
  }

  void RegisterWload(std::string mode, double (ThreadDerived::*member)()) {
  ...
  void Reset(std::string mode) {
    ResetFunct reset = _mode_resetfunct[mode];
    ((*this).*reset)();
  }

  double Workload(std::string mode) {
 ...
 protected:
  ...
  std::map<std::string, StartFunct> _mode_startfunct;
  std::map<std::string, ResetFunct> _mode_resetfunct;
  std::map<std::string, WloadFunct> _mode_wloadfunct;
};

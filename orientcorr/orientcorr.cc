...
void OrientCorrApp::Initialize() {
    ...
    AddProgramOptions("Neighbor search options")
        ("cutoff,c", boost::program_options::value<double>(&_cut_off)->default_value(1.0), 
            "cutoff for the neighbor search")
        ("nbins", boost::program_options::value<int>(&_nbins)->default_value(40), 
             "number of bins for the grid")
        ("nbmethod", boost::program_options::value<string>(&_nbmethod)->default_value("grid"),
...
NBList *OrientCorrApp::CreateNBSearch()
{
    ...
    throw std::runtime_error("unknown neighbor search method, use simple or grid");
    return NULL;
}
...
void OrientCorrApp::MergeWorker(Worker *worker) {
    ...
    _count_excl.data().y() = _count_excl.data().y() + myWorker->_count_excl.data().y();

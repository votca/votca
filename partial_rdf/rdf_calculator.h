...
public:
    ...
    double AnalyticVolumeCorrection(double t) {
        ...
        cout << "DBG " <<t << " " << 1.0/24.0*(16.0*t*t-12.0*t*t*t+t*t*t*t*t) << endl;
...
protected:
    ...
    public:
        ...
        double _cur_vol;
        double _cur_beadlist_1_count; //need to normalize to avg density for subvol
...
public:
 ...
 inline RDFCalculator::pair_t::pair_t(RDFCalculator::interaction_t *i1, RDFCalculator::interaction_t *i2,
         int offset_i, int offset_j, const pair_matrix &corr)
         :

...
If you do not have a .pdb/.gro file and want to read trajectory from LAMMPS .dump file or H5MD then it is also possible to directly define topology in \xml file.
Here is an example of such file for the case of butane, where trajectory is read from H5MD file
\begin{lstlisting}
<topology>
...
The section ordered list \texttt{molecules} contains information about the beads in the trajectory file.
The beads \texttt{<bead>} will be replicated \texttt{nmols} times and then the next \texttt{molecule} will be use.

The H5MD format contains information about box size but one can it define directly in the \xml file by tag
\begin{lstlisting}
...
For historical reasons, \prog{csg_boltzmann} uses a slightly different table format, it has no \texttt{flag} column and uses the third column as a force column when outputting a potential.

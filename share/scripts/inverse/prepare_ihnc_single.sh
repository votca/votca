...
initial_guess="$(csg_get_property cg.inverse.ihnc.initial_guess)"
...
"hnc")
    ...
    kbt="$(csg_get_property cg.inverse.kBT)"
    density="$(csg_get_property cg.inverse.ihnc.density)"
    cut_off="$(csg_get_property cg.inverse.ihnc.cut_off)"
    do_external dist invert_hnc ${name}.dist.tgt "${kbt}" "${density}" "${cut_off}" ${raw}
    ;;
*)
    die "cg.inverse.ihnc.initial_guess has to be either table, bi or hnc"
    ;;
esac

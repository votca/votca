...
if (defined($ARGV[0])&&("$ARGV[0]" eq "--help")){
  print <<EOF;
$progname, %version%
This script calculates the LJ12-6+CKD+gaussian potential
...
my $r_cut_max=csg_get_property("cg.non-bonded.max");
my $pi=3.14159265;

if ($r_cut_tot>=$r_cut_max) {die "$progname: Cutoff $r_cut_max exceeded!\n";}
...
for (my $i=0;$i<=$#r;$i++){
    ...
    if ($r[$i]>1e-10) {
      ...
      if ($r[$i]<=$r_cut_rep) {
        ...
        $pot_rep[$i]+=$height*exp(-($r[$i]-$shift)**2/(2*$stdev**2));
      }
      else {
        $pot_rep[$i]=0;
      }
      ...
      if ($r[$i]<$r_cut_rep) {
        ...
        $pot_att[$i]+=$height*exp(-($r[$i]-$shift)**2/(2*$stdev**2));
      }
      elsif ($r[$i]>$r_cut_tot && $r[$i]<=$r_cut_max) {
        $pot_att[$i]=$height*exp(-($r[$i]-$shift)**2/(2*$stdev**2));
      }
      else {
        $pot_att[$i]=0;

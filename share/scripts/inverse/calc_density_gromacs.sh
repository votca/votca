...
log "Running csg_density for ${type1}-${type2}"
if is_done "dens-$name"; then
  msg "csg_density for ${type1}-${type2} is already done"
else
  run_or_exit csg_density --trj traj.trr --top "$tpr" --out ${name}.dens.new.xvg --begin ${begin} --first-frame $first_frame --molname '*' --axis z --bins $bins

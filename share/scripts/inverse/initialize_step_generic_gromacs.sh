...
USES: die cp successful_or_die check_deps get_last_step_dir
...
successful_or_die mv confout.gro conf.gro
...
successful_or_die $grompp -n "${index}" -f "${mdp}" -p "$top" -o "$tpr" ${opts}

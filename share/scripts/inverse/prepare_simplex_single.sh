...
This script
- reads sinple interaction simplex infile
...
[[ ${#parameters[@]} -gt 0 ]] || die "${0##*/}: property inverse.simplex.parameters for interaction $name should contain more than 1 parameter"
...
if [[ -f $main_dir/$input ]]; then
  ...
  [[ ${#n[@]} -eq $total_parameter ]] || die "${0##*/}: We need $total_parameter of parameter sets (lines) in $input, but got ${#n[@]} lines"
  for ((i=0;i<${#n[@]};i++)); do

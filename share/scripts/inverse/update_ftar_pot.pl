...
my $aim_pot_file="$name.pot.tgt";
my $cur_pot_file="$name.pot.cur";

my @r_aim;
my @pot_aim;
my @flags_aim;
(readin_table($aim_pot_file,@r_aim,@pot_aim,@flags_aim)) || die "$progname: error at readin_table\n";

my @r_cur;
my @pot_cur;
my @flags_cur;
(readin_table($cur_pot_file,@r_cur,@pot_cur,@flags_cur)) || die "$progname: error at readin_table\n";
...
for(my $i=1;$i<($max-$min)/$dr;$i++) {
       $dpot[$i]=abs($pot_cur[$i]-$pot_aim[$i]);
       $ftar+=$dr*$dpot[$i];
       $ftar_aim+=$dr*$pot_aim[$i];
}
$ftar+=(0.5*$dr*abs($pot_cur[0]-$pot_aim[0]))+(0.5*$dr*abs($pot_cur[$max/$dr]-$pot_aim[$max/$dr]));
$ftar_aim+=(0.5*$dr*$pot_aim[0])+(0.5*$dr*$pot_aim[$max/$dr]);

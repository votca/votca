...
USES:  do_external run_or_exit csg_get_interaction_property csg_get_property csg_resample check_deps
...
if [ "${scheme[$scheme_nr]}" = 1 ]; then
  ...
  do_external dpot shift_nb ${name}.dpot.after_crop ${name}.dpot.new
else

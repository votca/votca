...
merged into the master branch of a child repository. If this occurs the
...
Developers can add bug fixes by making a pull request with the stable branch as target.

As the :code:`master` branch of each repo is a child of each repo's :code:`stable` branch,  
any bugfix added to a repos stable branch will also need to be pulled into its master branch. If the bugfix
is added in one of the child repositories (not votca/votca) then votca/votca will also need to
reflect these changes.

Keeping the repositories synchronised can be difficult. In order to help keep the master branches and votca/votca
synchronised with changes in the stable branch of a child repository the generation of four pull requests are
automatically generated anytime a bugfix is made to the stable branch of a child repository.

E.g. if :code:`hot-bug-fix` is merged into the :code:`stable` branch of tools:

1. A pull request is created to merge :code:`stable` from tools (child repo) into :code:`master` of tools (child repo).
2. A pull request is created to merge :code:`stable` from tools (child repo) into :code:`stable` of votca/votca (parent repo). This shoud consiste of updating the submodules in the :code:`stable` branch of votca/votca.
3. A pull request is created to merge :code:`master` from tools (child repo) into :code:`master` of votca/votca (parent repo). Again this should consist of updating the submodules but in the :code:`master` branch of votca/votca.
4. Finally, a pull request is made from :code:`stable` from votca/votca (parent repo) to :code:`master` of votca/votca (parent repo).

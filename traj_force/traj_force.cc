...
void TrajForce::Initialize(void)
{
  ...
  AddProgramOptions()
    ("scale", boost::program_options::value<double>(&_scale)->default_value(-1.0),
...
void TrajForce::BeginEvaluate(Topology *top, Topology *top_atom)
{
  ...
  if(_trjwriter == NULL)
    throw runtime_error(string("output trajectory format not supported: ") + _op_vm["out"].as<string>());
  bool append = true;
...
void TrajForce::EvalConfiguration(Topology *conf, Topology *conf_atom)
{
  if(conf->BeadCount() != _top_force.BeadCount())
    throw std::runtime_error("number of beads in topology and reference force topology does not match");
  for(int i=0; i<conf->BeadCount(); ++i)
    {
      ...
      _top_force.getBead(i)->F() = conf->getBead(i)->getF() + _scale*_top_force.getBead(i)->getF();
      vec d = conf->getBead(i)->getPos() - _top_force.getBead(i)->getPos();
      if(abs(d) > 1e-6)
        throw std::runtime_error("One or more bead positions in trajectory and reference force trajectory differ by more than 1e-6");
    }

...
if [[ ${INPUT_MINIMAL} = true ]]; then
  cmake_args+=( -DCMAKE_DISABLE_FIND_PACKAGE_HDF5=ON -DCMAKE_DISABLE_FIND_PACKAGE_FFTW3=ON -DCMAKE_DISABLE_FIND_PACKAGE_MKL=ON -DBUILD_MANPAGES=OFF -DBUILD_XTP=OFF )
  if [[ ${branch} != "stable" ]]; then
    cmake_args+=( -DCMAKE_DISABLE_FIND_PACKAGE_GROMACS=ON )
  fi

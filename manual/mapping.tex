...
In order to check the mapping one can use \texttt{ctp\_printqmattop} 
\begin{verbatim}
  \texttt{ctp_printqmattop} --
\end{verbatim}

This programme will read in the state from ctp\_map together with a list charge unit description and will create two output trajectory files corresponding to the coarse grained and quantum topologies. The coase grained topology is defined by two vectors per bead. The quantum topology has the coordinates from INPUT\_COORD backmapped onto the coarse grained topology. It is suggested to view all three file (atomistic, coarse grained and quantum) on top of each other to check the mapping before proceding.

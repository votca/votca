...
Evaluation of the electrostatic contribution is provided by the \calc{ecoulomb} \calculator.
Atomistic partial charges for charged an neutral molecule are taken from files specified in \xmlsegments and there are two screening methods for computing $E_{i}^\text{el}$.
Coulomb interactions can be screened by a constant relative dielectric constant $\epsilon_s$ as indicated in \equ{estatic}.
In this case only $\epsilon_s$ has to be specified in \xmloptions.

Alternatively the screening can be made distance-dependent by using \equ{epss}. In this case the relative dielectric constant $\epsilon_s$ and the inverse screening length $s$ have to be provided in the \xmloptions.
A cutoff radius based on molecular centers of mass can be given in  \xmloptions to speed up calculations for both methods.

The electrostatic site-energies are saved to the \sqlstate file.

\small \ctprun \opt \xmloptions  \seg  \xmlsegments \sql  \sqlstate \exe  \calc{ecoulomb}

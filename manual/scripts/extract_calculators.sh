...
echo "\subsection{Calculators}"

for file in latex/*.tex; do
  name=$(sed -ne '/\\subsection{Detailed Description}/,/\\subsection/p' $file | awk '/Callname/{print $3}')
  if [ ! -z "$name" ]; then
    echo "Generating documentation for calculator $name from $file"
    echo "\subsubsection{$name}" >> $out
    sed -ne '/\\subsection{Detailed Description}/,/\\subsection/p' $file | sed -e '1d' -e '$d' -e '/The documentation for this class was generated from the following files/,$d' -e '/Callname/d' >> $out
  fi
done
